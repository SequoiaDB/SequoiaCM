...
function iterDir( $root, $path, $edition, $type )
{
   if( $type == 'document' )
   {
      sendMkdir( "https://www.sequoiadb.com/cn/receiveImage.php",
                 "./index/Public/Home/$type/scm/$edition" ) ;
   }
   $ret = sendMkdir( "https://www.sequoiadb.com/cn/receiveImage.php",
                     $type == 'document' ? "./index/Public/Home/$type/scm/$edition/api/$path" : "./index/Public/Home/$type/scm/$edition/$path" ) ;
   if( $ret['errno'] != true )
   {
      echo "Failed to create dir, path: $root/$path\n" ;
      echo $type == 'document' ? "Dest path: ./index/Public/Home/$type/scm/$edition/api/$path\n" : "Dest path: ./index/Public/Home/$type/scm/$edition/$path\n" ;
      return false ;
   }
   $res = dir( "$root/$path" ) ;
   while( $file = $res->read() )
      ...
      if( $file != '.' && $file != '..' )
      {
         ...
         else
         {
            $ret = sendStreamFile( "https://www.sequoiadb.com/cn/receiveImage.php", getOSInfo() == 'linux' ? "$root/$path/$file" : "$root\\$path\\$file",
                                   $type == 'document' ? "./index/Public/Home/$type/scm/$edition/api/$path/$file" : "./index/Public/Home/$type/scm/$edition/$path/$file" ) ;
            if( $ret['errno'] != true )
            {
               echo "Failed to send image, path: $root/$path/$file\n" ;
               echo $type == 'document' ? "Dest path: ./index/Public/Home/$type/scm/$edition/api/$path/$file\n" : "Dest path: ./index/Public/Home/$type/scm/$edition/$path/$file\n" ;
               return false ;
            }
         }
      }
