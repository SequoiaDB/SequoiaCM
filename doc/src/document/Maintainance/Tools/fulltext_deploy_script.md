Fulltext-Server 安装包下的 deploy.py 提供 Fulltext-Server 服务的部署功能，deploy.py 本质上是通过 Fulltext-Server 管理工具、Fulltext-Server 节点控制工具实现的一个部署脚本，用户只需要配置一份 deploy.json 文件，就可以通过该脚本搭建起 Fulltext-Server 服务。
