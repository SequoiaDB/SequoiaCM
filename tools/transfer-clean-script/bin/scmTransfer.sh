...
commandStr="java -Xmx2048m -jar $jarPath --logbackPath $transferLogbackPath --fileMatcher '{ create_month: \"${ts_month}\" }' --sdbCoord ${metaSdbCoord} --sdbUser ${metaSdbUser} --sdbPassword ${metaSdbPassword} --sdbPasswordFile ${metaSdbPasswordFile} --scmPassword ${scmPassword} --scmPasswordFile ${scmPasswordFile} --srcSiteName ${srcSiteName} --targetSiteName ${targetSiteName} --url ${url} --scmUser ${scmUser} --workspace ${ts_workspace} --batchSize ${batchSize} --fileTransferTimeout ${fileTransferTimeout} --fileStatusCheckBatchSize ${fileStatusCheckBatchSize} --fileStatusCheckInterval ${fileStatusCheckInterval} --sdbConnectTimeout ${connectTimeout} --sdbSocketTimeout ${socketTimeout} --fileScope ${fileScope}"
