...
/**
 * A站点并发清理不同文件，文件内容存在于 A、B站点
 */
public class ConcurrentCleanDiffFileA extends ScmTestMultiCenterBase {
    private static final Logger logger = Logger.getLogger(ConcurrentCleanDiffFileA.class);
    ...
    private void prepareFile() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            ScmFactory.File.getInstance(wsSite1Session, file.getFileId()).getContent(new NullOutputStream());
        }
        for (int i = 0; i < 10; i++) {
            ScmFile file = ScmTestTools.createScmFile(wsSite2Session, 10);
