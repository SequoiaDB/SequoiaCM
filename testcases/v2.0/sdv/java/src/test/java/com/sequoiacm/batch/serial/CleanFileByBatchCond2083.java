...
public class CleanFileByBatchCond2083 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        startCleanTaskByBatch( wsA, sessionA, batchId1 );

        checkCleanFileSiteAndDataInfo( wsA, fileName1 );
        checkNoCleanFileInfo( wsM, batchId2 );
        runSuccess = true;
    }
    ...
    private void readFileFromM( ScmWorkspace ws ) throws Exception {
        int currentVersion = 2;
        int historyVersion = 1;
        for ( int i = 0; i < fileIdList.size(); i++ ) {
            // read current version file
            ScmFile currentFile = ScmFactory.File.getInstance( ws,
                    new ScmId( fileIdList.get( i ) ), currentVersion, 0 );
            ...
            ScmFile historyFile = ScmFactory.File.getInstance( ws,
                    new ScmId( fileIdList.get( i ) ), historyVersion, 0 );
