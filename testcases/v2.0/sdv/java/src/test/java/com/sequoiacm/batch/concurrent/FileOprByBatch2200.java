...
public class FileOprByBatch2200 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		for(  CreateFileThread createFile : createFiles ){
			Assert.assertTrue( createFile.isSuccess(), createFile.getErrorMsg());
		}
	...
	private class CreateFileThread extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {				
				ScmWorkspace wsTmp = ScmFactory.Workspace.getWorkspace(wsp.getName(), session);	
		...
		public void exec() throws Exception {
			...
			try {	
				...
				try {
					...
					//System.out.println("---remove file and get error is :"+e.getError());
					if (ScmError.FILE_NOT_FOUND!= e.getError()) {
						Assert.fail("expError:FILE NOT FOUND  actError:" + e.getError() + e.getMessage());
					}
				}			
			}finally {
				if (session != null) {
	...
	private void createFile( ScmWorkspace ws, byte[] data, int fileNums ) throws ScmException  {		
	...
	private void checkFileNumByBatch(ScmWorkspace ws) throws ScmException, InterruptedException{
		int expFileNums = 50;
		ScmBatch batch = ScmFactory.Batch.getInstance(ws, batchId);
	...
	private void deleteBatchAndCheckResult(ScmWorkspace ws) throws InterruptedException, ScmException{
			//check delete result
			ScmFactory.Batch.deleteInstance(ws, batchId);	
		
			//check file by Batch
			BSONObject fileCondition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(authorName).get();
			long remainFileNum = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, fileCondition);
			int expFileNum = 0;
