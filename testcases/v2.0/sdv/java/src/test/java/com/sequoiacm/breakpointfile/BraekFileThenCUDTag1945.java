...
public class BraekFileThenCUDTag1945 extends TestScmBase {
	...
	private void breakpointFile2ScmFile(ScmBreakpointFile breakpointFile, String name,ScmTags tags)
			throws ScmException {
	...
	private ScmTags creatTags(String name, boolean mutil) throws ScmException {
		tags = new ScmTags();
		if (!mutil) {
	...
	private void check(ScmId fileId, ScmTags tag) throws ScmException {
		...
		Assert.assertEquals(file.getTags().toSet().containsAll(tag.toSet()),true,
				"fileTags = " + file.getTags().toString() + ",expTags = " + tag.toString());
	}
