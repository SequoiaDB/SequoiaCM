...
/**
 * test content:specify directory to create multiple files,than transfer and clean file mathching directory
...
public class TransferAndCleanFile2060 extends TestScmBase {
	...
	private String fullPath = dirBasePath + "/2060_a/2060_b/2060_c/2060_e/2060_f/";
	...
	private void writeFileWithDir(ScmWorkspace ws) throws ScmException{
		...
		for (int i = 0; i < fileNum; i++) {
			...
			ScmId fileId = null;
			if( i % 2 == 0 ){
				fileId = createFileWithDir( ws, subfileName, writeData1, authorName ,scmDir);
			}else{
				fileId = createFileWithDir( ws, subfileName, writeData2, authorName ,scmDir);
			}			
			fileIdList.add(fileId.get());			
		}
	}
	
	private void startTransferTask(ScmWorkspace ws, ScmSession session) throws Exception {		
		condition = ScmQueryBuilder.start(ScmAttributeName.File.DIRECTORY_ID).is(scmDir.getId())
				.put(ScmAttributeName.File.SIZE).is(writeData1.length)
				.put(ScmAttributeName.File.AUTHOR).is(authorName).get();		
		transferTaskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT,targetSite.getSiteName());
		
		//wait task finish
		ScmTaskUtils.waitTaskFinish(session, transferTaskId);		
	}	
	
	private void startCleanTask(ScmWorkspace ws, ScmSession session) throws Exception {
	...
	private void checkTransferResult( ScmWorkspace ws ) throws Exception{
		...
		BSONObject condition1 = ScmQueryBuilder.start(ScmAttributeName.File.DIRECTORY_ID).is(scmDir.getId())
				.put(ScmAttributeName.File.SIZE).is(writeData2.length)
				.put(ScmAttributeName.File.AUTHOR).is(authorName).get();	
		ScmCursor<ScmFileBasicInfo> cursor1 = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_CURRENT, condition1);
		int size1 = 0;			
	...
	private void checkCleanResult( ScmWorkspace ws ) throws Exception{
		...
		ScmFileBasicInfo file;		
		List<ScmId> transferfileIdList = new ArrayList<ScmId>();
		SiteWrapper[] expCurSiteList = { targetSite };
		while (cursor.hasNext()) {
			file = cursor.getNext();			
			ScmId fileId = file.getFileId();
			ScmFileUtils.checkMeta(ws, fileId, expCurSiteList);
