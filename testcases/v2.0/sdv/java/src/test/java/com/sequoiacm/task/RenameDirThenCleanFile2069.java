...
public class RenameDirThenCleanFile2069 extends TestScmBase {
	...
	private void setUp() throws Exception {
		...
		if (siteList.size() <= 1) {
			throw new SkipException("branch site's num is less than 2, Skip!");
		}
	...
	private void test() throws Exception {
		...
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(name)
				    .and(ScmAttributeName.File.DIRECTORY_ID).is(dir.getId()).get();
	...
	private void checkResult(List<ScmId> fileIdList, boolean flag) throws Exception {
		...
		if (!flag) {
			...
			for (ScmId fileId : fileIdList) {
				...
				ScmFileUtils.checkMetaAndData(wsp, fileId, (SiteWrapper[]) expSiteList, localPath, filePath);
			}
		}
	}
