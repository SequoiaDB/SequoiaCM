		...
		public void exec() throws Exception {
			...
			try {
				...
				for (int i = 0; i < fileNum; i++) {
					...
					SiteWrapper[] expSiteList = { sourceSite, targetSite };
					ScmTaskUtils.waitAsyncTaskFinished(wsA, fileIdList.get(j), expSiteList.length);
					ScmFileUtils.checkMetaAndData(ws_T,fileIdList.get(j), expSiteList, localPath, filePath);
				}
			} finally {
				if (sessionA != null) {
					sessionA.close();
	...
	private void waitTaskFinish(ScmSession session, ScmId taskId) throws Exception {
		...
		while (true) {
			...
			if (CommonDefine.TaskRunningFlag.SCM_TASK_FINISH == task.getRunningFlag() 
					|| CommonDefine.TaskRunningFlag.SCM_TASK_ABORT == task.getRunningFlag()) {
				break;
			} 
