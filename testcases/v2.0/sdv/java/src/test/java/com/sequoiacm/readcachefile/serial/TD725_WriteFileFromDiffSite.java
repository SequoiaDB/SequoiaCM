...
public class TD725_WriteFileFromDiffSite extends TestScmBase {
	...
	private List<ScmId> fileIdList1 = Collections.synchronizedList(new ArrayList<ScmId>());
	private List<ScmId> fileIdList2 = Collections.synchronizedList(new ArrayList<ScmId>());
	private List<ScmId> fileIdList3 = Collections.synchronizedList(new ArrayList<ScmId>());
	...
	private void test() {
		try {
			...
			TestTools.setSystemTime(rootSite.getNode().getHost(), new Date().getTime() - 1000 * 60 * 5); // 5min
			TestTools.setSystemTime(branSites.get(0).getNode().getHost(), new Date().getTime() - 1000 * 60 * 10); // 10min
			TestTools.setSystemTime(branSites.get(1).getNode().getHost(), new Date().getTime() - 1000 * 60 * 15); // 15min
			...
			if (!(WriteFromM.isSuccess() && WriteFromA.isSuccess() && WriteFromB.isSuccess())) {
				Assert.fail(WriteFromM.getErrorMsg() + WriteFromA.getErrorMsg() + WriteFromB.getErrorMsg());
			}

			// check results
			SiteWrapper[] expSites1 = { rootSite };
			ScmFileUtils.checkMetaAndData(wsp, fileIdList1, expSites1, localPath, filePath);

			SiteWrapper[] expSites2 = { branSites.get(0) };
			ScmFileUtils.checkMetaAndData(wsp, fileIdList2, expSites2, localPath, filePath);

			SiteWrapper[] expSites3 = { branSites.get(1) };
			ScmFileUtils.checkMetaAndData(wsp, fileIdList3, expSites3, localPath, filePath);

			// recovery host's system time
			TestTools.restoreSystemTime(rootSite.getNode().getHost());
			TestTools.restoreSystemTime(branSites.get(0).getNode().getHost());
			TestTools.restoreSystemTime(branSites.get(1).getNode().getHost());
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private class WriteFromM extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				Assert.assertEquals(file2.getUpdateTime().getTime(), createTime);
				Assert.assertEquals(file2.getLocationList().get(0).getDate().getTime(), createTime);
			} finally {
				if (session != null) {
					session.close();
				}
