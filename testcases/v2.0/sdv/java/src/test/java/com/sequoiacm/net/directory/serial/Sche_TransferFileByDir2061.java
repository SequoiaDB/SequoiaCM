...
/**
 * test content:create ScheduleCleanTask,match directory to transfer files in the directory
...
public class Sche_TransferFileByDir2061 extends TestScmBase {	
	...
	private void test() throws Exception {	
		...
		VersionUtils.checkScheTaskFileSites(wsT, fileIdList2,currentVersion,expCurSiteList2);			
		runSuccess = true;
	}
	...
	private void createScheduleTask(ScmSession session,ScmDirectory scmDir) throws ScmException {		
		BSONObject condition = ScmQueryBuilder.start().put(ScmAttributeName.File.DIRECTORY_ID).in(scmDir.getId()).get();					
