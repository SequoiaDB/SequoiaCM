...
public class MainCenterReadMutiWsFile732 extends TestScmBase {
	...
	private void test() throws Exception {
		ReadScmFile rThread1 = new ReadScmFile(rootSite, wsList.get(0), fileIdList.get(0));
		rThread1.start(10);

		ReadScmFile rThread2 = new ReadScmFile(rootSite, wsList.get(1), fileIdList.get(1));
		rThread2.start(10);

		ReadScmFile rThread3 = new ReadScmFile(rootSite, wsList.get(2), fileIdList.get(2));
		rThread3.start(10);
	...
	private void checkResult(ScmId fileId, WsWrapper wsp) throws ScmException {
		ScmSession session = null;
		try {
	...
	private void checkMetadataAndLobs() throws Exception {
		...
		ScmFileUtils.checkMetaAndData(wsList.get(0), fileIdList.subList(0, 1), expSites, localPath, filePath);
		ScmFileUtils.checkMetaAndData(wsList.get(1), fileIdList.subList(1, 2), expSites, localPath, filePath);
		ScmFileUtils.checkMetaAndData(wsList.get(2), fileIdList.subList(2, 3), expSites, localPath, filePath);
	}
