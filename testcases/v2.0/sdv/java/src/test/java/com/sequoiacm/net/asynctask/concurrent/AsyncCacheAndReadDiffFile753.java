...
public class AsyncCacheAndReadDiffFile753 extends TestScmBase {
	...
	private void test() {
		try {
			CacheFile cThread1 = new CacheFile(targetSite, fileIdList.get(0));
			cThread1.start(10);

			ReadFile rThred1 = new ReadFile(targetSite, fileIdList.get(1));
			rThred1.start(10);
	...
	private void checkResult() {
		try {
			...
			ScmTaskUtils.waitAsyncTaskFinished(ws, fileIdList.get(0), expSiteList.length);
