...
public class TD738_ReadAndCleanAndTransferInDiffWs extends TestScmBase {
	...
	private void test() {
		try {
			...
			if (!(readInWs.isSuccess() && transferInWs2.isSuccess() && cleanInWs3.isSuccess())) {
				Assert.fail(readInWs.getErrorMsg() + transferInWs2.getErrorMsg() + cleanInWs3.getErrorMsg());
			}
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
	...
	private void tearDown() throws ScmException {
		...
		try {
			if (runSuccess || forceClear) {
				...
				ScmWorkspace ws1 = ScmFactory.Workspace.getWorkspace(wsList.get(0).getName(), sessionM);
				for (ScmId fileId : fileIdList1) {
					ScmFactory.File.deleteInstance(ws1, fileId, true);
				}

				ScmWorkspace ws2 = ScmFactory.Workspace.getWorkspace(wsList.get(1).getName(), sessionM);
				for (ScmId fileId : fileIdList2) {
					ScmFactory.File.deleteInstance(ws2, fileId, true);
				}

				ScmWorkspace ws3 = ScmFactory.Workspace.getWorkspace(wsList.get(2).getName(), sessionM);
	...
	private class CrossReadFileInWs extends TestThreadBase {
		...
		private List<SiteWrapper> sites = ScmNetUtils.getSortSites(wsp);
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmFileUtils.checkMetaAndData(wsp, fileIdList1, expSites, localPath, filePath);
	...
	private class StartTransferTaskInWs2 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				...
				ScmId taskId = ScmSystem.Task.startTransferTask(wsA, condition,ScopeType.SCOPE_CURRENT, sites.get(1).getSiteName());
				// check task info
				...
				SiteWrapper[] expSites = { sites.get(0), sites.get(1) };
				ScmFileUtils.checkMetaAndData(wsp, fileIdList2, expSites, localPath, filePath);
	...
	private class StartCleanTaskInWs3 extends TestThreadBase {
		...
		public StartCleanTaskInWs3() throws Exception {
			...
			ScmSession sessionM = null;
			try {
				sessionM = TestScmTools.createSession(sites.get(1));
				ScmWorkspace wsM = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionM);
		...
		public void exec() throws Exception {
			try {
				...
				SiteWrapper[] expSites = { sites.get(1) };
				ScmFileUtils.checkMetaAndData(wsp, fileIdList3, expSites, localPath, filePath);
	...
	private void writeFile(ScmWorkspace ws, List<ScmId> fileIdList) throws ScmException {
	...
	private void readFile(ScmWorkspace ws, List<ScmId> fileIdList) throws Exception {
		for (int i = 0; i < fileNum; i++) {
