...
public class CreateScmFileInMutiWs715 extends TestScmBase {
	...
	private void test() {
		try {
			WriteScmFile wThread1 = new WriteScmFile(wsList.get(0).getName());
			wThread1.start();

			WriteScmFile wThread2 = new WriteScmFile(wsList.get(1).getName());
			wThread2.start();

			WriteScmFile wThread3 = new WriteScmFile(wsList.get(2).getName());
			wThread3.start();
	...
	private void checkFileAttributes(ScmFile file, ScmId fileId, String wsName) {
		try {
