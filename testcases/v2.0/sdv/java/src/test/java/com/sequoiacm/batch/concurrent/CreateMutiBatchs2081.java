...
public class CreateMutiBatchs2081 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		for(  CreateBatchThread createBatch : createBatchs ){
			Assert.assertTrue( createBatch.isSuccess(), createBatch.getErrorMsg());
		}	
		checkBatch(ws);
		runSuccess = true;
	...
	private ScmId createFile( ScmWorkspace ws, String fileName, byte[] data ) throws ScmException  {		
	...
	private void checkBatch(ScmWorkspace ws) throws ScmException, InterruptedException{
		int expBatchNum = 100;
		Assert.assertEquals(batchIdQue.size(), expBatchNum,"batch nums is error!");			
