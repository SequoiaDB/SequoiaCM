	...
	private class ReadFile extends TestThreadBase {
		...
		public void readFileFromSubCenterB() throws ScmException {
			...
			try {
				...
				tmpPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
						Thread.currentThread().getId());
		...
		public void readScmFileByOff(ScmInputStream sisImpl, String downloadPath) throws IOException {
			OutputStream fileOutputStream = null;
			try {
				fileOutputStream = new FileOutputStream(new File(downloadPath));
				byte[] buffer = new byte[fileSize];
				while (true) {
					int readSize = sisImpl.read(buffer, 0, fileSize - seekSize);
					if (readSize <= 0) {
						break;
					}
