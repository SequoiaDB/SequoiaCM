package com.sequoiacm.net.batch.serial;
...
public class TransferFileByBatchCond2082 extends TestScmBase {	
	...
	private String authorName = "file_batch_2082";
	...
	private void setUp() throws IOException, ScmException {			
		...
		while (cursor.hasNext()) {
			...
			ScmFactory.Batch.deleteInstance(wsT, batchId);
		}
		cursor.close();		
			    
		//create file and update content
		fileId1 = VersionUtils.createFileByStream(wsS, fileName1, writeData1, authorName);
		fileId2 = VersionUtils.createFileByStream(wsS, fileName2, writeData2, authorName);
	...
	private void test() throws Exception {	
		batchId1 = createBatchAndAttachFile( wsS, batchName1, fileId1);
		batchId2 = createBatchAndAttachFile( wsS, batchName2, fileId2);			
		startTransferTaskByBatch(wsS, sessionS, batchId1);
		
				
		checkTransferedFileSiteAndDataInfo(wsS, fileName1);
		checkNoTransferFileInfo(wsT, batchId2);	
		runSuccess = true;
	}
	...
	private void tearDown() {
		try {	
			if( runSuccess ){
				...
				ScmFactory.Batch.deleteInstance(wsS, batchId1);
				ScmFactory.Batch.deleteInstance(wsS, batchId2);			
			}					
		} catch (Exception e) {
	...
	private void startTransferTaskByBatch(ScmWorkspace ws, ScmSession session, ScmId batchId) throws Exception {
	...
	private void checkTransferedFileSiteAndDataInfo(ScmWorkspace ws,String fileName) throws Exception{
		int currentVersion = 2;			
		//check the transfered file,check the sitelist and data	,only transfered current file		
	...
	private void checkNoTransferFileInfo(ScmWorkspace ws,ScmId batchId) throws ScmException{
		int currentVersion = 2;
