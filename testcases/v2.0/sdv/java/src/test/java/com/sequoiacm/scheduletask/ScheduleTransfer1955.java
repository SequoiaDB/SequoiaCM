...
public class ScheduleTransfer1955 extends TestScmBase {
	...
	private void test() throws Exception {
		BSONObject queryCond = ScmQueryBuilder.start(ScmAttributeName.File.PROPERTIES+"." + name)
				.lessThan(fileNum/2)
				.and("tags")
		...
		ScmScheduleUtils.checkScmFile(ws, fileIdList.subList(0, fileNum/2), expSites);
		SiteWrapper[] expSites1 = { site };
		ScmScheduleUtils.checkScmFile(ws, fileIdList.subList( fileNum/2, fileNum), expSites1);
		runSuccess = true;
	}
	...
	private void createScheduleTask(BSONObject queryCond) throws ScmException {
		String maxStayTime = "0d";
		SiteWrapper rootSite = ScmInfo.getRootSite();
		// create schedule task
		content = new ScmScheduleCopyFileContent(
				site.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond);
		cron = "* * * * * ?";
		ScmSchedule sche = ScmSystem.Schedule.create(session, wsp.getName(),
				ScheduleType.COPY_FILE, name, "ScheduleTransfer1955", content, cron);
	...
	private ScmId createFile(String name,ScmClassProperties properties, ScmTags tag) throws ScmException, ParseException {
