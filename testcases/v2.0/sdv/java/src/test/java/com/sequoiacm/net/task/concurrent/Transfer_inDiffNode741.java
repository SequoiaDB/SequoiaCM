...
public class Transfer_inDiffNode741 extends TestScmBase {
	...
	private void setUp() throws ScmException {
		...
		try {
			...
			List<SiteWrapper> siteList1 = ScmNetUtils.getCleanSites(ws_TList.get(0));
			sourceSite1 = siteList1.get(0);
			targetSite1 = siteList1.get(1);
			List<SiteWrapper> siteList2 = ScmNetUtils.getCleanSites(ws_TList.get(1));
			...
			wsA1 = ScmFactory.Workspace.getWorkspace(ws_TList.get(0).getName(), session1);
			wsA2 = ScmFactory.Workspace.getWorkspace(ws_TList.get(1).getName(), session2);
	...
	private void test() {
		try {
			...
			if (!(tfWs1FromNode1.isSuccess() && tfWs2FromNode2.isSuccess())) {
				Assert.fail(tfWs1FromNode1.getErrorMsg() + tfWs2FromNode2.getErrorMsg());
			}
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		}
	...
	private class StartTransferTaskFromNode1 extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmId taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT, targetSite1.getSiteName());
				...
				ScmFileUtils.checkMetaAndData(ws_TList.get(0), fileIdList, expSiteIdList, localPath, filePath);
			} finally {
				if (session != null) {
					session.close();
				}
	...
	private class StartTransferTaskFromNode2 extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author).get();
				ScmId taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT, targetSite2.getSiteName());
