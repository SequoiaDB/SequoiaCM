...
public class ScmScheduleUtils extends TestScmBase {
	...
	public static void checkScmFile(ScmWorkspace ws, List<ScmId> fileIds, 
		...
		checkScmFile(ws, fileIds, 0, fileIds.size(), expSites, defaultTimeOut);		
	}
	...
	public static void checkScmFile(ScmWorkspace ws, List<ScmId> fileIds, int startNum, int endNum, 
			SiteWrapper[] expSites) throws Exception {
	...
	public static void cleanTask(ScmSession session, ScmId scheId) throws Exception {
		ScmCursor<ScmTaskBasicInfo> cursor = null;
		try {
			BSONObject cond = ScmQueryBuilder.start("schedule_id").is(scheId.get()).get();
	 ...
	 */
	public static boolean isRunningOfSche(ScmSession session, ScmId scheduleId) throws Exception {
		BSONObject cond = ScmQueryBuilder.start("schedule_id").is(scheduleId.get()).get();
	 ...
	 */
	public static boolean isRunningOfSche(ScmSession session, ScmId scheduleId, int taskType) throws Exception {
		BSONObject condition = ScmQueryBuilder.start("schedule_id").is(scheduleId.get())
	 ...
	 * judge by condition
	 */
	public static boolean isRunningOfSche(ScmSession session, ScmId scheduleId, BSONObject condition) throws Exception {
		ScmCursor<ScmTaskBasicInfo> cursor = null;
		...
		try {
			while (true) {
				...
				if (isRunning) {
					logger.info("retryTimes = " + retryTimes + ", retry time = " + retryTimes * sleepTime);
					break;
	...
	public static void outputTaskInfo(ScmSession session, ScmId scheduleId) throws ScmException {
	...
	public static void outputScmfileInfo(ScmWorkspace ws, List<ScmId> fileIds) throws ScmException {
		List<String> infoList = new ArrayList<>();
		for (ScmId fileId : fileIds) {
			ScmFile file = ScmFactory.File.getInstance(ws, fileId);	
			infoList.add(file.toString() + ", " + file.getLocationList() + "\n");
		}
		logger.info("ws = " + ws.getName() + ", file info \n" + infoList);
	}

	public static ScmId getTaskId(ScmSession session, ScmId scheduleId) throws Exception {
		ScmCursor<ScmTaskBasicInfo> cursor = null;
		ScmId taskId = null;
		try {
			BSONObject cond = ScmQueryBuilder.start("schedule_id").is(scheduleId.get()).get();
	...
	/**
	 * scheduleServer host time is not sync with SCM host time, cause may not match scmFile
	 * need to add sleep strategy
	 */
	public static long sleepStrategy(ScmSession session, ScmWorkspace ws, ScmId scheduleId, ScmId fileId, int expEstCount) throws Exception {	
		long sleepTime = 0;
...
//			System.out.println("content: " + content);
			BSONObject condA = (BSONObject) ((BasicBSONList) cond.get("$and")).get(0);
			long scheMatchTime = (long) ((BSONObject) ((BSONObject) ((BSONObject) condA.get("site_list"))
					.get("$elemMatch"))
					.get("create_time"))
					.get("$lt");
			...
			if (timeDiff > 0 && timeDiff < maxDiffTime) {
				...
				logger.info("task content \n" + cond + "\nscheMatchTime: " + scheMatchTime);
				logger.info("file info \n" + file.toString() + "\nfileCreateTime: " + fileCreateTime);
				throw new Exception("the time difference between the schedule host and the scm host is more than "+maxDiffTime+".");
			} else if (timeDiff < 0) {
