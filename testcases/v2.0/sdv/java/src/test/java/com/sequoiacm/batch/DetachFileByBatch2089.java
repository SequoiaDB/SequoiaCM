...
/**
 * test content:attach file to the batch,update content by the file,than detach file by batch
...
public class DetachFileByBatch2089 extends TestScmBase {
	...
	private void updateFile(ScmWorkspace ws, ScmId fileId) throws ScmException{
		int updateTimes = 10;
	...
	private void detachFileAndDeteleBatch( ScmWorkspace ws, ScmId batchId, ScmId fileId) throws ScmException{		
		ScmBatch batch = ScmFactory.Batch.getInstance(ws, batchId);
		batch.detachFile(fileId);
		ScmFactory.Batch.deleteInstance(ws, batchId);		
	}
	
	private void checkDetachFileAndDeleteResult(ScmWorkspace ws, ScmId batchId, ScmId fileId) throws ScmException{
	...
	private void deleteFileAndCheckResult(ScmWorkspace ws, ScmId fileId) throws ScmException{
