...
public class UpdateFileAndTag1942 extends TestScmBase {
	...
	private void testSingle() throws Exception {
		...
		int histVersion = 1;
		// check
		checkAttr(fileIdList.get(0), name + "_0", fileSize / 2, currVersion, tags1);
		VersionUtils.CheckFileContentByFile(ws, fileIdList.get(0), currVersion, filePath2, localPath);
		VersionUtils.CheckFileContentByFile(ws, fileIdList.get(0), histVersion, filePath2, localPath);
	...
	private void testMutil() throws Exception {
		...
		int histVersion = 1;
		// check
		checkAttr(fileIdList.get(1), name + "_1", fileSize / 2, currVersion, tags1);
		VersionUtils.CheckFileContentByFile(ws, fileIdList.get(1), currVersion, filePath2, localPath);
		VersionUtils.CheckFileContentByFile(ws, fileIdList.get(1), histVersion, filePath2, localPath);
		SiteWrapper[] expSites = { site };
	...
	private ScmId createFile(String filename, ScmTags tags, String filePath) throws ScmException {
	...
	private void updateFile(ScmId fileId, ScmTags tags, String filePath) throws ScmException {
	...
	private void checkAttr(ScmId fileId, String fileName, int fileSize, int version, ScmTags tags) throws ScmException {
