...
public class ScheduleClean1956 extends TestScmBase {
	...
	private void createScheduleTask(BSONObject queryCond) throws ScmException {
		...
		content = new ScmScheduleCleanFileContent(site1.getSiteName(), maxStayTime, queryCond);
		cron = "* * * * * ?";
		ScmSchedule sche = ScmSystem.Schedule.create(session, wsp.getName(), ScheduleType.CLEAN_FILE, name,
	...
	private ScmId createFile(String name, ScmClassProperties properties, ScmTags tag)
	...
	private void checkResult(List<ScmId> fileIdList, boolean isClean) throws Exception {
		if (!isClean) {
			SiteWrapper[] expSites = { ScmInfo.getRootSite(), site1, site2 };
			ScmScheduleUtils.checkScmFile(ws, fileIdList, expSites);
		} else {
			SiteWrapper[] expSites = { ScmInfo.getRootSite(), site2 };
			ScmScheduleUtils.checkScmFile(ws, fileIdList, expSites);
		}
	}
}
