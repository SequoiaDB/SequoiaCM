...
public class WriteAndReadInMutiWs716 extends TestScmBase {
	...
	private void test() {
		try {
			// write
			WriteScmFile wThread1 = new WriteScmFile(wsList.get(0), fileIdList0);
			wThread1.start();

			WriteScmFile wThread2 = new WriteScmFile(wsList.get(1), fileIdList1);
			wThread2.start();

			WriteScmFile wThread3 = new WriteScmFile(wsList.get(2), fileIdList2);
			wThread3.start();

			if (!(wThread1.isSuccess() && wThread2.isSuccess() && wThread3.isSuccess())) {
				Assert.fail(wThread1.getErrorMsg() + wThread2.getErrorMsg() + wThread3.getErrorMsg());
			}

			// read
			ReadScmFile rThread1 = new ReadScmFile(wsList.get(0), fileIdList0);
			rThread1.start();

			ReadScmFile rThread2 = new ReadScmFile(wsList.get(1), fileIdList1);
			rThread2.start();

			ReadScmFile rThread3 = new ReadScmFile(wsList.get(2), fileIdList2);
			rThread3.start();
