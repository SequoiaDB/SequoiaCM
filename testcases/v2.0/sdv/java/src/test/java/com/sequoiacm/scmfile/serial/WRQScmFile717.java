...
public class WRQScmFile717 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			...
			BSONObject cond1 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author + "_W").get();
	...
	private class WriteScmFile extends TestThreadBase {
		...
		private void checkWrite(ScmFile file, ScmId fileId, String downloadPath) {
			try {
	...
	private class QueryScmFile extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, scopeType, condition);
				checkQuery(cursor);
			} finally {
				if (null != session) {
					session.close();
