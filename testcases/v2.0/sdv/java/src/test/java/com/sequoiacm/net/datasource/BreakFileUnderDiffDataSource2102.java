...
public class BreakFileUnderDiffDataSource2102 extends TestScmBase {
	...
	private void setUp() throws IOException, ScmException {
		...
		for (int i = 0; i < siteList.size(); i++) {
			if (siteList.get(i).getDataType().equals(DatasourceType.HBASE)) {
				hbaseSite = siteList.get(i);
			}
			
			if(siteList.get(i).getDataType().equals(DatasourceType.HDFS)){
	...
	private void testCreateFileInHbase() throws Exception {
		ScmBreakpointFile breakpointFile = createBreakpointFile(ws2, name, filePath);
	...
	private ScmBreakpointFile createBreakpointFile(ScmWorkspace ws,String name, String filePath) throws ScmException {
		// create file
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.createInstance(ws, name+"_" + UUID.randomUUID());
		breakpointFile.upload(new File(filePath));
		return breakpointFile;
	}

	private ScmId breakpointFile2ScmFile(ScmWorkspace ws,ScmBreakpointFile breakpointFile, String name)
			throws ScmException {
	...
	private void checkResult(ScmId fileId,ScmWorkspace ws,int currentVerion) throws Exception{
		...
		//check content
		VersionUtils.CheckFileContentByFile(ws, fileId,currentVerion ,filePath, localPath);
	}
}