...
/**
 * test content:create a file under a dir,than update Content of the file, delete file
...
public class DeleteFileWithDir2064 extends TestScmBase {
	...
	private String fullPath = dirBasePath + "/2064_a/2064_b/2064_c/2064_e/2064_f/";
	...
	private void writeandUpdateFileWithDir(ScmWorkspace ws) throws ScmException{
	...
	private void checkWriteAndUpdateResult( ScmWorkspace ws ) throws Exception{
		...
		while (cursor.hasNext()) {
			...
			if ( majorVersion == 1){
				VersionUtils.CheckFileContentByStream(ws, fullPath + fileName, majorVersion, writeData);	
			}else if ( majorVersion == 2){
				VersionUtils.CheckFileContentByStream(ws, fullPath + fileName, majorVersion, updateData);	
			}else {
		...
		BSONObject condition1 = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).in(fileIdList).get();
		...
		while (cursor1.hasNext()) {
			...
			VersionUtils.CheckFileContentByStream(ws, fullPath + fileName1, 1, writeData);		
			size1++;
		}
		cursor1.close();	
	...
	private void deleteFileAndCheckResult(ScmWorkspace ws) throws ScmException{
		for (String fileId : fileIdList) {
			ScmFactory.File.deleteInstance(ws, new ScmId(fileId), true);
		}
		
		BSONObject fileCondition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).in(fileIdList).get();
		
		long currentCount = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, fileCondition);
		Assert.assertEquals(currentCount, 0);
		long historyCount = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_HISTORY, fileCondition);
