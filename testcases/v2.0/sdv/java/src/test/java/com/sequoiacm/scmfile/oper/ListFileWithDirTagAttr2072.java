...
public class ListFileWithDirTagAttr2072 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		System.out.println("fileIdList = " + fileIdList.subList(20, 25).toString());
		int count = 0;
		ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_CURRENT, queryCond);
	...
	private void prepareFile(String name, String dirPath, ScmTags tags, ScmClassProperties properties)
			throws Exception {
		List<String> pathList = getSubPaths(dirPath);
		for (int i = 0; i < pathList.size(); i++) {
			for (int j = 0; j < fileNum; j++) {
				ScmDirectory dir = ScmFactory.Directory.getInstance(ws, pathList.get(i));
	...
	private ScmId createFile(String name, ScmDirectory dir,ScmTags tags,ScmClassProperties properties) throws ScmException {
