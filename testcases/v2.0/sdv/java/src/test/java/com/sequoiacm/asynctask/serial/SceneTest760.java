...
public class SceneTest760 extends TestScmBase {
	...
	private class SceneThread3 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				...
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList3, expSiteArr, localPath, filePath);
			} finally {
				if (null != ssB) {
					ssB.close();
				}
	...
	private class SceneThread4 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				...
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList4, expSiteArr, localPath, filePath);
			} finally {
				if (null != ssM) {
					ssM.close();
				}
	...
	private class SceneThread5 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				...
				SiteWrapper[] expSiteArr = { rootSite,branceSiteList.get(0),branceSiteList.get(1) };
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList5, expSiteArr, localPath, filePath);
			} finally {
				if (null != ssB) {
					ssB.close();
				}
	...
	private class SceneThread6 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				...
				SiteWrapper[] expSiteArr = { rootSite,branceSiteList.get(0),branceSiteList.get(1) };
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList6, expSiteArr, localPath, filePath);
	...
	private class SceneThread7 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				ssA = TestScmTools.createSession(branceSiteList.get(0));
				ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(ws_T.getName(), ssA);
				int expSiteNum = 2;
				for (ScmId fileId : fileIdList7) {
				...
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList7, expSiteArr, localPath, filePath);
			} finally {
				if (null != ssA) {
					ssA.close();
				}
	...
	private class SceneThread8 extends TestThreadBase {
		...
		public void exec() throws Exception {
			try {
				ssB = TestScmTools.createSession(branceSiteList.get(1));
				ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(ws_T.getName(), ssB);
				int expSiteNum = 2;
				for (ScmId fileId : fileIdList8) {
					ScmFactory.File.asyncTransfer(ws, fileId);
					ScmTaskUtils.waitAsyncTaskFinished(ws, fileId, expSiteNum);
				}
				SiteWrapper[] expSiteArr = { rootSite,branceSiteList.get(1)};
				ScmFileUtils.checkMetaAndData(ws_T,fileIdList8, expSiteArr, localPath, filePath);
			} finally {
				if (null != ssB) {
					ssB.close();
				}
	...
	private void writeFile(ScmSession ss, List<ScmId> fileIdList, List<String> filePathList, int writeFileNum)
			throws ScmException {
		ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(ws_T.getName(), ss);
	...
	private void readFile(ScmSession ss, List<ScmId> fileIdList, List<String> filePathList) throws ScmException {
		try {
			ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(ws_T.getName(), ss);
			...
			for (int i = 0; i < fileIdList.size(); ++i) {
				...
				Assert.assertEquals(TestTools.getMD5(filePathList.get(i)), TestTools.getMD5(downloadPath));
			}
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		}
