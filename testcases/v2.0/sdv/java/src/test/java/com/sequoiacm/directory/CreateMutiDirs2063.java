...
public class CreateMutiDirs2063 extends TestScmBase {
	...
	public class CreateDir extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmId fileId = createFileWithDir(ws, fileName+"_"+scmDir.getId(), writeData, authorName, scmDir);
	...
	private void checkFileDirInfoAndData(ScmWorkspace ws, ScmId fileId, ScmDirectory scmDir) throws Exception {
		ScmFile file = ScmFactory.File.getInstance(ws, fileId);		
		Assert.assertEquals(file.getDirectory().toString(), scmDir.toString());		
		VersionUtils.CheckFileContentByStream(ws, scmDir.getPath()+file.getFileName(), file.getMajorVersion(), writeData);		
	}
