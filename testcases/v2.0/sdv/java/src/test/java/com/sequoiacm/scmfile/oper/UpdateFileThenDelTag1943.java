...
public class UpdateFileThenDelTag1943 extends TestScmBase {
	...
	private void testSingle() throws Exception {
		...
		for (int i = 1; i <= currVersion; i++) {
			checkAttr(fileId, name, (int) (fileSize / Math.pow(2, i - 1)), i);
			VersionUtils.CheckFileContentByFile(ws, fileId, i, filePathList.get(i - 1), localPath);
	...
	private void checkAttr(ScmId fileId, String fileName, int fileSize, int version) throws ScmException {
