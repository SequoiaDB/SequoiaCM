...
public class Audit2355 extends TestScmBase {
    ...
    private void test() throws ScmException, InterruptedException {
        ...
        confMap.put(ConfigCommonDefind.scm_audit_user+username1, "USER_DML");
        confMap.put(ConfigCommonDefind.scm_audit_userType+ScmUserPasswordType.TOKEN.name(), "USER_DQL");
    ...
    private void checkAudit(String username,String password,boolean isLogged1, boolean isLogged2) throws ScmException, InterruptedException {
        ...
        try {
            ...
            BSONObject bson1 = new BasicBSONObject().append(ScmAttributeName.Audit.TYPE, "CREATE_USER")
                    .append(ScmAttributeName.Audit.USERNAME, username);
            BSONObject bson2 = new BasicBSONObject().append(ScmAttributeName.Audit.TYPE, "USER_DQL")
                    .append(ScmAttributeName.Audit.USERNAME, username);
            Assert.assertEquals(ConfUtil.checkAudit(session, bson1, newUsername), isLogged1, "Has the configuration been updated? newUsername = " + newUsername);
            Assert.assertEquals(ConfUtil.checkAudit(session, bson2, newUsername), isLogged2, "Has the configuration been updated? newUsername = " + newUsername);
        } finally {
            if(user != null) {
    ...
    private ScmUser createAndQueryUser(String username,String password) throws ScmException {
        ScmSession session = null;
        ScmUser user = null;
        try {
            session = TestScmTools.createSession(site,username,password);
            user = ScmFactory.User.createUser(session,newUsername,ScmUserPasswordType.LOCAL,"123456");
