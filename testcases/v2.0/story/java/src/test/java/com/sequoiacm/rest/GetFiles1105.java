...
public class GetFiles1105 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		String fileMetaList = rest.setRequestMethod(HttpMethod.GET)
				.setApi("files?workspace_name=" +ws.getName() +"&scope=" + ScopeType.SCOPE_CURRENT.getScope()+ "&filter={uri}")
				.setUriVariables(new Object[]{ "{\"author\":\"" + author + "\"}"})
				.setResponseType(String.class).exec().getBody().toString();
		JSONArray fileArr = new JSONArray(fileMetaList);
		Assert.assertEquals(fileArr.length() > fileNum || 
				fileArr.length()
				== fileNum, true,
				"fileListInfo.getJSONArray('files').length = " + fileArr.length() + ",fileNum = " + fileNum);
		
		
		int index = fileNum - 1;
        String fileMeta = rest.setApi("files/id/"+ fileIdList.get(index) + "?workspace_name=" +ws.getName()+"&scope=" + ScopeType.SCOPE_CURRENT.getScope())
                   .setRequestMethod(HttpMethod.HEAD)
                   .setResponseType(String.class)
                   .exec().getHeaders().get("file").toString();
        fileMeta =  URLDecoder.decode(fileMeta, "UTF-8");
        JSONObject fileInfo = new JSONObject(fileMeta.substring(1, fileMeta.length() - 1));
		JSONObject desc1 = (JSONObject) descs.get(index);
		Assert.assertEquals(fileInfo.getString("name"), desc1.getString("name"));
		Assert.assertEquals(fileInfo.getString("author"), desc1.getString("author"));
		Assert.assertEquals(fileInfo.getString("title"), desc1.getString("title"));
