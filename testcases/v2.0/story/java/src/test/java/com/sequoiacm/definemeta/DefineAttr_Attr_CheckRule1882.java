...
public class DefineAttr_Attr_CheckRule1882 extends TestScmBase {
	...
	private void testDefault(){
		try {
		    ...
		    ScmAttribute attr1 = ScmFactory.Attribute.getInstance(ws, attr.getId());
		    Assert.assertEquals(attr1.getCheckRule().toStringFormat(), "{ \"max_length\" : -1 }");
	...
	private void testNormal1(){
		try {
		    ...
		    ScmAttribute attr1 = ScmFactory.Attribute.getInstance(ws, attr.getId());
		    Assert.assertEquals(attr1.getCheckRule().toStringFormat(), "{ \"max_length\" : 18 }");
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private void testNormal2(){
		try {
		    ...
		    ScmAttribute attr1 = ScmFactory.Attribute.getInstance(ws, attr.getId());
		    Assert.assertEquals(attr1.getCheckRule().toStringFormat(), "{ \"max_length\" : 100 }");
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private void testNormal3(){
		try {
			ScmStringRule rule = new ScmStringRule(new BasicBSONObject().append("max_length", 20));
			ScmAttribute attr = craeteAttr(attrname+"_4" ,rule);
		    ScmAttribute attr1 = ScmFactory.Attribute.getInstance(ws, attr.getId());
		    Assert.assertEquals(attr1.getCheckRule().toStringFormat(), "{ \"max_length\" : 20 }");
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private ScmAttribute craeteAttr(String name,ScmAttrRule rule) throws ScmException {
