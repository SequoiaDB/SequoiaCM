...
public class ScmWorkspaceUtil extends TestScmBase {
	...
	private static final Logger logger = Logger.getLogger(ScmWorkspaceUtil.class);
	
	public static ScmWorkspace createWS(ScmSession session, String wsName, int siteNum) throws ScmException, InterruptedException{
		...
		scmMetaLocation = new ScmSdbMetaLocation(rootSite.getSiteName(), ScmShardingType.YEAR, TestSdbTools.getDomainNames(rootSite.getMetaDsUrl()).get(0));
	...
	public static void wsSetPriority(ScmSession session, String wsName) throws ScmException, InterruptedException{
		
		wsSetPriority(session, wsName, ScmPrivilegeType.ALL);
	}
	
	public static void wsSetPriority(ScmSession session, String wsName, ScmPrivilegeType privilege) throws ScmException, InterruptedException{
		...
		ScmFactory.Role.grantPrivilege(session, superuser.getRoles().iterator().next(), rs, privilege);
		
		for (int i = 0; i < 6; i++) {
			Thread.sleep(10000);
			try {
				ScmFactory.File.listInstance(ScmFactory.Workspace.getWorkspace(wsName, session), ScopeType.SCOPE_ALL, new BasicBSONObject());
				return;
			} catch (ScmException e) {
	...
	public static ScmWorkspace createWs(ScmSession session, String wsName, String metaStr, String dataStr)
			throws Exception {
		Ssh ssh = null;
		try {
			...
			String cmd = installPath + "/bin/scmadmin.sh createws -n " + wsName + " -m \"" + metaStr + "\" -d \""
                    + dataStr + "\" --url \""+ TestScmBase.gateWayList.get(0)+"/"+ScmInfo.getRootSite().getSiteName().toLowerCase()+"\" --user " +TestScmBase.scmUserName +" --password " + TestScmBase.scmUserName;
			ssh.exec(cmd);
	...
	public static void deleteWs(String wsName, ScmSession session) throws Exception {
		try {
	...
	public static void wsRemoveSite(ScmWorkspace ws, String siteName) throws ScmException, InterruptedException{
	 ...
	 */
	public static void wsAddSite(ScmWorkspace ws, SiteWrapper site) throws ScmException, InterruptedException {
		
		ScmDataLocation dataLocation = null;
		String siteName = site.getSiteName();
		switch( site.getDataType().toString() ){
		case "sequoiadb": 
			String domainName = TestSdbTools.getDomainNames(site.getDataDsUrl()).get(0);
		...
		case "ceph_swift":
			...
			break;
		default:
			Assert.fail("dataSourceType not match: "+ site.getDataType().toString());
	} 
	...
	public static void wsAddSite(ScmWorkspace ws, ScmDataLocation dataLocation) throws ScmException, InterruptedException {
