...
public class Param_readByOff275 extends TestScmBase {
	...
	private void testOffAddLenGteByteSize() throws ScmException {
		...
		try {
			...
			if (e.getError() != ScmError.INVALID_ARGUMENT || !e.getMessage().contains("indexOutOfBound,arraySize:10,off:5,len:6")) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			}
		} finally {
	...
	private void testBIsNull() throws ScmException {
		...
		try {
			...
			if (e.getError() != ScmError.INVALID_ARGUMENT || !e.getMessage().contains("byteArray is null")) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			}
		} finally {
	...
	private void testOffIsNegative() throws ScmException {
		...
		try {
			...
			if (e.getError() != ScmError.INVALID_ARGUMENT || !e.getMessage().contains("indexOutOfBound,arraySize:10,off:-1,len:1")) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			}
		} finally {
	...
	private void testLenIs0() throws ScmException {
		...
		try {
			...
			if (e.getError() != ScmError.INVALID_ARGUMENT || !e.getMessage().contains("len must be greater than zero:0")) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			}
		} finally {
