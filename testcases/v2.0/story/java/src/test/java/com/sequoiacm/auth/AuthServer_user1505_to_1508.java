...
public class AuthServer_user1505_to_1508 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1505_to_1508.class);
	...
	private void test_disabledUser01() throws ScmException {		
		...
		for (int i = 0; i < ssNum; i++) {
			ScmSession tmpSS = TestScmTools.createSession(site, NAME, PASSWORD);
			ss.add(tmpSS);
		}
		...
		try {
			...
			logger.info("login but the user disabled, errorMsg = [" + e.getError() + "]");
		}
