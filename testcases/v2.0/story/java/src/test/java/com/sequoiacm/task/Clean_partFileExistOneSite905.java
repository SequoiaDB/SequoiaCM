...
public class Clean_partFileExistOneSite905 extends TestScmBase {
	...
	private void test() {
		try {
			...
			int totolFileSize = fileIdList1.size() + fileIdList2.size() + fileIdList3.size() + fileIdList4.size();
			...
			Assert.assertEquals(taskInfo.getActualCount(), fileIdList4.size());
			Assert.assertEquals(taskInfo.getFailCount(), 0);
			Assert.assertEquals(taskInfo.getSuccessCount(), fileIdList4.size());
	...
	private void startCleanTask(ScmSession ss, String author) throws Exception {
	...
	private void checkResults() throws Exception {
		...
		SiteWrapper[] siteArr = { rootSite, branceSiteList.get(0), branceSiteList.get(1)};
		for (int i = 0; i < siteArr.length; i++) {
			SiteWrapper[] expSiteIdList1 = { siteArr[i] };
			ScmFileUtils.checkMetaAndData(ws_T,fileIdLists.get(i), expSiteIdList1, localPath, filePath);
		}

		// clean files
		SiteWrapper[] expSiteList2 = { rootSite };
		ScmFileUtils.checkMetaAndData(ws_T,fileIdList4, expSiteList2, localPath, filePath);
	}
}