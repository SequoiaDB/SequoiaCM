...
public class Not370 extends TestScmBase {
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			ScmQueryBuilder cond2 = ScmQueryBuilder.start().not(obj).not(obj);
			list.add(cond1);
			list.add(cond2);
			for (ScmQueryBuilder cond : list) {
				cond = cond.and(ScmAttributeName.File.AUTHOR).is(authorName);
				String subStr = "{ \"" + key + "\" : \"" + value + "\"}";
				String expCond = "{ \"$not\" : [ " + subStr + " , " + subStr + "] , \"author\" : \"" + authorName
						+ "\"}";

				Assert.assertEquals(cond.get().toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));

				// count
				long count = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, cond.get());
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start().not(obj1, obj2).and(ScmAttributeName.File.AUTHOR).is(authorName)
					.get();

			String expCond = "{ \"$not\" : [ " + obj1.toString() + " , " + obj2.toString() + "] , " + "\"author\" : \""
					+ authorName + "\"}";
