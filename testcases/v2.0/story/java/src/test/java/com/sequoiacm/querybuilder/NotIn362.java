...
/**
 * @Testcase: SCM-362:notIn匹配不存在的字段
 * @author huangxiaoni init
 * @date 2017.5.26
 */

public class NotIn362 extends TestScmBase {
	...
	private String fileName = "NotIn362";
	...
	private void testQuery() throws Exception {
		try {
			...
			list.add("a");
			BSONObject cond = ScmQueryBuilder.start("notExist").notIn(list).and(ScmAttributeName.File.FILE_NAME)
					.is(fileName).get();
			Assert.assertEquals(cond.toString().replaceAll("\\s*", ""),
					("{ \"notExist\" : { \"$nin\" : [ \"a\"]} , \"name\" : \"" + fileName + "\"}").replaceAll("\\s*",
							""));
