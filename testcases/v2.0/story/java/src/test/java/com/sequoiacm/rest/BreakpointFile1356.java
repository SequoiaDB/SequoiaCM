...
public class BreakpointFile1356 extends TestScmBase {
	...
	private void uploadBreakpointFile() throws Exception{
		...
		rest.setRequestMethod(HttpMethod.PUT )
				.setApi("/breakpointfiles/"+fileName+"?workspace_name="+ws.getName()+"&&offset=0&&is_last_content=true")
	...
	private void errorRequest() throws Exception{
		...
		try {
			...
			rest.setRequestMethod(HttpMethod.POST )
					...
					.setParameter("is_last_content", false)
					.exec();
			Assert.fail("use error option create breakpointFile should error");
	...
	private void uploadBreakpointFileFail() throws Exception{
		...
		try{
			rest.connect(site.getSiteServiceName(), TestScmBase.scmUserName,TestScmBase.scmPassword);
			rest.setRequestMethod(HttpMethod.PUT )
					.setApi("/breakpointfiles/"+fileName+"?workspace_name="+ws.getName()+"&&offset=524288&&is_last_content=true")
	...
	private void checkScmFile() throws ScmException, IOException{
		ScmFile scmFile = ScmFactory.File.getInstance(ws, new ScmId(fileId));
