...
public class AuthServer_user_token1808 extends TestScmBase {
    private static final Logger logger = Logger.getLogger(AuthServer_user_token1808.class);
	...
	try {
	    ...
	    // clean scmFile
	    // BSONObject cond =
	    // ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(NAME).get();
	...
	// create user
	ScmUser user = ScmFactory.User.createUser(session, NAME, ScmUserPasswordType.TOKEN, PASSWORD);
	...
	try {
	    ...
	    // logout
	    ss.close();
	    ScmCursor<ScmSessionInfo> cursor = ScmFactory.Session.listSessions(session, NAME);
