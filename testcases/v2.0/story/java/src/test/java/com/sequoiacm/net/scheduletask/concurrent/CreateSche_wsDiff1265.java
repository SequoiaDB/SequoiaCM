...
public class CreateSche_wsDiff1265 extends TestScmBase {
	...
	private List<ScmId> scheIds = Collections.synchronizedList(new ArrayList<ScmId>());
	...
	private void setUp() {
		try {
			...
			wsB = ScmFactory.Workspace.getWorkspace(wss.get(1).getName(), ssA1);
	...
	private void test() {
		try {
			CreateSchedule crtSche1 = new CreateSchedule(wss.get(0),rootSite,branSite);
			crtSche1.start();

			CreateSchedule crtSche2 = new CreateSchedule(wss.get(1),rootSite1,branSite1);
			crtSche2.start();
			...
			ScmScheduleUtils.checkScmFile(wsB, fileIds, fileNum / 2, fileNum, expSites1);
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private class CreateSchedule extends TestThreadBase {
		...
		public CreateSchedule(WsWrapper wsp,SiteWrapper rootSite,SiteWrapper branSite) {
			this.wsp = wsp;
