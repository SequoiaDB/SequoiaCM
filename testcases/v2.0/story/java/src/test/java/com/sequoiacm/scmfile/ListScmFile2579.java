...
public class ListScmFile2579 extends TestScmBase {
    ...
    private void setUp() throws Exception {
        ...
        getInitScmFileInfo(ScmType.ScopeType.SCOPE_CURRENT, filter, currFileList);
        getInitScmFileInfo(ScmType.ScopeType.SCOPE_HISTORY, filter, histFileList);
    }
    ...
    public Object[][] generateRangData() throws Exception {
        return new Object[][]{
                //orderby:null
                {ScmType.ScopeType.SCOPE_CURRENT, filter, null, 0, 10, currFileList},
                {ScmType.ScopeType.SCOPE_HISTORY, filter, null, 2, 5, histFileList}
        };
    }

    @Test(dataProvider = "dataProvider")
    private void test(ScmType.ScopeType scopeType, BSONObject filter, BSONObject orderby,
                      long skip, long limit, List<ScmFileBasicInfo> fileList) throws Exception {
        int actPageSize = 0;
        long tmpSkip = skip;
        int totalNum = 0;
        ...
        try {
            ...
            while (tmpSkip < fileList.size()) {
                ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, scopeType,
    ...
    private void getInitScmFileInfo(ScmType.ScopeType scopeType, BSONObject filter, List<ScmFileBasicInfo> fileList)
            throws ScmException {
