 ...
 * @version:1.0
 */
public class ListBatch2586 extends TestScmBase {
	...
	private String batchNamePrefix = "batch2586";
	private String tag = "tag2586";
	...
	public Object[][] generateRangData() throws Exception {
		BSONObject orderby = ScmQueryBuilder.start(ScmAttributeName.Batch.CREATE_TIME).is(1)
				.and(ScmAttributeName.Batch.NAME).is(-1).get();
		BSONObject orderby1 = ScmQueryBuilder.start(ScmAttributeName.Batch.NAME).is(-1)
				.and(ScmAttributeName.Batch.CREATE_TIME).is(1).get();
		return new Object[][]{
				//filter  skip   limit initScmFiles  sortnameArr  typeArr
				//orderby:多个字段 正序
				{filter,orderby, 0, 10, batchList,new String[]{"createTime","name"},new boolean[]{true,false}},
				{filter,orderby1, 2, 20, batchList,new String[]{"name","createTime"},new boolean[]{false,true}}
		};
	}
			...
			while (tmpSkip < tmpList.size()) {
				...
				while (cursor.hasNext()) {
					...
					try {
						...
						throw new Exception( "filter = " + filter.toString() + ",orderby = " + orderby
