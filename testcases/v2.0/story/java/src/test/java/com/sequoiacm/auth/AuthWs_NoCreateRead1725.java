...
public class AuthWs_NoCreateRead1725 extends TestScmBase {
    ...
    private void testCreateBreakPointFileByName() throws ScmException, IOException {
        ...
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsCR, fileName);
    ...
    private void testCreateBreakPointFileByChecksumType() throws ScmException, IOException {
        ...
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsCR, fileName, ScmChecksumType.CRC32);
            InputStream inputStream = new BreakpointInputStream(new FileInputStream(new File(filePath)));
            expBreakpointFile.incrementalUpload(inputStream, false);
            inputStream.close();
    ...
    private void testCreateBreakPointFileBySize() throws ScmException, IOException {
        ...
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsCR, fileName, ScmChecksumType.ADLER32, 0);
    ...
    private void testCreateSche() throws ScmException {
        ...
        try {
            ...
            ScmScheduleContent content = new ScmScheduleCopyFileContent(rootSite.getSiteName(),
                    branchSite.getSiteName(), maxStayTime, queryCond);
            String crond = "* * * * * 2029";
            ScmSystem.Schedule.create(sessionCR, wsp.getName(), ScheduleType.COPY_FILE, scheName, null, content, crond);
            Assert.fail("the user have not privilege to do something");
        } catch (ScmException e) {
    ...
    private void tearDown() {
        try {
            ScmFactory.Role.revokePrivilege(sessionA, role, wsrs, ScmPrivilegeType.UPDATE);
            ScmFactory.Role.revokePrivilege(sessionA, role, wsrs, ScmPrivilegeType.READ);
            ScmFactory.Role.revokePrivilege(sessionA, role, dirrs, ScmPrivilegeType.CREATE);
    ...
    private void prepare() throws Exception {
        try {
            ScmUser user = ScmFactory.User.createUser(sessionA, username, ScmUserPasswordType.LOCAL, passwd);
            ...
            grantPriAndAttachRole(sessionA, wsrs, user, role, ScmPrivilegeType.UPDATE);
