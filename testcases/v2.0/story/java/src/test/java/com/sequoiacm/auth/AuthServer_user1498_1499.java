...
public class AuthServer_user1498_1499 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1498_1499.class);	
	...
	private void test_delUserByOrdinaryUser() throws ScmException {
		ScmSession ss = TestScmTools.createSession(site, NAME + "_0", PASSWORD);
		try {
			ScmFactory.User.deleteUser(ss, NAME + "_1");
			Assert.fail("expect failed but actual succ.");
		} catch (ScmException e) {
			logger.info("delete user by ordinary user, errorMsg = [" + e.getError() + "]");
		}		
	...
	private void test_delUsingUser() throws ScmException {
		...
		// check results
		ScmCursor<ScmSessionInfo> cursor = ScmFactory.Session.listSessions(session);
	...
	private void createOrdinaryUser() throws ScmException {		
		...
		for (int i = 0; i < USER_NUM; i++) {
			ScmUser scmUser = ScmFactory.User.createUser(session, NAME + "_" + i, ScmUserPasswordType.LOCAL, PASSWORD);
