...
public class DelAndUpSche1268 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			wsB = ScmFactory.Workspace.getWorkspace(wss.get(1).getName(), ssA);
	...
	private void test() throws Exception {
		...
		ScmScheduleUtils.checkScmFile(wsB, fileIds, fileNum / 2, fileNum, expSites2);
	...
	private void tearDown() throws Exception {
		if (!runSuccess) {
			...
			ScmScheduleUtils.outputScmfileInfo(wsB, fileIds.subList(fileNum / 2, fileNum));
		}
		for (ScmId scheduleId : scheIds) {
			if (null != scheduleId) {
				try {
	...
	private class UpdateSchedule extends TestThreadBase {
		...
		public void exec() throws ScmException {
			...
			try {
				...
				newContent = new ScmScheduleCleanFileContent(branSite.getSiteName(), maxStayTime, queryCond);
