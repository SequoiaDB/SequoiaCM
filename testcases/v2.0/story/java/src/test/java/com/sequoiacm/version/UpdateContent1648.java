...
/**
 * test content:specify that the inputStream update Content of  the current scm file 
...
public class UpdateContent1648 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		VersionUtils.CheckFileContentByStream(  ws1, fileName, currentVersion, updatedata );
		VersionUtils.CheckFileContentByStream(  ws1, fileName, historyVersion, filedata );	
		checkFileAtrributes();
		runSuccess = true;
	}
	...
	private void createScmUser() throws ScmException, InterruptedException{
		ScmUser user = ScmFactory.User.createUser(session1, newUsername, ScmUserPasswordType.LOCAL, newPassword);
		...
		ScmFactory.Role.grantPrivilege(session1, role, rs, ScmPrivilegeDefine.ALL);
	...
	private void updateContent(ScmWorkspace ws, byte[] updateData) throws ScmException, IOException, InterruptedException {
	...
	private void checkFileAtrributes() throws ScmException{
		...
		ScmFile hisVersionFile = ScmFactory.File.getInstance(ws1, fileId, histroyVersion, 0);
		Assert.assertEquals(hisVersionFile.getUser(),TestScmTools.scmUserName);
