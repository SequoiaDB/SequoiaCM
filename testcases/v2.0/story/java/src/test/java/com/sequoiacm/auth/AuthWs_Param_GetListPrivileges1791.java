...
public class AuthWs_Param_GetListPrivileges1791 extends TestScmBase {
	...
	try {
	    ...
	    Assert.assertEquals(pri.getPrivilege(), ScmPrivilegeType.READ + "|" + ScmPrivilegeType.CREATE);
	    Assert.assertEquals(pri.getRoleId(), role.getRoleId());
	    Assert.assertEquals(pri.getResource().toStringFormat(), rs.toStringFormat());
	    Assert.assertEquals(pri.getRole().getRoleName(), role.getRoleName());
	} catch (ScmException e) {
	    e.printStackTrace();
	    Assert.fail(e.getMessage());
	}
	...
	try {
	    cursor = ScmFactory.Privilege.listPrivilegesByResource(sessionA, rs);
	    int i = 0;
	...
	try {
		...
		Assert.assertEquals(pri.getRoleId(), role.getRoleId());
		Assert.assertEquals(pri.getResource().toStringFormat(), rs.toStringFormat());
		Assert.assertEquals(pri.getPrivilege(), ScmPrivilegeDefine.READ + "|" + ScmPrivilegeDefine.CREATE);
		Assert.assertEquals(pri.getRoleType(), "role");
		priId = pri.getId();
		i++;
	    }
