...
public class ReTransferCurVersionFile1664 extends TestScmBase {	
	...
	private void setUp() throws IOException, ScmException {
		...
		fileId = VersionUtils.createFileByStream( wsA, fileName, filedata, authorName );
	...
	private void test() throws Exception {
		...
		//asyncTransfer the  historyVersion file, is both on rootSite and branchSite		
	...
	private void startTransferTaskByCurrentVerFile(ScmWorkspace ws, ScmSession session) throws Exception {
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).is(fileId.get()).get();		
		taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT);
