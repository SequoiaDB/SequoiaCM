...
public class RenameDirOnSameName1143 extends TestScmBase {
	...
	private void test() throws Exception {
		// create test dir
		ScmDirectory dir = ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName2);
	...
	private void tearDown() throws Exception {
		try {
			if (runSuccess || TestScmBase.forceClear) {
				...
				ScmFactory.Directory.deleteInstance(ws, dirBasePath + "/" + eleName1);
				ScmFactory.Directory.deleteInstance(ws, dirBasePath + "/" + newName);
	...
	private void createDirAndFile(ScmWorkspace ws, ScmDirectory dir) throws ScmException {
		try {
			ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName1);
	...
	private void check(ScmId fileId, ScmDirectory dir, ScmWorkspace ws) {
		...
		try {
			...
			Assert.assertEquals(file.getDirectory().getPath(), dirBasePath + "/");
			...
			Assert.assertEquals(dir.getParentDirectory().getPath(), dirBasePath + "/");
