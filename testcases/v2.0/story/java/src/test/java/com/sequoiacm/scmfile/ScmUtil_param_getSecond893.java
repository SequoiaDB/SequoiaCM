...
public class ScmUtil_param_getSecond893 extends TestScmBase {
	...
	private void testNomal() {
		try {
			...
			Assert.assertEquals(ScmUtil.Id.getSecond(fileId), file.getCreateTime().getTime() / 1000);

			Assert.assertEquals(ScmUtil.Id.getSecond(new ScmId("0000000000000300012c1947")), 0);
		} catch (ScmException e) {
			Assert.fail(e.getMessage());
		}
		runSuccess1 = true;
