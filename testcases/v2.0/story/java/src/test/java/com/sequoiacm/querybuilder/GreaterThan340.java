...
public class GreaterThan340 extends TestScmBase {
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).greaterThan(111).put(key).greaterThan(value)
					.put(ScmAttributeName.File.AUTHOR).is(file.getAuthor()).get();

			String expCond = "{ \"" + key + "\" : { \"$gt\" : " + value + "}" + " , \"author\" : \"GreaterThan340\"}";
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).greaterThan(value).put("k2").greaterThan(" ").get();

			String expCond = "{ \"" + key + "\" : { \"$gt\" : " + value + "} , " + "\"k2\" : { \"$gt\" : \" \"}}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));
