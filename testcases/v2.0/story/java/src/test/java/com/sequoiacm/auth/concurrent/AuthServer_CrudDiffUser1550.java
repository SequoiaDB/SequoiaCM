...
public class AuthServer_CrudDiffUser1550 extends TestScmBase {
	...
	private class CreateUser extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				ScmUser user = ScmFactory.User.createUser(session, username1, ScmUserPasswordType.LOCAL, passwd);
				userList.add(user);
		...
		private void check(String username, ScmUser user) {
			...
			try {
				...
				Assert.assertEquals(actUser.getPasswordType(), user.getPasswordType());
			} catch (ScmException e) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			}
	...
	private class QueryUser extends TestThreadBase {
		...
		public void exec() {
			try {
				ScmUser actuser = ScmFactory.User.getUser(session, userList.get(1).getUsername());
	...
	private class ModifierUser extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				ScmUser actUser = ScmFactory.User.alterUser(session, userList.get(2), modifier);
		...
		private void check(ScmUser actuser, ScmUser user) {
			...
			Assert.assertEquals(actuser.getUserId(), user.getUserId());
			Assert.assertEquals(actuser.getUsername(), user.getUsername());
			Assert.assertEquals(actuser.getPasswordType(), ScmUserPasswordType.LDAP);
		}
	}
}
