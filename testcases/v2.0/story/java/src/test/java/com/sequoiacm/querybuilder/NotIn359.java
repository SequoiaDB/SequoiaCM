...
public class NotIn359 extends TestScmBase {
	...
	private void testNotInByObj() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).notIn(value).and(key2).is(value2).get();

			String bsStr = "{ \"" + key + "\" : { \"$nin\" : [ \"" + value + "\"]} , \"" + key2 + "\" : \"" + value2
					+ "\"}";
	...
	private void testNotInByList() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).notIn(list).and(key2).is(value2).get();

			String bsStr = "{ " + "\"" + key + "\" : { \"$nin\" : [ \"" + value + "\"]} , \"" + key2 + "\" : \""
					+ value2 + "\"}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), bsStr.replaceAll("\\s*",""));
