	...
	private void test() {
		try {
			...
			if (!(readScmFile.isSuccess() && deleteScmFile.isSuccess())) {
				Assert.fail(readScmFile.getErrorMsg() + deleteScmFile.getErrorMsg());
			}
	...
	private class ReadScmFile extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				String dwPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
						Thread.currentThread().getId());
				file.getContent(dwPath);
				// check results
				Assert.assertEquals(TestTools.getMD5(filePath), TestTools.getMD5(dwPath));
			} catch (ScmException e) {
				if (e.getError() !=  ScmError.DATA_ERROR
						&& e.getError() != ScmError.FILE_NOT_FOUND 
						&& e.getError() != ScmError.DATA_NOT_EXIST && e.getError() != ScmError.DATA_UNAVAILABLE
						&& e.getError() != ScmError.DATA_CORRUPTED) {
					System.out.println("read file, fileId = " + fileId.get() + ", errno = " + e.getErrorCode());
					e.printStackTrace();
					Assert.fail(e.getMessage());
				} 
			} finally {
		...
		public void exec() throws Exception {
			...
			try {
				...
				if ( ScmError.FILE_NOT_FOUND  != e.getError()) {
					System.out.println("delete file, fileId = " + fileId.get() + ", errno = " + e.getErrorCode());
					e.printStackTrace();
					Assert.fail(e.getMessage());
				}
			} finally {
