...
public class MthCombin332 extends TestScmBase {
    ...
    private ScmQueryBuilder ScmBuilder() throws ScmException {
        ...
        BSONObject greaterThan = ScmQueryBuilder.start( keyGt ).greaterThan( 0 )
                .get();

        // lessThan
        String keyLt = ScmAttributeName.File.MINOR_VERSION;
        BSONObject lessThan = ScmQueryBuilder.start( keyLt ).lessThan( 1 )
                .get();

        // not
        String keyNot = ScmAttributeName.File.MAJOR_VERSION;
        BSONObject notCond = ScmQueryBuilder.start( keyNot ).lessThan( 0 )
                .get();
        ...
        int valueE2 = 12345;
        BSONObject objE = ScmQueryBuilder.start( keyE ).is( valueE )
                .and( keyE2 ).greaterThan( valueE2 ).get();

        ScmQueryBuilder builder = ScmQueryBuilder.start( keyIn ).in( inList )
                .put( keyNin ).notIn( ninList ).or( in, nin )
                .put( ScmAttributeName.File.TITLE ).exists( 1 )
                .put( ScmAttributeName.File.CREATE_TIME ).greaterThan( 100000 )
