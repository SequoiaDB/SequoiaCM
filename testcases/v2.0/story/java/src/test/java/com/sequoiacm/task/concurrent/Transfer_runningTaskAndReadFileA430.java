...
/**
 * @Testcase: SCM-430:“执行迁移任务”过程中读取文件内容 （并发在分中心A迁移文件、读取文件内容）
 * @author huangxiaoni init
 * @date 2017.6.14
 */

public class Transfer_runningTaskAndReadFileA430 extends TestScmBase {
	private boolean runSuccess = false;

	private List<ScmId> fileIdList = new ArrayList<ScmId>();
	private String authorName = "transfer430A";
	...
	private void test() {
		try {
			...
			Assert.assertEquals(taskInfo.getActualCount(), fileNum);
			Assert.assertEquals(taskInfo.getFailCount(), 0);
			Assert.assertEquals(taskInfo.getSuccessCount(), fileNum);
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
