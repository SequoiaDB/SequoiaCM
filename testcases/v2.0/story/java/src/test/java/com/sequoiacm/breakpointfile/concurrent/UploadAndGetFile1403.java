...
public class UploadAndGetFile1403 extends TestScmBase {	
	...
	private void test() throws Exception {		
		...
		Assert.assertTrue( uploadBreakpointFile.isSuccess(), uploadBreakpointFile.getErrorMsg());
		Assert.assertTrue( getBreakpointFile.isSuccess(), getBreakpointFile.getErrorMsg());	
		checkFileData();		
	}
	...
	private void createBreakPointFile() throws ScmException, IOException{
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.createInstance(ws, fileName,ScmChecksumType.CRC32);			
		InputStream inputStream = new BreakpointStream(filePath, uploadedSize);
	...
	private class GetBreakpointFileThread extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				Assert.assertEquals(breakpointFile.getChecksumType(), ScmChecksumType.CRC32);
				long uploadsize = breakpointFile.getUploadSize();
				if (   uploadsize < uploadedSize ){
					Assert.fail("get size must be larger than uploaded size:"+uploadsize);
				}				
