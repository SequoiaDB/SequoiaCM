	...
	private class QueryThread extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				String endVal = ScmFactory.File.getInstance(ws, fileIdList.get(fileNum - 1)).getTitle();

				BSONObject fileCond = ScmQueryBuilder.start().and(key).greaterThanEquals(startVal).and(key)
						.lessThan(endVal).and("notexistkey").exists(0).and(ScmAttributeName.File.AUTHOR).is(author)
						.get();

				long actCount = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, fileCond);
				long expCount = fileNum - 1;
				Assert.assertEquals(actCount, expCount, "fileCond = " + fileCond.toString());
			} finally {
				if (session != null) {
					session.close();
				}
