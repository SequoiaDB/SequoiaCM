...
public class WorkspaceRemoveSite2183 extends TestScmBase{
	...
	public void test() throws ScmException, InterruptedException, IOException{
		...
		ScmWorkspace wsM = ScmWorkspaceUtil.createWS(sessionM, wsName, siteNum);
		ScmWorkspaceUtil.wsSetPriority(sessionM, wsName);
		wsUploadFile();
		ScmWorkspace wsA = ScmFactory.Workspace.getWorkspace(wsName, sessionA);
		...
		try {
			...
			Assert.assertEquals(e.getError(), ScmError.SERVER_NOT_IN_WORKSPACE);
		}finally {
			is.close();
		}
	}
