...
public class AuthServer_user1509_to_1516 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1509_to_1516.class);
	...
	private void test_listUserByFilter() throws ScmException {	
		...
		ScmCursor<ScmUser> cursor = ScmFactory.User.listUsers(session, cond);
	...
	private void test_listUserByExistRole() throws ScmException {
		BSONObject cond = ScmQueryBuilder.start("has_role").is("test111A").get();
	...
	private void test_getNotExistUser() throws ScmException {
		try {
			...
			logger.info("get not exist user, errorMsg = [" + e.getError() + "]");
		}
	...
	private void createUser() throws ScmException {	
		...
		scmUser = ScmFactory.User.createUser(session, NAME + "_2", ScmUserPasswordType.LOCAL, PASSWORD);		
		modifier = new ScmUserModifier();
