...
public class DefineAttr_class1936 extends TestScmBase {
	...
	private void test() throws Exception {
		String attrStr1 = "{name:'attr1936_string', display_name:'dispalyName1936_1', description:'I am a Attribute 1936_1', type:'STRING', required:true}";
		String attrStr2 = "{name:'attr1936_date', display_name:'dispalyName1936_2', description:'I am a Attribute 1936_2', type:'STRING', required:false}";
		String attrStr3 = "{name:'attr1936_double', display_name:'dispalyName1936_3', description:'中文', type:'DOUBLE', required:true}";
		
		attrList.add(createAttr(attrStr1));
		attrList.add(createAttr(attrStr2));
		attrList.add(createAttr(attrStr3));
		
		ScmClass scmClass = ScmFactory.Class.createInstance(ws, className, "i am a class1388");
		...
		props.addProperty(attrList.get(1).getName(), attrList.get(1).getCreateTime().toString());
	...
	private void checkResult(ScmBatch expBatch, List<ScmAttribute> expAttrList) throws ScmException{
		ScmBatch batch = ScmFactory.Batch.getInstance(ws, batchId);
		Assert.assertEquals(batch.getClassProperties().toString(), expBatch.getClassProperties().toString());
		Assert.assertEquals(batch.getTags().toString(), expBatch.getTags().toString());
		Assert.assertEquals(batch.getClassId(), expBatch.getClassId());
		
		ScmClass scmClass = ScmFactory.Class.getInstance(ws, batch.getClassId());
		...
		for (ScmAttribute actScmAttribute : actAttrList) {
			...
			for (ScmAttribute expScmAttribute : expAttrList) {
				if(actScmAttribute.toString().equals(expScmAttribute.toString())){
					result = true;
					break;
				}
			}
			Assert.assertTrue(result, "exp: "+expAttrList.toString()+";   act: "+actAttrList.toString());
		}
	}
