...
public class DeleteScmFile285 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			...
			for (int i = 0; i < fileNum * threadNum; i++) {
				BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(fileName+"_"+i).get();
	...
	private void test() {
		try {
			...
			for (int i = 0; i < threadNum; i++) {
				SiteWrapper site = sites.get(new Random().nextInt(sites.size()));
			...
			for (int j = 0; j < delList.size(); j++) {
				...
				Assert.assertTrue(deleteFile.isSuccess(),
						"siteId = " + site.getSiteId() + ", msg = " + deleteFile.getErrorMsg());
			}
	...
	private class DeleteFile extends TestThreadBase {
		...
		public void exec() {
			...
			try {
				...
				ScmWorkspace sws = ScmFactory.Workspace.getWorkspace(wsp.getName(), ss);

				for (int i = fileNum * startNum; i < fileNum * (startNum + 1); i++) {
					ScmFactory.File.getInstance(sws, fileIdList.get(i)).delete(true);
				}
	...
	private void writeFile() throws ScmException {
		for (int i = 0; i < fileNum * threadNum; i++) {
			ScmId fileId = ScmFileUtils.create(ws, fileName+"_"+i, filePath);
