...
/**
 * test content : the currentVersion file in both the sourceSite and the targetSite,
...
public class ReTransferCurVersionFile1664 extends TestScmBase {
	...
	private void setUp() throws IOException, ScmException {			
		...
		fileId = VersionUtils.createFileByStream(wsS, fileName, filedata, authorName);
	...
	private void test() throws Exception {
		...
		// targetSite
		taskId1 = startTransferTaskByCurrentVerFile(wsS, sessionS, ScopeType.SCOPE_HISTORY);
		Collection<ScmFileLocation> firstSiteInfo = getSiteInfo(historyVersion);

		// Transfer the current file once again
		taskId2 = startTransferTaskByCurrentVerFile(wsS, sessionS, ScopeType.SCOPE_CURRENT);
		...
		Assert.assertEquals(firstSiteInfo.toString(), secondSiteInfo.toString(),
				"fisrt get siteList:" + firstSiteInfo.toString() + " 2nd get siteList:" + secondSiteInfo.toString());
		runSuccess = true;
	}
	...
	private ScmId startTransferTaskByCurrentVerFile(ScmWorkspace ws, ScmSession session, ScopeType type)
			throws Exception {
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).is(fileId.get()).get();
		ScmId taskId = ScmSystem.Task.startTransferTask(ws, condition, type, transTargetSite.getSiteName());
	...
	private Collection<ScmFileLocation> getSiteInfo(int majorVersion) throws ScmException {
		...
		ScmFile file = ScmFactory.File.getInstance(wsS, fileId, majorVersion, 0);
