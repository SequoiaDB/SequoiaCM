...
public class ListInstanceByCondition128 extends TestScmBase {
	...
	private void testListInstanceByAllAttributes() {
		...
		try {
			...
			condition.put(ScmAttributeName.File.FILE_NAME, file.getFileName());
			condition.put(ScmAttributeName.File.AUTHOR, file.getAuthor());
			condition.put(ScmAttributeName.File.TITLE, file.getTitle());
			condition.put(ScmAttributeName.File.MIME_TYPE, file.getMimeType());
			condition.put(ScmAttributeName.File.FILE_ID, file.getFileId().get());
			condition.put(ScmAttributeName.File.MAJOR_VERSION, file.getMajorVersion());
			condition.put(ScmAttributeName.File.MINOR_VERSION, file.getMinorVersion());
			condition.put(ScmAttributeName.File.USER, file.getUser());
			condition.put(ScmAttributeName.File.CREATE_TIME, file.getCreateTime().getTime());
			condition.put(ScmAttributeName.File.UPDATE_USER, file.getUpdateUser());
			condition.put(ScmAttributeName.File.UPDATE_TIME, file.getCreateTime().getTime());
			...
			while (cursor.hasNext()) {
				...
				Assert.assertEquals(fileInfo.getFileId(), fileId, fileInfo.toString());
				Assert.assertEquals(fileInfo.getMinorVersion(), 0);
				Assert.assertEquals(fileInfo.getMajorVersion(), 1);
				
				Assert.assertEquals(fileInfo.getUser(), TestScmBase.scmUserName);
				Assert.assertNotNull(fileInfo.getCreateDate());
				Assert.assertEquals(fileInfo.getCreateDate(),file.getDataCreateTime());
				Assert.assertEquals(fileInfo.getMimeType(),MimeType.CSS.getType());
				size++;
			}
