...
public class MoveDirToBrother1147 extends TestScmBase {
	...
	private void test() {
		try {
			ScmDirectory dir3 = ScmFactory.Directory.getInstance(ws, dirBasePath + "/1147_b/1147_d");
			createFile(ws, dir3);
			// eg:dirBasePath + "/b/d/e" mv d to dirBasePath + "/b/c" dir
			dir3.move(ScmFactory.Directory.getInstance(ws, dirBasePath + "/1147_b/1147_c"));
			// check sub path
			check(fileIdList.get(0), ScmFactory.Directory.getInstance(ws, dirBasePath + "/1147_b/1147_c/1147_d"), ws);
			createFile(ws, dir3);
	...
	private void check(ScmId fileId, ScmDirectory dir, ScmWorkspace ws) {
		...
		try {
			...
			Assert.assertEquals(dir.getParentDirectory().getPath(), dirBasePath + "/1147_b/1147_c/");
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
