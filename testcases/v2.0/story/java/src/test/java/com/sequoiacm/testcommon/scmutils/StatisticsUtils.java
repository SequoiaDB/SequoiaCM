...
public class StatisticsUtils extends TestScmBase {
	...
	public static HashMap<String, Long> statisticsFile( ScmWorkspace ws, ScmSession session ) throws ScmException{		
		long currentTimestamp = getTimestampOfTheDay();		
		ScmSystem.Statistics.refresh(session, StatisticsType.TRAFFIC ,ws.getName());		
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Traffic.WORKSPACE_NAME)
				.is(ws.getName())
				.put(ScmAttributeName.Traffic.RECORD_TIME)				
				.is(currentTimestamp).get();	
		
		ScmCursor<ScmStatisticsTraffic> cursor = ScmSystem.Statistics.listTraffic(session, cond);		
	...
	public static HashMap<String, Long> statisticsFileDelta( ScmWorkspace ws, ScmSession session ) throws ScmException{		
		long currentTimestamp = getTimestampOfTheDay();		
		ScmSystem.Statistics.refresh(session, StatisticsType.FILE_DELTA ,ws.getName());		
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Traffic.WORKSPACE_NAME)
				.is(ws.getName())
				.put(ScmAttributeName.FileDelta.RECORD_TIME)				
				.is(currentTimestamp).get();	
		
		ScmCursor<ScmStatisticsFileDelta> cursor = ScmSystem.Statistics.listFileDelta(session, cond);
		HashMap<String, Long> map = new HashMap<String, Long>();	
		long count_delta = 0;
		long size_delta = 0;
		while (cursor.hasNext()) {
	...
	public static  ScmId createFileByStream( ScmWorkspace ws, String fileName, byte[] data, String authorName, long timestamp ) throws ScmException  {		
