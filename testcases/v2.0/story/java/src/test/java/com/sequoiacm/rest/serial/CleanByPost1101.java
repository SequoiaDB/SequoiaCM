...
public class CleanByPost1101 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			String response1 = rest2.reset().setApi("tasks").setRequestMethod(HttpMethod.POST)
					.setParameter("task_type", "2").setParameter("workspace_name", ws.getName())
					.setParameter("options", options.toString()).setResponseType(String.class).exec().getBody()
					.toString();
	...
	private JSONObject getTaskInfo(String taskId) throws JSONException {
		JSONObject taskInfo = null;
		try {
			String response = rest1.reset().setApi("tasks/" + taskId).setRequestMethod(HttpMethod.GET).exec().getBody()
					.toString();
