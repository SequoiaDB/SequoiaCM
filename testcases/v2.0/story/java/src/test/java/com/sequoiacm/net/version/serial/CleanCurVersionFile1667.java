...
public class CleanCurVersionFile1667 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		checkHisVersionFileInfo(wsL, historyVersion);
		runSuccess = true;
	}
		...
		try {
			if (runSuccess) {
				...
				for (String fileId : fileIdList) {
					ScmFactory.File.deleteInstance(wsL, new ScmId(fileId), true);
				}
				TestTools.LocalFile.removeFile(localPath);
			}
		} catch (Exception e) {
	...
	private void startCleanTaskByCurrentVerFile(ScmWorkspace ws, ScmSession session, ScopeType scopeType)
			throws Exception {
		condition = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).greaterThanEquals(fileSize1)
	...
	private void checkCurrentVerFileSiteInfo(ScmWorkspace ws, int currentVersion) throws Exception {
		...
		while (cursor1.hasNext()) {
			...
			VersionUtils.checkSite(ws, fileId1, currentVersion, expCurSiteList1);
			size1++;
		}
		cursor1.close();
	...
	private void checkHisVersionFileInfo(ScmWorkspace ws, int version) throws Exception {
		// all history version file only on the lastSite
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).in(fileIdList).get();
		...
		while (cursor.hasNext()) {
			...
			size++;
			ScmWorkspace ws1 = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionL);
