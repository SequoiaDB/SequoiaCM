...
public class AuthServer2178 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		Assert.assertEquals(privileges.getInt("version")>1,true,privileges.getInt("version")+"");
		...
		Assert.assertEquals(relations.length()>1,true,relations.toString());
		String privilegeId = null;
		for(int i = 0; i < relations.length(); i++){
			String roleId1 = relations.getJSONObject(i).getString("role_id");
		...
		Assert.assertEquals(privilegeInfo.getString("role_id"), roleId,privilegeInfo.toString());
		Assert.assertEquals(privilegeInfo.getString("privilege"),"ALL",privilegeInfo.toString());
		...
		Assert.assertEquals(resources.length()>=1,true,resources.toString());
		...
		Assert.assertEquals(resourcesInfo.getString("type"), "workspace",resourcesInfo.toString());
		Assert.assertEquals(resourcesInfo.getString("id"),resourcesId,resourcesInfo.toString());
		...
		// delete role
		rest.setServerType("auth-server")
		    .setRequestMethod(HttpMethod.DELETE).setApi("/roles/" + username)
		    .setResponseType(String.class).exec();
		
		// delete users
		rest.setServerType("auth-server").setRequestMethod(HttpMethod.DELETE)
		        .setApi("/users/" + username)
				.setResponseType(String.class).exec();
	}
