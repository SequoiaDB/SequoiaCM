...
public class GetWorkSpaceInfo923 extends TestScmBase {
		...
		try {
			...
			while (dbcursor.hasNext()) {
				...
				for (ScmWorkspaceInfo info : infoList) {
					...
					if (id == info.getId()) {
						Assert.assertEquals(info.getName(), (String) obj.get("name"));
						Assert.assertEquals(info.getMetaLocation(), obj.get("meta_location"));
						Assert.assertEquals(info.getDataLocation(), obj.get("data_location"));
						/*deprecated interface
						Assert.assertEquals(info.getDataOption(), obj.get("data_options"));
						Assert.assertEquals(info.getDataShardingType(), obj.get("data_sharding_type"));
						Assert.assertEquals(info.getMetaShardingType(), (String) obj.get("meta_sharding_type"));
						*/
					}
				}
			}
	...
	private DBCursor listWorkSpaceFromDB() {
		...
		DBCollection wsCL = sdb.getCollectionSpace(TestSdbTools.SCM_CS).getCollection(TestSdbTools.SCM_CL_WORKSPACE);
		dbcursor = wsCL.query();
		return dbcursor;
	}
}
