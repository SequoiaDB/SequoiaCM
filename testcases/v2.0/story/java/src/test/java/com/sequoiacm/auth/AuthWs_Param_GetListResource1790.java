...
public class AuthWs_Param_GetListResource1790 extends TestScmBase {
	...
	try {
	    int i = 0;
	    cursor = ScmFactory.Resource.listResourceByWorkspace(sessionA, wsp.getName());
	...
	try {
	    cursor = ScmFactory.Privilege.listPrivilegesByResource(sessionA, dirrs);
	    int i = 0;
	    while (cursor.hasNext()) {
	    ...
	    ScmResource rs = ScmFactory.Resource.getResourceById(sessionA, rsId);
	    Assert.assertNotNull(rs.getType());
	    Assert.assertEquals(rs.toStringFormat(), wsp.getName() + ":/AuthWs_Param_GetListResource1790");
	} catch (ScmException e) {
	    e.printStackTrace();
	    Assert.fail(e.getMessage());
	} finally {
