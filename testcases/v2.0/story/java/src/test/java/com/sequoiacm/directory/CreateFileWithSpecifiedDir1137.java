...
public class CreateFileWithSpecifiedDir1137 extends TestScmBase {
	...
	private void test() {
		...
		try {
			ScmId fileId1 = createFileBySpecifiedDir(ScmFactory.Directory.getInstance(ws, "/"));
		...
		try {
			ScmDirectory dir = ScmFactory.Directory.createInstance(ws, dirBasePath + "/unexist");
			ScmFactory.Directory.deleteInstance(ws, dirBasePath + "/unexist");
			createFileBySpecifiedDir(dir);
			Assert.fail("create file successfully when specified dir does not exist");
		} catch (ScmException e) {
	...
	private ScmId createFileBySpecifiedDir(ScmDirectory dir) throws ScmException {
		// create file
