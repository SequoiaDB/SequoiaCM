...
public class AuthServer_user1492 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1492.class);
	...
	private void test_createUserAndRole() throws ScmException, InterruptedException {
		// create ScmUser
		...
		Assert.assertEquals(mUser.getRoles().iterator().next(), authAdminRole);
		Assert.assertEquals(mUser.getRoles().iterator().next().getRoleName(), "ROLE_AUTH_ADMIN");
		Assert.assertEquals(mUser.getRoles().iterator().next().getDescription(), "authentication administrator");
