...
public class DefineAttr_class_getClassInfo1722 extends TestScmBase {
    ...
    private String[] types = { "date", "string", "boolean", "double", "int",
            "string" };
    private String[] names = { "test_attr_name_date_1722",
            "test_attr_name_string1722", "test_attr_name_bool_1722",
            "test_attr_name_double_1722", "test_attr_name_int_1722",
            "test_attr_name_string_1722" };
    ...
    private void test_getInstance() throws ScmException {
        try {
            ...
            Assert.assertEquals( scmClass.getCreateUser(),
                    TestScmBase.scmUserName );
            Assert.assertEquals( scmClass.getWorkspace().getName(),
                    wsp.getName() );
            Assert.assertEquals( scmClass.listAttrs().toString(),
                    attrs.toString() );
    ...
    private void test_listInstance() throws ScmException {
        try {
            BSONObject matcher = ScmQueryBuilder
                    .start( ScmAttributeName.Class.NAME ).is( name ).get();
