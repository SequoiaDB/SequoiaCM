...
public class AuthServer_CrudSameUser1549 extends TestScmBase {
	...
	private class QuerySameUser extends TestThreadBase {
		...
		public void exec() {
			try {
				ScmUser actuser = ScmFactory.User.getUser(session, user.getUsername());
				check(actuser, user);
			} catch (ScmException e) {
				if (e.getError() != ScmError.HTTP_NOT_FOUND) {
					e.printStackTrace();
		...
		private void check(ScmUser actuser, ScmUser user) {
			...
			Assert.assertEquals(actuser.getPasswordType().equals( ScmUserPasswordType.LOCAL) || 
					actuser.getPasswordType().equals(ScmUserPasswordType.LDAP), true);
		}
	}
	...
	private class ModifierSameUser extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				ScmUser actUser = ScmFactory.User.alterUser(session, user, modifier);
				check(actUser, user);
			} catch (ScmException e) {
				if (e.getError() != ScmError.HTTP_NOT_FOUND) {
					e.printStackTrace();
