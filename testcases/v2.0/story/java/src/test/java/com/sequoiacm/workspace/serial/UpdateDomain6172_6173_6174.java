...
/**
 * @Descreption SCM-6172:管理员用户使用驱动更新工作区同名元数据域 
 *              SCM-6173:管理员用户使用驱动更新工作区不存在的元数据域
 *              SCM-6174:管理员用户使用驱动更新工作区元数据域为空
 * @Author yangjianbo
 ...
 * @Version 1.0
 */
public class UpdateDomain6172_6173_6174 extends TestScmBase {
    private ScmSession session = null;
    private SiteWrapper rootSite = null;
    private String wsName = "ws6169";
    private ScmWorkspace ws;
    private String filePath = null;
    private int fileSize = 1024 * 1024;
    private String author = "Author6169";
    private String fileName = "file6169";
    ...
    public void test() throws Exception {
        ...
        try {
            ...
            Assert.fail( "update doMain NotExit should be failed" );
        } catch ( ScmException ex ) {
            if ( ex.getErrorCode() != ScmError.CONFIG_SERVER_ERROR
                    .getErrorCode() ) {
                throw ex;
            }
        }
        ...
        try {
            ws.updateMetaDomain( null );
            Assert.fail( "update doMain null should be failed" );
        } catch ( ScmException ex ) {
            if ( ex.getErrorCode() != ScmError.INVALID_ARGUMENT
                    .getErrorCode() ) {
                throw ex;
            }
        }
        ...
        try {
            ws.updateMetaDomain( "" );
            Assert.fail( "update doMain empty should be failed" );
        } catch ( ScmException ex ) {
            if ( ex.getErrorCode() != ScmError.CONFIG_SERVER_ERROR
                    .getErrorCode() ) {
                throw ex;
