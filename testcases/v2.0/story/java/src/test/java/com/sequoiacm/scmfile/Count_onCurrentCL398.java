...
public class Count_onCurrentCL398 extends TestScmBase {
	...
	private void test() throws Exception {
		try {
			...
			String lowBound = ScmFactory.File.getInstance(ws, fileIdList.get(0)).getFileName();
			String upBound = ScmFactory.File.getInstance(ws, fileIdList.get(3)).getFileName();
			BSONObject cond = ScmQueryBuilder.start(key).greaterThan(lowBound).and(key).lessThanEquals(upBound).get();
