...
/**
 * @description SCM-4136:清理文件时，使用ScmFile.getContentLocaltions()接口获取文件数据源信息
 * @author ZhangYanan
 ...
 * @version v1.0
 */
public class FileContentLocaltions4136 extends TestScmBase {
    ...
    private String fileName = "file_4136";
    private ScmWorkspace ws;
    private ScmId fileId = null;
    private ScmId taskID = null;
    private BSONObject queryCond;
    private List< ScmId > taskIdList = new ArrayList<>();
    private List< ScmContentLocation > fileContentLocationsInfo;
    ...
    public void test() throws Exception {
        ...
        es.run();
        if ( fileContentLocationsInfo.size() == 1 ) {
            ScmFileUtils.checkContentLocation( fileContentLocationsInfo, site,
                    fileId, ws );
        } else if ( fileContentLocationsInfo.size() == 2 ) {
            ScmFileUtils.checkContentLocation( fileContentLocationsInfo,
                    rootSite, fileId, ws );
            ScmFileUtils.checkContentLocation( fileContentLocationsInfo, site,
                    fileId, ws );
        } else {
            Assert.fail( "the fileContentLocationsInfo size must 1 or 2" );
        }
        runSuccess = true;
    }
