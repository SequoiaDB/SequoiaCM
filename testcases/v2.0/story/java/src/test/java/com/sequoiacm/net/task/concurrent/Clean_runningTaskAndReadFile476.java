...
public class Clean_runningTaskAndReadFile476 extends TestScmBase {
	...
	private class StartTaskFromSubCenterA extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				// start task
				BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR)
				...
				while (true) {
					...
					if (taskInfo.getStopTime() != null) {
						if (taskInfo.getRunningFlag() == CommonDefine.TaskRunningFlag.SCM_TASK_FINISH) {
							...
							// task detail
							try (Sequoiadb db = TestSdbTools.getSdb(TestScmBase.mainSdbUrl)) {
								DBCollection clDB = db.getCollectionSpace(TestSdbTools.SCM_CS)
										.getCollection(TestSdbTools.SCM_CL_TASK);
								BSONObject matcher = new BasicBSONObject();
								matcher.put("id", taskId.get());
								String detail = (String) clDB.query(matcher, null, null, null).getNext().get("detail");
								if (!detail.toString().contains("open lob failed")) {
	...
	private void waitTaskFlagIsRunning() throws ScmException, InterruptedException {
		ScmSession session = null;
		try {
			// login
			session = TestScmTools.createSession(sourceSite);
			while (true) {
				...
				if (taskId != null) {
					ScmTask taskInfo = ScmSystem.Task.getTask(session, taskId);
					if (taskInfo.getRunningFlag() != CommonDefine.TaskRunningFlag.SCM_TASK_INIT) {
						break;
					}
				}
			}
	...
	private List<Integer> getSiteNumList() {
		...
		try {
			...
			ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(ws_T.getName(), session);
			for (int i = 0; i < fileNum; i++) {
				ScmFile file = ScmFactory.File.getInstance(ws, fileIdList.get(i));
