...
/**
 * test content:Clean the all version file, matcheing partial current version file clean
...
public class CleanAllVersionFile1669 extends TestScmBase {	
	...
	private void test() throws Exception {			
		...
		checkNoClearedFileInfo(wsM, currentVersion);	
		runSuccess = true;
	}
	...
	private void startCleanTaskByAllVerFile(ScmWorkspace ws, ScmSession session, ScopeType scopeType) throws Exception {
		condition = ScmQueryBuilder.start(ScmAttributeName.File.SIZE).lessThanEquals(fileSize1)
	...
	private void checkClearedFileSiteInfo(ScmWorkspace ws, int currentVersion,int historyVersion) throws Exception{
		//check the cleared current and history version file sitelist ,only on the rootSite		
	...
	private void checkNoClearedFileInfo(ScmWorkspace ws,int version) throws ScmException{
		//no cleared current version file 
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.SIZE).greaterThan(fileSize1)
				.put(ScmAttributeName.File.FILE_ID).in(fileIdList).get();		
		ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_ALL, condition);
