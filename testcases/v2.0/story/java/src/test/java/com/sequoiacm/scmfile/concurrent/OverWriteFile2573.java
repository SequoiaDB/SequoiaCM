...
public class OverWriteFile2573 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        for (int i = 0; i < newFileIdList.size(); i++) {
            ScmFile actFile = ScmFactory.File.getInstance(ws, newFileIdList.get(i));
            checkFile(actFile, fileNamePre, updateFileSize, updateFilePath, scmTags);
        }
    ...
    private class OverWriteFile {
        ...
        private void overwriteScmFile() throws Exception {
            try {
                try {
                    newFileIdList.add(scmFile.save(new ScmUploadConf(true)));
                } catch (ScmException e) {
                    if (e.getError() != ScmError.FILE_EXIST) {
                        throw new Exception("scmfile = " + scmFile.toString(), e);
                    }
                }
            } finally {
                if (session != null) {
    ...
    private class DeleteFile {
        ...
        private void deleteFile() throws ScmException {
            try {
                ...
                ScmFactory.File.deleteInstance(ws, origFileIdList.get(index), true);
            } catch (ScmException e) {
                if (e.getError() != ScmError.FILE_NOT_IN_BATCH && e.getError() != ScmError.FILE_NOT_FOUND) {
                    throw e;
                }
            } finally {
                if (session != null) {
