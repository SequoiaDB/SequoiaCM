...
public class UpdateWorkspace2180 extends TestScmBase {
	 ...
	 */
	private BSONObject siteBson(SiteWrapper site) throws ScmInvalidArgumentException, JSONException {
		...
		case "sequoiadb": 
			...
			return new ScmSdbDataLocation(siteName, domainName).getBSONObject();
		case "hbase":
	...
	private void wsRemoveSite() throws Exception {
		...
		for (int i = 0; i < 15; i++) {
			Thread.sleep(1000);
			ScmWorkspace updateWs = ScmFactory.Workspace.getWorkspace(wsName, session);
			...
			for (int j = 0; j < dataList.size(); j++) {
				if(!nameList.contains(branchSite1.getSiteName())){
					Assert.assertEquals(updateWs.getDataLocations().size(), ScmInfo.getSiteNum()-1, obj.toString());
					System.out.println(updateWs.getDataLocations().size());
					return;
				}
			}
			if(i==14)Assert.fail("remove site is not done in 15 seconds: "+obj.toString());
		}
	}
		...
		for (int i = 0; i < 15; i++) {
			...
			for (int j = 0; j < dataList.size(); j++) {
				...
				if(nameList.contains(branchSite2.getSiteName())){
					Assert.assertEquals(updateWs.getDataLocations().size(), ScmInfo.getSiteNum(), obj.toString());
					System.out.println(updateWs.getDataLocations().size());
					return;
				}
			}
			if(i==14)Assert.fail("add site is not done in 15 seconds: "+obj.toString());
		}
	...
	private void wsAddAndRemoveSite() throws Exception {
		...
		for (int i = 0; i < 15; i++) {
			...
			for (int j = 0; j < dataList.size(); j++) {
				...
				if(!nameList.contains(branchSite2.getSiteName())){
					System.out.println("------------------------------------"+ws.getDataLocations().size());
					Assert.assertEquals(updateWs.getDataLocations().size(), ScmInfo.getSiteNum()-1, obj.toString());
					return;
				}
			}
