...
public class SetContentAndDeleteFile1401 extends TestScmBase {	
	...
	private String fileName = "breakpointfile1401";	
	...
	private void test() throws Exception {		
		...
		if( setContentBreakpointFile.isSuccess()){
			if( !deleteBreakpointFile.isSuccess()){
				Assert.assertTrue( !deleteBreakpointFile.isSuccess(), deleteBreakpointFile.getErrorMsg());
				ScmException e =(ScmException) deleteBreakpointFile.getExceptions().get(0);				
				Assert.assertEquals(e.getError(),ScmError.FILE_NOT_FOUND,"delete must fail:"+deleteBreakpointFile.getErrorMsg());
				checkFileData();
			}else{
				Assert.fail("delete must be fail");				
			}
		}else if(!setContentBreakpointFile.isSuccess()){
			Assert.assertTrue( deleteBreakpointFile.isSuccess(), deleteBreakpointFile.getErrorMsg());
			ScmException e =(ScmException) setContentBreakpointFile.getExceptions().get(0);			
			if( ScmError.FILE_NOT_FOUND != e.getError() && ScmError.INVALID_ARGUMENT != e.getError()&& ScmError.HTTP_NOT_FOUND != e.getError()){
				Assert.fail("upload should fail e:"+e.getErrorCode()+":"+setContentBreakpointFile.getErrorMsg());		
			}			
			checkDeleteResult();
		}		
	}
	...
	private class SetContentBreakpointFileThread extends TestThreadBase {
		@Override
		public void exec() throws Exception {
			ScmSession session = null;
			try {
