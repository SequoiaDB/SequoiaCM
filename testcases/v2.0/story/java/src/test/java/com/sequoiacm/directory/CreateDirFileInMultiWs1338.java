...
public class CreateDirFileInMultiWs1338 extends TestScmBase {
    ...
    private void setUp() throws ScmException {
        if ( ScmInfo.getWsNum() < wsNum ) {
            throw new SkipTestException( "ws num is less than need,skip" );
        }
    ...
    private void tearDown() throws Exception {
        try {
            if ( runSuccess || TestScmBase.forceClear ) {
                ...
                deleteDir( ws1,
                        dirBasePath + "/" + eleName1 + "_" + ws1.getName() );
                deleteDir( ws2,
                        dirBasePath + "/" + eleName1 + "_" + ws2.getName() );
            }
        } finally {
            if ( session != null ) {
                session.close();
    ...
    private void createDiffDirAndFile( ScmWorkspace ws, ScmDirectory dir )
            throws ScmException {
        ScmId fileId = null;
        try {
            ScmFile file = ScmFactory.File.createInstance( ws );
            ...
            fileIdList.add( fileId );
            ScmFactory.Directory.createInstance( ws,
                    dirBasePath + "/" + eleName1 + "_" + ws.getName() );
        } catch ( ScmException e ) {
            e.printStackTrace();
            Assert.fail( e.getMessage() );
        }
