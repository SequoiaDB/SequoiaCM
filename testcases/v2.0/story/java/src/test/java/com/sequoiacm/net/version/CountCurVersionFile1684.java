...
public class CountCurVersionFile1684 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		BSONObject[] option1 = {new BasicBSONObject("author", "fileVersion1684"), 
								new BasicBSONObject("size",new BasicBSONObject("$gt",102400)), 
								new BasicBSONObject("major_version",new BasicBSONObject("$lt",5))};
		filter1.put("$and", option1 );
		String[] fileNameArr1 = {"fileVersion1684_1", "fileVersion1684_2", "fileVersion1684_3"}; 
		listInstanceByOption( ws, ScopeType.SCOPE_CURRENT, filter1, fileNameArr1);
		
		//组合id/major_version
		BSONObject filter2 = new BasicBSONObject();
		BSONObject[] option2 = {new BasicBSONObject("author", "fileVersion1684"), 
								new BasicBSONObject("id", fileId.get()), 
								new BasicBSONObject("major_version",new BasicBSONObject("$gt",1))};
		filter2.put("$and", option2 );
		String[] fileNameArr2 = {"fileVersion1684_3"}; 
		listInstanceByOption( ws, ScopeType.SCOPE_CURRENT, filter2, fileNameArr2);
		
		//组合name/major_version
		BSONObject filter3 = new BasicBSONObject();
		BSONObject[] option3 = {new BasicBSONObject("author", "fileVersion1684"), 
								new BasicBSONObject("name", "fileVersion1684_1"), 
								new BasicBSONObject("major_version",new BasicBSONObject("$lt",6)),
								new BasicBSONObject("site_list.0.site_id",site.getSiteId())};
		filter3.put("$and", option3 );
		String[] fileNameArr3 = {"fileVersion1684_1"}; 
		listInstanceByOption( ws, ScopeType.SCOPE_CURRENT, filter3, fileNameArr3);
	...
	private void listInstanceByOption(ScmWorkspace ws, ScopeType scopeType, BSONObject filter, String[] fileNameArr) throws ScmException {
		System.out.println("filter: "+filter.toString());
		ScmCursor<ScmFileBasicInfo> fileCursor = ScmFactory.File.listInstance(ws, scopeType, filter);
		...
		Assert.assertEquals(fileInfoList.size(), fileNameArr.length, "select file number error");
	...
	private class ListComparator implements Comparator<ScmFileBasicInfo>{
		...
		@Override
		public int compare(ScmFileBasicInfo fileInfo1, ScmFileBasicInfo fileInfo2){
