...
public class ListFileInPaDir2592 extends TestScmBase{
	...
	public Object[][] generateRangData() throws Exception {
		BSONObject dPositive1 = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).is(1)
				.and(ScmAttributeName.File.CREATE_TIME).is(-1).and(ScmAttributeName.File.FILE_NAME).is(-1).get();
		BSONObject dPositive2 = ScmQueryBuilder.start(ScmAttributeName.File.CREATE_TIME).is(1)
				.and(ScmAttributeName.File.FILE_ID).is(-1).and(ScmAttributeName.File.FILE_NAME).is(1).get();
		String[] sortNameAtr1 = new String[]{"fileId", "createDate", "fileName"};
		boolean[] typeAtr1 = new boolean[]{true, false, false};

		String[] sortNameAtr2 = new String[]{"createDate", "fileId", "fileName"};
		boolean[] typeAtr2 = new boolean[]{true, false, true};
		return new Object[][]{
				//orderby:多个字段正序
				{filter, dPositive1, 0, 10, fileList, sortNameAtr1, typeAtr1},
				{filter, dPositive2, 0, 10, fileList, sortNameAtr2, typeAtr2}
		};
	}
