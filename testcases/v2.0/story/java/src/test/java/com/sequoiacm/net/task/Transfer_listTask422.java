...
public class Transfer_listTask422 extends TestScmBase {
    ...
    private void setUp() throws Exception {
        ...
        try {
            ...
            List< SiteWrapper > siteList = ScmNetUtils
                    .getRandomSites( wsList_T.get( 0 ) );
            ...
            ScmWorkspace ws2 = ScmFactory.Workspace
                    .getWorkspace( wsList_T.get( 1 ).getName(), session );
            prepareFiles2( ws2 );
        } catch ( Exception e ) {
            if ( session != null ) {
                session.close();
    ...
    private void listMultiWs() throws Exception {
        ...
        try {
            ...
            BSONObject subCond1 = ScmQueryBuilder.start( taskIdKey )
                    .is( taskId1.get() ).get();
            BSONObject subCond2 = ScmQueryBuilder.start( taskIdKey )
                    .is( taskId2.get() ).get();
            BSONObject cond = ScmQueryBuilder.start().or( subCond1, subCond2 )
                    .get();
            ...
            while ( cursor.hasNext() ) {
                ...
                if ( info.getId().get().equals( taskId1.get() ) ) {
                    Assert.assertEquals( info.getWorkspaceName(),
                            wsList_T.get( 0 ).getName() );
                } else if ( info.getId().get().equals( taskId2.get() ) ) {
                    Assert.assertEquals( info.getWorkspaceName(),
                            wsList_T.get( 1 ).getName() );
                } else {
    ...
    private void listOneWs() throws Exception {
        try {
            ...
            for ( int i = 0; i < taskNum; ++i ) {
                ScmId taskId = transferAllFile( session,
                        wsList_T.get( 0 ).getName() );
            ...
            for ( ScmId taskId : taskIdList ) {
                BSONObject subCond = ScmQueryBuilder.start( taskIdKey )
                        .is( taskId.get() ).get();
                cond.or( subCond );
            }

            ScmCursor< ScmTaskBasicInfo > cursor = ScmSystem.Task
                    .listTask( session, cond.get() );
            while ( cursor.hasNext() ) {
                ScmTaskBasicInfo info = cursor.getNext();
                Assert.assertEquals( info.getRunningFlag(),
                        CommonDefine.TaskRunningFlag.SCM_TASK_FINISH ); // 3:
                // finish
                Assert.assertEquals( info.getType(),
                        CommonDefine.TaskType.SCM_TASK_TRANSFER_FILE ); // 1:
                // transfer
                Assert.assertEquals( info.getWorkspaceName(),
                        wsList_T.get( 0 ).getName() );
            ...
            e.printStackTrace();
            Assert.fail(
                    e.getMessage() + "taskList Info " + taskIdList.toString() );
        }
        runSuccess = true;
    }
    ...
    private ScmId transferAllFile( ScmSession session, String wsName )
            throws ScmException {
