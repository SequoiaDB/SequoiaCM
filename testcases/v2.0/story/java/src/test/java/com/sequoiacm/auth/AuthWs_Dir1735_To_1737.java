...
public class AuthWs_Dir1735_To_1737 extends TestScmBase {
    ...
    private String[] privileges = { ScmPrivilegeDefine.READ, ScmPrivilegeDefine.DELETE,
	    ...
	    ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ + "|" + ScmPrivilegeDefine.UPDATE,

	    ScmPrivilegeDefine.READ + "|" + ScmPrivilegeDefine.UPDATE + "|" + ScmPrivilegeDefine.DELETE,

	    ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.UPDATE + "|" + ScmPrivilegeDefine.DELETE,

	    ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ + "|" + ScmPrivilegeDefine.UPDATE + "|"
		    + ScmPrivilegeDefine.DELETE,

	    ScmPrivilegeDefine.ALL };
    private String[] dirpaths = { "/1735_A/1735_B", "/1735_B/1735_C", "/1735_C/1735_D", "/1735_D/1735_E",

	    "/1735_E/1735_E/1735_G", "/1735_E/1735_E/1735_H",

	    "/1736_E/1736_E/1736_G", "/1736_E/1736_F/1736_A/1736_B/1736_C/1736_D",

	    "/1736_F", "/1736_H", "/1736_H/1736_A",

	    "/H_1737/I_1737/A_1737", "/A_1737/TEXT_1737/MV_1737/WORD_B/PICTURE_C/EngLish_D",

	    "/LANGUAGE_1737_1", "/TEXT_1737_BACKUP", "/1737_I",

	    "/OneSevenThreeFive_Four", "/SevenThreeSevenOne_Three", "/SixThreeSevenOne_Two",
	...
	try {
	    ...
	    ScmDirectory readDir = ScmFactory.Directory.getInstance(wsUser, path);
	...
	try {
	    ...
	    Assert.fail("the user does not have priority to delete dir = " + path);
	} catch (ScmException e) {
	...
	try {
	    ...
	    Assert.fail("the user have delete priority but delete dir fail,dir = " + path);
	} catch (ScmException e) {
	...
	try {
	    ...
	    ScmDirectory createDir = ScmFactory.Directory.createInstance(wsUser, path);
	...
	try {
	    ...
	    // read again
	    ScmDirectory dir = ScmFactory.Directory.getInstance(wsUser, newpath);
	...
	try {
	    ...
	    Assert.fail("the user does not have priority to create dir= " + path);
	} catch (ScmException e) {
	    if (e.getError() != ScmError.OPERATION_UNAUTHORIZED) {
		e.printStackTrace();
		Assert.fail(e.getMessage());
	...
	try {
	    actDir = ScmFactory.Directory.createInstance(wsA, path);
	    ScmFactory.Directory.getInstance(wsUser, path);
	    Assert.fail("the user does not have priority to read dir= " + path);
	} catch (ScmException e) {
	    if (e.getError() != ScmError.OPERATION_UNAUTHORIZED) {
		e.printStackTrace();
		Assert.fail(e.getMessage());
	...
	try {
		...
		ScmFactory.Role.revokePrivilege(sessionA, role, dirrsList.get(i), privileges[i % privileges.length]);
	    }
	...
	try {
		...
		ScmResource dirrs = ScmResourceFactory.createDirectoryResource(wsp.getName(), dirpaths[i]);
		dirrsList.add(dirrs);
		System.out
			.println("dir" + i + " = " + dirpaths[i] + ",privilege = " + privileges[i % privileges.length]);
		grantPriAndAttachRole(sessionA, dirrs, user, role, privileges[i % privileges.length]);
	    }
	    ...
	    wsUser = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionUser);
	} catch (ScmException e) {
	    e.printStackTrace();
	    Assert.fail(e.getMessage());
	}
