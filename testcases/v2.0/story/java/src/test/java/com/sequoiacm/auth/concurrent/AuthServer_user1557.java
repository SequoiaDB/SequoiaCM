...
public class AuthServer_user1557 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1557.class);
	...
	private void test() throws ScmException {
		...
		ScmSession ss = TestScmTools.createSession(site, NAME, "new_" + PASSWORD);
		ss.close();	
	...
	private class CleanSession extends TestThreadBase {		
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmFactory.Session.deleteSession(session, ssIdList.get(tmpI));
			} catch (ScmException e) {
