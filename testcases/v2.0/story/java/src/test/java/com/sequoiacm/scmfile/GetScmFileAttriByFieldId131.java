...
public class GetScmFileAttriByFieldId131 extends TestScmBase {
	...
	private void testGetScmFileAllAttriByFieldID() {
		try {
			...
			Assert.assertEquals(file.getDataId().get(), file.getFileId().get());

			int siteId = file.getLocationList().get(0).getSiteId();
			Assert.assertEquals(siteId, site.getSiteId());

			long accessTime = file.getLocationList().get(0).getDate().getTime();
			if (accessTime != file.getUpdateTime().getTime()) {
				throw new Exception("file.getLocationList is error. " + "accessTime=" + accessTime + ", "
						+ "createTime=" + file.getCreateTime().getTime());
			}
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		}
