...
public class And327 extends TestScmBase {
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			BSONObject cond2 = ScmQueryBuilder.start().and(obj).and(obj).get();
			list.add(cond1);
			list.add(cond2);
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			String expCond = "{ \"$and\" : [ " + obj1.toString() + " , " + obj2.toString() + "]}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));
	...
	private void readyScmFile() {
		try {
			for (int i = 0; i < fileNum; i++) {
				...
				scmfile.setFileName(TestTools.getRandomString(9) + "_" + author + "_" + i);
