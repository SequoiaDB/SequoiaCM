...
public class ListScmFile2591 extends TestScmBase {
    ...
    private void testInvalidScope() throws ScmException {
        try {
            ScmFactory.File.listInstance(ws, ScmType.ScopeType.SCOPE_ALL,new BasicBSONObject(),
    ...
    private void testInvalidLimit() throws ScmException {
        try {
            ScmFactory.File.listInstance(ws, ScmType.ScopeType.SCOPE_CURRENT,new BasicBSONObject(),
    ...
    private void testInvalidSkip() throws ScmException {
        try {
            ScmFactory.File.listInstance(ws, ScmType.ScopeType.SCOPE_HISTORY,new BasicBSONObject(),
    ...
    private void testWSisNull() throws ScmException {
        try {
            ScmFactory.File.listInstance(null, ScmType.ScopeType.SCOPE_HISTORY,new BasicBSONObject(),
                    new BasicBSONObject(),0,1);
            Assert.fail("exp fail but act success!!!");
        } catch (ScmException e) {
            if(e.getError() != ScmError.INVALID_ARGUMENT){
    ...
    private void testCondIsNull() throws ScmException {
        try {
            ScmFactory.File.listInstance(null, ScmType.ScopeType.SCOPE_HISTORY,null,
                    new BasicBSONObject(),0,1);
            Assert.fail("exp fail but act success!!!");
        } catch (ScmException e) {
            if(e.getError() != ScmError.INVALID_ARGUMENT){
