...
public class Sche_UpdateScopeByClean1805b extends TestScmBase {	
	...
	private void test() throws Exception {			
		...
		VersionUtils.checkScheTaskFileSites(wsM, fileIdList,historyVersion,exphHisSiteList);		
		...
		VersionUtils.checkScheTaskFileSites(wsA, fileIdList,currentVersion,expCurSiteList_b);
		SiteWrapper[] exphHisSiteList_b = {  rootSite, branSite  };	
		VersionUtils.checkScheTaskFileSites(wsM, fileIdList,historyVersion,exphHisSiteList_b);
		...
		VersionUtils.checkScheTaskFileSites(wsA, fileIdList,currentVersion,expCurSiteList_c);
		SiteWrapper[] exphHisSiteList_c = {  rootSite };	
		VersionUtils.checkScheTaskFileSites(wsM, fileIdList,historyVersion,exphHisSiteList_c);
		runSuccess = true;
	}
	...
	private void createScheduleTask(ScmSession session) throws ScmException {
		...
		content = new ScmScheduleCleanFileContent(branSite.getSiteName(), maxStayTime, condition, ScopeType.SCOPE_HISTORY);
		cron = "* * * * * ?";
	...
	private void updateScheTaskByScopeType(ScmWorkspace ws, ScopeType scope) throws ScmException {			
