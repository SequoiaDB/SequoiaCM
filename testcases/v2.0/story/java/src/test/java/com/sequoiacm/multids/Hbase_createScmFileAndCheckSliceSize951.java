...
public class Hbase_createScmFileAndCheckSliceSize951 extends TestScmBase {
	...
	private int fileSize = 1024 * 1024 * 2 + new Random().nextInt(1024 * 1024);
	...
	private void checkHbaseFileSliceSize() throws ScmException {
		...
		try {
			...
			String tableName = TestSdbTools.getDataTableNameInHbase(site, wsp);
			...
			for (int i = 0; i < num; i++) {
				...
				get.addColumn(Bytes.toBytes("SCM_FILE_DATA"), Bytes.toBytes("PIECE_NUM_" + i));
				...
				Assert.assertEquals(actSize, expSize, "fileId=" + fileId.get());
			}
			table.close();
		} catch (IOException e) {
			e.printStackTrace();
