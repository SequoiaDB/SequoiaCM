...
public class DefineAttr_tags_crudTags_2628 extends TestScmBase {
	...
	private void testFileTag() throws ScmException {
		...
		Assert.assertEquals(fileTags.toSet(),tags.toSet(),
				"fileTags = " + fileTags.toString() + ",tagSet = " + tags.toString());

		//set null
		file.setTags(null);
		file = ScmFactory.File.getInstance(ws,fileId);
		Assert.assertEquals(file.getTags().toString(),"[]",
				"fileTags = " + file.getTags().toString() + ",tagSet = " + tags.toString());
		//remove tag
		file.removeTag("test");
		file = ScmFactory.File.getInstance(ws,fileId);
		Assert.assertEquals(file.getTags().toString(),"[]",
		...
		file.addTag("test");
		file = ScmFactory.File.getInstance(ws,fileId);
		Assert.assertEquals(file.getTags().toSet().size(),1,file.getTags().toString());
		Assert.assertTrue(file.getTags().toSet().contains("test"),file.getTags().toString());
	}
	...
	private void testBatchTag() throws ScmException {
		...
		Assert.assertEquals(batch.getTags().toSet(),tags.toSet(),
				"actTags = " + batch.toString() + ",expTags = " + tags.toString());
		//set null
		batch.setTags(null);
		batch = ScmFactory.Batch.getInstance(ws,batchId);
		Assert.assertEquals(batch.getTags().toString(),"[]","actTags = " + batch.getTags().toString());
		...
		batch.addTag("test");
		batch = ScmFactory.Batch.getInstance(ws,batchId);
		Assert.assertEquals(batch.getTags().toSet().size(),1,batch.getTags().toString());
		Assert.assertTrue(batch.getTags().toSet().contains("test"),batch.getTags().toString());
	}
}