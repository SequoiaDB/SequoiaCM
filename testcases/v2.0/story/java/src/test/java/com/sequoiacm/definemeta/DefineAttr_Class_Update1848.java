...
public class DefineAttr_Class_Update1848 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			// create class
			expClass1 = ScmFactory.Class.createInstance(ws, classname + "_" + 1, desc);
			expClass2 = ScmFactory.Class.createInstance(ws, classname + "_" + 2, desc);
		} catch (Exception e) {
	...
	private void test() {
		...
		try {
			ScmClass actclass1 = ScmFactory.Class.getInstance(ws, expClass1.getId());
	...
	private void check(ScmClass actClass, ScmClass expClass) {
		...
		Assert.assertEquals(actClass.getDescription(), expClass.getDescription());
		Assert.assertEquals(actClass.getCreateUser(), expClass.getCreateUser());
		Assert.assertEquals(actClass.getUpdateUser(), expClass.getUpdateUser());
		Assert.assertEquals(actClass.getWorkspace(), expClass.getWorkspace());
		Assert.assertEquals(actClass.listAttrs(), expClass.listAttrs());
		Assert.assertEquals(actClass.getCreateTime(), expClass.getCreateTime());
		Assert.assertEquals(actClass.getUpdateTime(), expClass.getUpdateTime());
	}
}
