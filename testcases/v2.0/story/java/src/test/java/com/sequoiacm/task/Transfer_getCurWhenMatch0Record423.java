	...
	private ScmCursor<ScmTaskBasicInfo> getTaskList() {
		...
		while (true) {
			try {
				...
				if (task.getRunningFlag() == CommonDefine.TaskRunningFlag.SCM_TASK_FINISH) {
					BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Task.TYPE).is("aaa2").get();
					cursor = ScmSystem.Task.listTask(sessionA, cond);
					break;
				} else if (task.getRunningFlag() == CommonDefine.TaskRunningFlag.SCM_TASK_ABORT) {
