...
public class CreateWorkspace2179 extends TestScmBase {
	...
	private void createWorkspace(String wsName, int siteNum) throws Exception{
		...
		for (int i = 0; i < 15; i++) {
			...
			if(i==14)Assert.fail("create ws is not done in 15 seconds: "+obj.toString());
		}
		
		Assert.assertEquals(ws.getDataLocations().size(), ScmInfo.getSiteNum());
	 ...
	 */
	private BSONObject dataJson(int siteNum) throws ScmInvalidArgumentException, JSONException {
		...
		for (int i = 0; i < siteList.size(); i++) {
			...
			case "sequoiadb": 
				...
				scmDataLocationList.add(new ScmSdbDataLocation(siteName, domainName).getBSONObject());
				break;
			case "hbase":
				scmDataLocationList.add(new ScmHbaseDataLocation(siteName).getBSONObject());
				break;
			case "hdfs":
				scmDataLocationList.add(new ScmHdfsDataLocation(siteName).getBSONObject());
				break;
			case "ceph_s3":
				scmDataLocationList.add(new ScmCephS3DataLocation(siteName).getBSONObject());
				break;
			case "ceph_swift":
				scmDataLocationList.add(new ScmCephSwiftDataLocation(siteName).getBSONObject());
				break;
			default:
