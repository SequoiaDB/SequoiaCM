...
public class TestTaskCond1102 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			String response1 = rest.setApi("tasks").setRequestMethod(HttpMethod.POST).setParameter("task_type", "2")
	...
	private JSONObject generateOptions() throws JSONException {
		...
		filter.put("id", new JSONObject().put("$lt", "ffffffffffffffffffffffff"));
		filter.put("author", new JSONObject().put("$lt", author));
		filter.put("title", new JSONObject().put("$gte", author));
		filter.put("mime_type", new JSONObject().put("$gte", "application/octet-stream"));
		filter.put("major_version", new JSONObject().put("$lt", "2147483647"));
		filter.put("minor_version", new JSONObject().put("$lt", "2147483647"));
		filter.put("user", new JSONObject().put("$gt", TestScmBase.scmUserName));
