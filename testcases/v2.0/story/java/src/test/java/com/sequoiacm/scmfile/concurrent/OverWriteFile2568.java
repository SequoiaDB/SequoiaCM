...
public class OverWriteFile2568 extends TestScmBase{
    ...
    private void test() throws Exception {
        ...
        threadExec.run();
        //check result
        Assert.assertEquals(fileIdList.size(), fileNum, "fileIdList = " + fileIdList.toString());
    ...
    private class OverWriteFile {
        ...
        private void overwriteScmFile() throws ScmException {
            //这里是覆盖文件
            try {
                fileIdList.add(this.scmFile.save(new ScmUploadConf(true)));
            } finally {
                if (session != null) {
                    session.close();
                }
    ...
    private void checkFile(ScmFile file, String expVal, int expSize, String expFilePath) throws Exception {
        try {
