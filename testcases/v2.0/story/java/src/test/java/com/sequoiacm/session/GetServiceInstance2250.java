...
public class GetServiceInstance2250 extends TestScmBase {
	...
	private void testListByNull() {
		...
		try {
			list = ScmSystem.ServiceCenter.getServiceInstanceList(session, null);
			for (ScmServiceInstance instance : list) {
				Assert.assertEquals(instance.getStatus(), "UP",instance.toString());
				Assert.assertNotNull(instance.getIp(),instance.toString());
				Assert.assertNotNull(instance.getPort(),instance.toString());
				Assert.assertNotNull(instance.getRegion(),instance.toString());
	...
	private void testListByEmptyStr() {
		...
		try {
			...
			List<ScmServiceInstance> list1 = ScmSystem.ServiceCenter.getServiceInstanceList(session, null);
			Assert.assertEquals(list.size(), list1.size(),
					"list = " + list.toString() + ";list1 = " + list1.toString());
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private void testListByNotAuthServiceName() {
		...
		try {
			session = sessionMgr.getSession(SessionType.NOT_AUTH_SESSION);
			list = ScmSystem.ServiceCenter.getServiceInstanceList(session, site.getSiteServiceName());
			for (ScmServiceInstance instance : list) {
	...
	private void testListByInexist() {
		try {
			ScmSystem.ServiceCenter.getServiceInstanceList(session, site.getSiteServiceName() + "1");
