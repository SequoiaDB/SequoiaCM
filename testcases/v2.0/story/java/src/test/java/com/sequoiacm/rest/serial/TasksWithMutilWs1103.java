...
public class TasksWithMutilWs1103 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			for (int i = 0; i < fileNum; i++) {
				...
				// for clean
				String fileId1 = upload(filePath, wsList.get(0), desc.toString(), rest1);
				download(fileId1, rest2, wsList.get(0));
				fileIdList1.add(new ScmId(fileId1));
				// for transfer
				String fileId2 = upload(filePath, wsList.get(1), desc.toString(), rest2);
	...
	private void tearDown() throws Exception {
		try {
			if (runSuccess || TestScmBase.forceClear) {
				for (int i = 0; i < fileNum; i++) {
					rest1.reset()
							.setApi("files/" + fileIdList1.get(i).get() + "?workspace_name=" + wsList.get(0).getName()+"&is_physical=true")
							.setRequestMethod(HttpMethod.DELETE).setResponseType(String.class).exec();
					rest2.reset()
							.setApi("files/" + fileIdList2.get(i).get() + "?workspace_name=" + wsList.get(1).getName()+"&is_physical=true")
							.setRequestMethod(HttpMethod.DELETE).setResponseType(String.class).exec();
	...
	private void transferAndCheck() throws JSONException {
		...
		String response1 = rest2.setApi("tasks").setRequestMethod(HttpMethod.POST).setParameter("task_type", "1")
				.setParameter("workspace_name", wsList.get(1).getName()).setParameter("options", options.toString())
	...
	private void cleanAnCheck() throws JSONException {
		JSONObject options = new JSONObject().put("filter", new JSONObject().put("author", author));
		try {
			String response1 = rest2.setApi("tasks").setRequestMethod(HttpMethod.PUT).setParameter("task_type", "2")
					.setParameter("workspace_name", wsList.get(0).getName()).setParameter("options", options.toString())
					.setResponseType(String.class).exec().getBody().toString();
			taskId2 = new JSONObject(response1).getJSONObject("task").getString("id");
	...
	private void check(JSONObject taskInfo, WsWrapper ws, List<ScmId> fileIdList, SiteWrapper[] expSiteList)
			throws JSONException {
		// check site
		try {
			ScmFileUtils.checkMetaAndData(ws, fileIdList, expSiteList, localPath, filePath);
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	public void download(String fileId, RestWrapper rest, WsWrapper ws) {
		rest.reset().setApi("files/" + fileId + "?workspace_name=" + ws.getName()).setRequestMethod(HttpMethod.GET)
