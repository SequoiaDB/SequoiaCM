	...
	private class ReloadConf extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmConfigOption scOpt = new ScmConfigOption(TestScmBase.gateWayList.get(0)+"/"+rootSite.getSiteServiceName(), user, passwd);
				session = ScmFactory.Session.createSession(SessionType.NOT_AUTH_SESSION, scOpt);
				ScmSystem.Configuration.reloadBizConf(ServerScope.ALL_SITE, ScmInfo.getRootSite().getSiteId(), session);
			} catch (ScmException e) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
			} finally {
