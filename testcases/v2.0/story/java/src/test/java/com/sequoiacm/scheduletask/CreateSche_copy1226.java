...
public class CreateSche_copy1226 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			...
			//checkTask info
			BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Schedule.ID).is(scheduleId.get()).get();
			checkTaskInfo(cond);
	...
	private void createScheduleTask() {
		...
		try {
			// test interface
			String srName = new ScmScheduleCopyFileContent(branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond).getSourceSiteName();
			String trName = new ScmScheduleCopyFileContent(branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond).getTargetSiteName();
			String msTime = new ScmScheduleCopyFileContent(branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond).getMaxStayTime();
			BSONObject eCond = new ScmScheduleCopyFileContent(branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond).getExtraCondition();
...
//			System.out.println("sche info :" + sche);
	...
	private void checkTaskInfo(BSONObject cond) throws ScmException {
		ScmCursor<ScmTaskBasicInfo> cursor = ScmSystem.Task.listTask(ssA, cond);
		try {
			while (cursor.hasNext()) {
				ScmTaskBasicInfo info = cursor.getNext();
				Assert.assertEquals(info.getRunningFlag(), CommonDefine.TaskRunningFlag.SCM_TASK_FINISH);
				Assert.assertEquals(info.getType(), CommonDefine.TaskType.SCM_TASK_TRANSFER_FILE);
