...
public class BreakpointFile1370 extends TestScmBase {
	...
	private void test() throws JSONException, ScmException, IOException {
		...
		BreakpointUtil.createBreakpointFile(ws, filePath,fileName,partFileSize, ScmChecksumType.ADLER32);
		//检查断点文件信息
	...
	private void checkBreakpointFile(int partFileSize) throws ScmException {
		...
		Assert.assertEquals(breakpointFile.getWorkspace().getName(), ws.getName());
		Assert.assertEquals(breakpointFile.getChecksumType().name(), ScmChecksumType.ADLER32.name());
	}
	
	private ScmBreakpointFile uploadBreakpointFile() {
		ScmBreakpointFile breakpointFile = null;
		try{
			breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName);
