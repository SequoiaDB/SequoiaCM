...
public class ListScmFile2577 extends TestScmBase {
    ...
    private void setUp() throws Exception {
        ...
        //prepare filter
        //all file
        filter1 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(fileNamePrefix).get();
        //part of the file
        filter2 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(fileNamePrefix)
                .and(ScmAttributeName.File.TAGS).in("tally").get();
        //one file
        filter3 =ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(fileNamePrefix)
                .and(ScmAttributeName.File.FILE_NAME).is(fileNamePrefix + "-" + 50).get();
        //zero file
        filter4 =ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(fileNamePrefix)
                .and(ScmAttributeName.File.FILE_NAME).is(fileNamePrefix + "1-" + 50).get();
    ...
    public Object[][] generateRangData() throws Exception {
        BSONObject positive = ScmQueryBuilder.start(ScmAttributeName.File.CREATE_TIME).is(1).get();
        BSONObject negative = ScmQueryBuilder.start(ScmAttributeName.File.CREATE_TIME).is(-1).get();

        BSONObject dPositive = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME).is(1)
                .and(ScmAttributeName.File.CREATE_TIME).is(-1).get();
        BSONObject dNegative = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME).is(-1)
                .and(ScmAttributeName.File.CREATE_TIME).is(1).get();
        ...
        return new Object[][]{
                ...
                //skip>10 limt=50
                {filter2,positive, 10, 100,fileList2,sortNameAtr1,typeAtr1},
                //skip == fileList.size
                {filter3,positive, 1, 10, fileList3,sortNameAtr1,typeAtr1},
                //skip > fileList.size
                {filter4,positive, 1, 10,new ArrayList<ScmFileBasicInfo>(),sortNameAtr1,typeAtr1},
                //limit > fileList.size
                {filter2,positive, 0,fileList2.size() + 1,fileList2,sortNameAtr1,typeAtr1},
                //limit = -1
                {filter2,positive, 10, -1,fileList2,sortNameAtr1,typeAtr1},

                //orderby:单个字段 逆序
                {filter1,negative, 0, 1,fileList1,sortNameAtr1,typeAtr2},
                //skip>10 limt=50
                {filter2,negative, 10, 100,fileList2,sortNameAtr1,typeAtr2},
                //skip == fileList.size
                {filter3,negative, 1, 10, fileList3,sortNameAtr1,typeAtr2},
                //skip > fileList.size
                {filter4,negative, 1, 10,new ArrayList<ScmFileBasicInfo>(),sortNameAtr1,typeAtr2},
                //limit > fileList.size
                {filter2,negative, 0,fileList2.size() + 1,fileList2,sortNameAtr1,typeAtr2},
                //limit = -1
                {filter2,negative, 0, -1 ,fileList2,sortNameAtr1,typeAtr2},

                //orderby:多个字段 正逆序
                {filter1,dPositive, 0, 10, fileList1,sortNameAtr2,typeAtr3},
                {filter1,dNegative, 2, 20, fileList1,sortNameAtr2,typeAtr4}
        };
    }

    @Test(dataProvider = "dataProvider")
    private void test(BSONObject filter,BSONObject orderby, long skip, long limit, List<ScmFileBasicInfo> list
        ...
        try {
            ...
            while (tmpSkip < tmpList.size()) {
                ...
                while (cursor.hasNext()) {
                    ...
                    ScmFileBasicInfo exp = tmpList.get((int) (tmpSkip + count));
                    try {
                        Assert.assertEquals(act.getFileName(), exp.getFileName());
                        Assert.assertEquals(act.getUser(), exp.getUser());
                        Assert.assertEquals(act.getFileId(), exp.getFileId());
                        Assert.assertEquals(act.getUser(),exp.getUser());
                        Assert.assertEquals(act.getCreateDate(),exp.getCreateDate());
                        Assert.assertEquals(act.getMajorVersion(),exp.getMajorVersion());
                        Assert.assertEquals(act.getMinorVersion(),exp.getMinorVersion());
                        Assert.assertEquals(act.getMimeType(),exp.getMimeType());
                        count++;
                    } catch (AssertionError e) {
                        throw new Exception( "filter = " + filter.toString() + ",orderby = " + orderby.toString()
                                + ",skip = " + skip + ",limit = " + limit + "，act = " + act.toString() + ",exp = "
                                + exp.toString(), e);
                    }
                }
                if (limit == 0 || count == 0) {
    ...
    private void getInitScmFileInfo(BSONObject filter,List<ScmFileBasicInfo> fileList) throws ScmException {
        ...
        try {
            cursor = ScmFactory.File.listInstance(ws, ScmType.ScopeType.SCOPE_CURRENT, filter);
            while (cursor.hasNext()){
