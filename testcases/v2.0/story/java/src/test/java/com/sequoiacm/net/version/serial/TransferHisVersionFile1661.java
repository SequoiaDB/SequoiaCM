...
public class TransferHisVersionFile1661 extends TestScmBase {	
	...
	private void test() throws Exception {			
		...
		startTransferTaskByHistoryVerFile(wsS, sessionS, scopeType);
		checkHistoryVerFileSiteAndDataInfo(wsT, historyVersion);
		checkCurVersionFileInfo(wsS, currentVersion);	
		runSuccess = true;
	}
	...
	private void startTransferTaskByHistoryVerFile(ScmWorkspace ws, ScmSession session, ScopeType scopeType) throws Exception {
		BSONObject condition = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).greaterThanEquals(fileSize1)
				.put(ScmAttributeName.File.MAJOR_VERSION).greaterThan(0).get();		
	...
	private void checkHistoryVerFileSiteAndDataInfo(ScmWorkspace ws, int historyVersion) throws Exception{
		//check the transfered file,check the sitelist and data
		BSONObject condition = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).greaterThanEquals(fileSize1)
				.put(ScmAttributeName.File.FILE_ID).in(fileIdList).get();		
		ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_HISTORY, condition);
		int size = 0;			
		...
		while (cursor.hasNext()) {
			...
			VersionUtils.checkSite(ws, fileId, historyVersion, expHisSiteList);
		...
		BSONObject condition1 = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).lessThan(fileSize1)
				.put(ScmAttributeName.File.FILE_ID).in(fileIdList).get();		
		...
		while (cursor1.hasNext()) {
			...
			VersionUtils.checkSite(ws, fileId1, historyVersion, expCurSiteList1);			
			size1++;
		}
		cursor1.close();		
	...
	private void checkCurVersionFileInfo(ScmWorkspace ws,int version) throws ScmException{
		//all current version file only on the branSite
		BSONObject condition = ScmQueryBuilder.start().put(ScmAttributeName.File.AUTHOR).is(authorName).get();	
