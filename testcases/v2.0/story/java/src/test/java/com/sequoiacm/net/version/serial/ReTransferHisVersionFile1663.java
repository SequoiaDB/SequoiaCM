...
/**
 * test content : the historyVersion file in both the sourceSite and the targetSite, 
...
public class ReTransferHisVersionFile1663 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		VersionUtils.CheckFileContentByStream(wsS, fileName, historyVersion, filedata);
		runSuccess = true;
	}
	...
	private ScmId startTransferTaskByHistoryVerFile(ScmWorkspace ws, ScmSession session) throws Exception {
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).is(fileId.get()).get();
		ScmId taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_HISTORY,
