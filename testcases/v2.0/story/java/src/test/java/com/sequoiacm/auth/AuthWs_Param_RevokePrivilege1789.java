...
public class AuthWs_Param_RevokePrivilege1789 extends TestScmBase {
	...
	try {
	    ...
	    session.close();
	    ScmFactory.Role.revokePrivilege(session, role, rs, ScmPrivilegeType.READ);
	} catch (ScmException e) {
	...
	try {
	    ScmFactory.Role.revokePrivilege(sessionA, role, null, ScmPrivilegeType.READ);
	} catch (ScmException e) {
	    if (e.getError() != ScmError.INVALID_ARGUMENT) {
		e.printStackTrace();
		Assert.fail(e.getMessage());
	...
	try {
	    ScmFactory.Role.revokePrivilege(sessionA, role, rs, ScmPrivilegeType.READ + "1");
	} catch (ScmException e) {
	    if (e.getError() != ScmError.PRIVILEGE_REVOKE_FAILED) {
		e.printStackTrace();
		Assert.fail(e.getMessage());
