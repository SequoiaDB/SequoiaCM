...
public class CrudDirectory2198 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		String response = rest.setRequestMethod(HttpMethod.POST).setApi("/directories")
		...
		String response1 = rest.setRequestMethod(HttpMethod.POST).setApi("/directories")
				.setParameter("workspace_name", wsp.getName())
		...
		String response2 = rest.setRequestMethod(HttpMethod.HEAD)
				.setApi("/directories/id/" + id1 + "?workspace_name=" + wsp.getName())
		...
		Assert.assertEquals(dirInfo1.getString("name"), "CrudDirectory2198_A");
		Assert.assertEquals(dirInfo1.getString("id"), id1);
		Assert.assertEquals(dirInfo1.getString("parent_directory_id"), rootId);
		
		//get directory by path
		String response3 = rest.setRequestMethod(HttpMethod.HEAD)
				.setApi("/directories/path/" + fullPath2 + "?workspace_name=" + wsp.getName())
				.setResponseType(String.class)
				.exec().getHeaders().getFirst("directory");
		response3 = URLDecoder.decode(response3, "UTF-8");
		JSONObject dirInfo2 = new JSONObject(response3);
		Assert.assertEquals(dirInfo2.getString("name"), "CrudDirectory2198_B");
		Assert.assertEquals(dirInfo2.getString("id"), id2);
		Assert.assertEquals(dirInfo2.getString("parent_directory_id"), rootId);
		
		
		//get path by directory id
		String response4 = rest.setRequestMethod(HttpMethod.GET)
				.setApi("/directories/id/" + id1 + "/path?workspace_name=" + wsp.getName())
				.setResponseType(String.class).exec().getBody().toString();
		...
		String response7 = rest.setRequestMethod(HttpMethod.PUT)
				.setApi("/directories/path/" + "/CrudDirectory2198_B" + "/rename")
				.setParameter("workspace_name", wsp.getName())
		...
		String response8 = rest.setRequestMethod(HttpMethod.PUT)
				...
				.setParameter("parent_directory_path", "/CrudDirectory2198_B_New")
				.setParameter("workspace_name", wsp.getName())
				.exec().getBody()
				.toString();
		...
		String response9 = rest.setRequestMethod(HttpMethod.PUT)
				.setApi("/directories/path/" + "/CrudDirectory2198_B_New/CrudDirectory2198_A_New" + "/move")
		...
		rest.setRequestMethod(HttpMethod.DELETE)
				.setApi("/directories/path/" + "/CrudDirectory2198_B_New?workspace_name=" + wsp.getName())
				.exec();
