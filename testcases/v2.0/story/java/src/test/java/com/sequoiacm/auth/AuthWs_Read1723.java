...
public class AuthWs_Read1723 extends TestScmBase {
    ...
    private void testGetBatch() throws ScmException {
        ...
        try {
            ...
            ScmBatch actBatch = ScmFactory.Batch.getInstance(wsR, expBatch.getId());
    ...
    @Test(groups = {"twoSite", "fourSite"})
    private void testReadBreakpointFileByName() throws IOException, ScmException {
        BreakpointUtil.checkDBDataSource();
        String fileName = "AuthWs_Read1723" + UUID.randomUUID();
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsA, fileName);
            ...
            ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(wsR, fileName);

            Assert.assertEquals(breakpointFile.getFileName(), expBreakpointFile.getFileName());
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
    ...
    @Test(groups = {"twoSite", "fourSite"})
    private void testReadBreakpointFileBySize() throws IOException, ScmException {
        BreakpointUtil.checkDBDataSource();
        String fileName = "AuthWs_Read1723" + UUID.randomUUID();
        ScmBreakpointFile expBreakpointFile = null;
        try {
            ...
            ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(wsR, fileName, 0);
    ...
    private void testReadFileByPath() throws Exception {
        ...
        try {
            ...
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsR, "/" + fileName);
            String downloadPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
                    Thread.currentThread().getId());
            actfile.getContent(downloadPath);
            Assert.assertEquals(actfile.getFileName(), actfile.getFileName());
    ...
    private void testReadFileByPathVersion() throws Exception {
        ...
        try {
            ...
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsR, "/" + fileName, 1, 0);
            String downloadPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
                    Thread.currentThread().getId());
            actfile.getContent(downloadPath);
            Assert.assertEquals(actfile.getFileName(), actfile.getFileName());
    ...
    private void testGetSche() throws Exception {
        ...
        try {
            ...
            ScmSchedule actSche = ScmSystem.Schedule.get(sessionR, expSche.getId());
    ...
    private void testCountFile() throws Exception {
        long i = 0;
        try {
            i = ScmFactory.File.countInstance(wsR, ScopeType.SCOPE_ALL, new BasicBSONObject());
