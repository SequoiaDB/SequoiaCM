...
public class AuthWs_dir2164 extends TestScmBase {
    private static final Logger logger = Logger.getLogger(AuthWs_dir2164.class);
    ...
    private static final String[] DIR_PATH_ARRAY = {"/" + NAME + "_a/", "/" + NAME + "_a/" + NAME + "_b1/",
            "/" + NAME + "_a/" + NAME + "_b2/", "/" + NAME + "_a/" + NAME + "_b3/",
            "/" + NAME + "_a/" + NAME + "_b3/" + NAME + "_c4/", "/" + NAME + "_a/" + NAME + "_b3/" + NAME + "_c5/"};

    private static final String[] targetDIR_PATH_ARRAY = {"/" + NAME + "_a/" + NAME + "_b4/",
            "/" + NAME + "_a/" + NAME + "e/" + NAME + "g/", "/" + NAME + "_a/" + NAME + "_b3/" + NAME + "_c4/"};
    ...
    private void setUp() throws Exception {
        ...
        for (int i = 0; i < DIR_PATH_ARRAY.length - 1; i++) {
            ScmResource resource = ScmResourceFactory.createDirectoryResource(wsp.getName(), DIR_PATH_ARRAY[i]);
        ...
        ScmFactory.Role.grantPrivilege(session, role, resources.get(0), ScmPrivilegeType.CREATE);
        for (int i = 0; i < DIR_PATH_ARRAY.length - 1; i++) {
            ScmFactory.Role.grantPrivilege(session, role, resources.get(i), ScmPrivilegeType.READ);
        }
        ScmFactory.Role.grantPrivilege(session, role, resources.get(4), ScmPrivilegeType.DELETE);
    ...
    private void test1() throws Exception {
        ...
        try {
            ...
            ScmWorkspace tWS = ScmFactory.Workspace.getWorkspace(wsp.getName(), tSS);
    ...
    private void test2() throws Exception {
        ...
        try {
            ...
            System.out.println("path = " + path);
            ScmFactory.Directory.createInstance(tWS, "/" + NAME + "_a/" + NAME + "e/");
            ...
            if (dir != null) {
                ScmFactory.Directory.deleteInstance(ws, targetDIR_PATH_ARRAY[1]);
                ScmFactory.Directory.deleteInstance(ws, "/" + NAME + "_a/" + NAME + "e/");
            }
        }
        runSuccess = true;
    }
    ...
    private void test3() throws Exception {
        ...
        try {
            ...
            try {
                ...
                Assert.assertEquals(e.getError(), ScmError.DIR_NOT_FOUND, e.getMessage());
            }
        } finally {
            if (null != tSS) {
                tSS.close();
    ...
    private void tearDown() throws InterruptedException, ScmException {
        try {
            if (runSuccess || TestScmBase.forceClear) {
                ScmFactory.Role.revokePrivilege(session, role, resources.get(0), ScmPrivilegeType.CREATE);
                for (int i = 1; i < DIR_PATH_ARRAY.length - 1; i++) {
                    if (i != 4) {
                        ScmFactory.Role.revokePrivilege(session, role, resources.get(i), ScmPrivilegeType.READ);
                    }
                }
                ScmFactory.User.deleteUser(session, NAME);
                ScmFactory.Role.deleteRole(session, NAME);
