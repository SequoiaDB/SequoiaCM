...
/**
 * @description SCM-6084:禁用桶限额操作与上传对象并发
 * @author ZhangYanan
...
public class BucketQuota6084 extends TestScmBase {
    private boolean runSuccess = false;
    private String keyName = "object6084";
    private String bucketName = "bucket6084";
    ...
    public void test() throws Exception {
        ...
        t.addWorker( new DisableQuoteThread() );
        t.run();

        ScmBucketQuotaInfo quotaInfo = ScmFactory.Quota.getBucketQuota( session,
                bucketName );
        Assert.assertFalse( quotaInfo.isEnable() );
        checkObjectContent();
        runSuccess = true;
    }
    ...
    private class DisableQuoteThread {
        @ExecuteOrder(step = 1)
