...
public class NotEquals374 extends TestScmBase {
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).notEquals(111).put(key).notEquals(value)
					.put(ScmAttributeName.File.AUTHOR).is(file.getAuthor()).get();

			String expCond = "{ \"" + key + "\" : { \"$ne\" : " + value + "} , \"author\" : \"" + authorName + "\"}";
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).notEquals(value).put("k2").notEquals(" ")
					.put(ScmAttributeName.File.AUTHOR).is(file.getAuthor()).get();

			String expCond = "{ \"" + key + "\" : { \"$ne\" : " + value + "} , "
					+ "\"k2\" : { \"$ne\" : \" \"} , \"author\" : \"" + authorName + "\"}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));
