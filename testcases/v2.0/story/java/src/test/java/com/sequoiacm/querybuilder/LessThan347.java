...
/*
 * 1、countInstance带查询条件查询文件，对单个字段做lessThan匹配查询； 2、检查ScmQueryBuilder结果正确性；
 * 3、检查查询结果正确性；
 */

public class LessThan347 extends TestScmBase {
	...
	private static String fileName = "LessThan347";
	...
	private void testQuery() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).lessThan(value).put(ScmAttributeName.File.AUTHOR)
					.is(file.getAuthor()).get();

			Assert.assertEquals(cond.toString().replaceAll("\\s*",""),
					("{ \"" + key + "\" : { \"$lt\" : " + value + "} , \"author\" : \"" + author + "\"}").replaceAll("\\s*",""));
