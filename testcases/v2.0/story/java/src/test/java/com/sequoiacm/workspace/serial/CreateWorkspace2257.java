...
public class CreateWorkspace2257 extends TestScmBase {
	...
	private void test() throws ScmException, InterruptedException {
		...
		//check getCsOptions() and getClOptions
		Assert.assertEquals(scmMetaLocation.getClOptions().toString(), clOpt.toString());
		Assert.assertEquals(scmMetaLocation.getCsOptions().toString(), csOpt.toString());
		...
		BSONObject clBSON = snapshot(Sequoiadb.SDB_SNAP_CATALOG,
				"{\"Name\":\"" + wsName + "_META.FILE_" + Calendar.getInstance().get(Calendar.YEAR) + "\"}")
						.getCurrent();
		BSONObject csBSON = snapshot(Sequoiadb.SDB_SNAP_COLLECTIONSPACES, "{\"Name\":\"" + wsName + "_META\"}")
				.getCurrent();
		Assert.assertEquals(csBSON.get("PageSize"), csOpt.get("PageSize"));
		Assert.assertEquals(csBSON.get("LobPageSize"), csOpt.get("LobPageSize"));
		Assert.assertEquals(clBSON.get("CompressionTypeDesc"), clOpt.get("CompressionType"));
		Assert.assertEquals(clBSON.get("Partition"), clOpt.get("Partition"));
		Assert.assertEquals(clBSON.get("AutoSplit"), clOpt.get("AutoSplit"));
		Assert.assertEquals(clBSON.get("AttributeDesc"), "Compressed | StrictDataMode");
	}
	...
	private ScmWorkspace createWS(ScmSession session, String wsName, int siteNum, ScmMetaLocation scmMetaLocation)
