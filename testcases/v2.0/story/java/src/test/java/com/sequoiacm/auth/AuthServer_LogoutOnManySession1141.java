...
public class AuthServer_LogoutOnManySession1141 extends TestScmBase {
	...
	try {
	    ...
	    if (cursor == null) {
		Assert.fail("exp session cursor is not null but find cursor is null success");
	    }
	    while (cursor.hasNext()) {
		if (cursor.getNext().getSessionId().equals(session1.getSessionId())) {
		    Assert.fail("sesssion close is fail,session1 = " + session1.toString());
		}
		count++;
	    }
	...
	public void exec() {
	    try {
		ScmSession session = TestScmTools.createSession(site, username, passwd);
	...
	for (; i < 60; i++) {
		...
		if (e.getError() != ScmError.OPERATION_UNAUTHORIZED || i == 59) {
		    e.printStackTrace();
	...
	try {
	    ...
	    ScmFactory.Role.grantPrivilege(session, role, rs, ScmPrivilegeType.ALL);
