...
public class CreateWorksPace1937 extends TestScmBase {
	...
	private void setUp() throws ScmException{
		...
		metaDomainName = TestSdbTools.getDomainNames(rootSite.getMetaDsUrl()).get(0);
		rootDomainName = TestSdbTools.getDomainNames(rootSite.getDataDsUrl()).get(0);
		
		scmDataLocationList.add(new ScmSdbDataLocation(rootSite.getSiteName(), rootDomainName));
		scmDataLocationList.add(dataLocation(branSite1));
		scmMetaLocation = new ScmSdbMetaLocation(rootSite.getSiteName(), ScmShardingType.YEAR, metaDomainName);
	...
	private ScmDataLocation dataLocation(SiteWrapper site) throws ScmInvalidArgumentException{
