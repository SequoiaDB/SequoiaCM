...
public class CreateDir1132 extends TestScmBase {
	...
	private void check(ScmWorkspace ws, String dirPath) throws Exception {
		try {
			ScmDirectory scmDir = ScmFactory.Directory.getInstance(ws, dirPath);
			Assert.assertEquals(scmDir.getSubfile(author + "_" + scmDir.getName()).getFileName(), author + "_" + scmDir.getName());
			checkDirAttr(scmDir,'/'+ dirName + "/",dirName,"/");

			ScmCursor<ScmFileBasicInfo> fileCursor = scmDir
					.listFiles(new BasicBSONObject(ScmAttributeName.File.FILE_NAME, author + "_" + scmDir.getName()));
			checkFileCursorAttr(fileCursor);

			ScmDirectory subDir = scmDir.getSubdirectory(subDirName);
			Assert.assertNotNull(subDir.getSubfile(author + "_" + subDir.getName()));
			checkDirAttr(subDir,'/'+ dirName + "/" + subDirName  + "/",subDirName,dirName);
             
			ScmCursor<ScmDirectory> dirCursor = scmDir
					.listDirectories(new BasicBSONObject(ScmAttributeName.Directory.NAME, subDirName));
	...
	private void checkDirAttr(ScmDirectory scmDir,String path,String dirName,String paName) {
		try {
			Assert.assertEquals(scmDir.getName(), dirName);
			Assert.assertEquals(scmDir.getParentDirectory().getName(), paName);
			Assert.assertEquals(scmDir.getPath(), path);
			Assert.assertEquals(scmDir.getUpdateUser(), TestScmBase.scmUserName);
	...
	private void checkFileCursorAttr(ScmCursor<ScmFileBasicInfo> fileCursor) throws Exception {
		int i = 0;
		try {
			while (fileCursor.hasNext()) {
				...
				ScmFileUtils.checkMetaAndData(wsp, fileInfo.getFileId(), expSites, localPath, filePath);
				i++;
			}
			Assert.assertEquals(i, fileNum, "the dir has more than expected file's num");
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private void checkDirCurSorAttr(ScmCursor<ScmDirectory> dirCursor) {
		...
		try {
			while (dirCursor.hasNext()) {
				...
				Assert.assertEquals(dirInfo.getPath(), "/" + dirName + "/" + subDirName + "/");
				Assert.assertEquals(dirInfo.getWorkspaceName(), wsp.getName());
				i++;
			}
			Assert.assertEquals(i, subDirNum, "the dir has more than expected subdir's num");
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
