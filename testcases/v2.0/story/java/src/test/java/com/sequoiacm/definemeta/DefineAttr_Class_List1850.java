...
public class DefineAttr_Class_List1850 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			for (int i = 0; i < classNum; i++) {
				ScmClass class1 = ScmFactory.Class.createInstance(ws, classname + "_" + i, desc);
	...
	private void test() {
		...
		try {
			...
			ScmCursor<ScmClassBasicInfo> cursor = ScmFactory.Class.listInstance(ws, cond);
			int i = 0;
	...
	private BSONObject createCond() throws ScmException {
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Class.DESCRIPTION).exists(1)
				.and(ScmAttributeName.Class.CREATE_USER).greaterThanEquals(TestScmBase.scmUserName)
				.and(ScmAttributeName.Class.UPDATE_USER).lessThanEquals(TestScmBase.scmUserName)
				.and(ScmAttributeName.Class.CREATE_TIME).greaterThan(0).and(ScmAttributeName.Class.CREATE_TIME)
				.lessThan(Long.MAX_VALUE).and(ScmAttributeName.Class.NAME).notEquals(Integer.MAX_VALUE)
				.not(ScmQueryBuilder.start(ScmAttributeName.Class.DESCRIPTION).is(desc+"_1").get())
				.notIn(ScmQueryBuilder.start(ScmAttributeName.Class.DESCRIPTION).is(desc+"_1").get())
				.or(ScmQueryBuilder.start(ScmAttributeName.Class.DESCRIPTION).in(desc).get()).get();
		return cond;
	}
}
