...
public class DefineAttr_Class_ClassDetachAttr1874 extends TestScmBase {
	...
	private void test() throws ScmException {
		...
		Assert.assertEquals(class2.listAttrs().size(), 1, class2.listAttrs().toString());
		Assert.assertEquals(class2.listAttrs().get(0).getId(), attrList.get(0).getId(),
				class2.listAttrs().get(0).toString());
		Assert.assertEquals(class2.listAttrs().get(0).getName(), attrList.get(0).getName(),
				class2.listAttrs().get(0).toString());
		runSuccess = true;
	}
