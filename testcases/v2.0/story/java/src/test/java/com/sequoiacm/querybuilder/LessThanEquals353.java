...
/**
 * @FileName SCM-353: lessThanEquals多个不同字段，覆盖所有文件属性
 * @Author linsuqiang
 * @Date 2017-05-25
 * @Version 1.00
 */
...
/*
 * 1、countInstance带查询条件查询文件，对多个不同字段做lessThanEquals匹配查询，覆盖所有文件属性；
 * 2、检查ScmQueryBuilder结果正确性； 3、检查查询结果正确性；
 */

public class LessThanEquals353 extends TestScmBase {
	...
	private final String authorName = "LessThanEquals353";
		...
		try {
			...
			for (Object[] kv : kvs) {
				...
				if (kv[1] instanceof String) {
					subStr = key + "\" : { \"$lte\" : \"" + value + "\"}";
				} else {
					subStr = key + "\" : { \"$lte\" : " + value + "}";
				}
				if (null == builder) {
					builder = ScmQueryBuilder.start(key).lessThanEquals(value);
					bsStr = bsStr + subStr;
				} else {
					builder.put(key).lessThanEquals(value);
					bsStr = bsStr + " , \"" + subStr;
				}
			}
	...
	private Object[][] kvsArr() throws ScmException {
		...
		return new Object[][] { new Object[] { ScmAttributeName.File.FILE_ID, "ffffffffffffffffffffffff" }, // max
				...
				new Object[] { ScmAttributeName.File.UPDATE_USER, file.getUser() },
