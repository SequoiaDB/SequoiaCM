...
public class AuthWs_role1746 extends TestScmBase {
    private static final Logger logger = Logger.getLogger(AuthWs_role1746.class);
    ...
    private void test() throws Exception {
        ...
        ScmFactory.Role.grantPrivilege(session, roles.get(0), resource, ScmPrivilegeType.ALL);
        ScmAuthUtils.checkPriority(site, NAME, PASSWORD, roles.get(0), wsp.getName());
        ScmSession ss = null;
        try {
            ss = TestScmTools.createSession(site, NAME, PASSWORD);
            ScmFactory.Role.grantPrivilege(ss, roles.get(1), resource, ScmPrivilegeType.ALL);
            Assert.fail("expect failed but actual succ.");
        } catch (ScmException e) {
            logger.info("grantPrivilege but the user is not ADMIN_AUTH(role) or ws_ALL(privilege), errorMsg = ["
                    + e.getError() + "]");
        } finally {
            if (null != ss)
                ss.close();
        ...
        ScmCursor<ScmPrivilege> cursor = ScmFactory.Privilege.listPrivileges(session, role);
