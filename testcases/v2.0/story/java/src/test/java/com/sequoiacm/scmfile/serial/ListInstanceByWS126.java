...
public class ListInstanceByWS126 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			...
			for (int i = 0; i < fileNum; i++) {
				ScmId fileId = ScmFileUtils.create(ws, author+"_"+i, filePath);
	...
	private void testListInstanceByWS() {
		...
		try {
		    ...
		    condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author+"_"+0).get();
	...
	private void cleanFile() throws ScmException {
		for(int i = 0; i < fileNum; i++){
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author+"_"+i).get();
		preCount = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, condition);
		if (0 != preCount) {
