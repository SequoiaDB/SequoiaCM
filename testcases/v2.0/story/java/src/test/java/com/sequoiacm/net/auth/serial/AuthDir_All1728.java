...
public class AuthDir_All1728 extends TestScmBase {
    ...
    private void testCreateDir() {
        ...
        try {
            ScmDirectory expdir = ScmFactory.Directory.createInstance(wsALL, subpath);
            ScmDirectory actdir = ScmFactory.Directory.getInstance(wsALL, subpath);
    ...
    private void testCreateFileInDir() throws ScmException {
        ...
        try {
            // create dir
            ScmDirectory actdir = ScmFactory.Directory.createInstance(wsALL, subpath);
            ...
            // check
            ScmDirectory dir = ScmFactory.Directory.getInstance(wsALL, subpath);
    ...
    private void testAsynTransferByVersion() throws ScmException {
        ...
        try {
            session = TestScmTools.createSession(branchsite);
            ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(wsp.getName(), session); // get
            // dir
            dir = ScmFactory.Directory.getInstance(ws, dirPath);
            ScmFile expfile = ScmFactory.File.createInstance(ws);
            expfile.setFileName(fileName);
    ...
    private void testListDir() {
        ...
        try {
            cursor = ScmFactory.Directory.listInstance(wsALL, new BasicBSONObject());
    ...
    private void testListSche() {
        ...
        try {
            cursor = ScmSystem.Schedule.list(sessionALL, new BasicBSONObject());
            Assert.assertNotNull(cursor);
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
    ...
    private void testMoveDir() throws ScmException {
        ...
        ScmId fileId = null;
        try {
            ScmDirectory srcDir = ScmFactory.Directory.createInstance(wsALL, srcpath);
            ScmDirectory dstDir = ScmFactory.Directory.getInstance(wsALL, dstpath);
            srcDir.move(dstDir);
            ScmDirectory actdir = ScmFactory.Directory.getInstance(wsALL, newpath);
    ...
    private void testMoveFile() throws ScmException {
        ...
        ScmId fileId = null;
        try {
            ScmDirectory dir1 = ScmFactory.Directory.getInstance(wsALL, oldpath);
            ...
            // move file
            ScmFile file = ScmFactory.File.getInstanceByPath(wsALL, oldpath + "/" + fileName);
            ScmDirectory newDir = ScmFactory.Directory.getInstance(wsALL, newpath);
            file.setDirectory(newDir);

            // check
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsALL, newpath + "/" + fileName);
            Assert.assertEquals(actfile.getFileName(), expfile.getFileName());
            Assert.assertEquals(actfile.getDirectory().getPath(), newpath + "/");
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
    ...
    private void testRenameFile() throws ScmException {
        ...
        try {
            ...
            // rename file
            ScmFile file1 = ScmFactory.File.getInstanceByPath(wsALL, path + "/" + fileName);
            file1.setFileName(newFileName);

            // check
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsALL, path + "/" + newFileName);
    ...
    private void tearDown() {
        try {
            ScmFactory.Role.revokePrivilege(sessionA, role, rs, ScmPrivilegeType.ALL);
            ScmFactory.Role.revokePrivilege(sessionA, role, rs, ScmPrivilegeType.ALL);
    ...
    private void prepare() throws Exception {
        try {
            ...
            rs1 = ScmResourceFactory.createDirectoryResource(wsp.getName(), path1);

            createDir(wsA, path);
            createDir(wsA, path1);
            grantPriAndAttachRole(sessionA, rs, user, role, ScmPrivilegeType.ALL);
            grantPriAndAttachRole(sessionA, rs1, user, role, ScmPrivilegeType.ALL);

            ScmAuthUtils.checkPriority(rootsite, username, passwd, role, wsp);

            sessionALL = TestScmTools.createSession(branchsite, username, passwd);
            wsALL = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionALL);
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        }
