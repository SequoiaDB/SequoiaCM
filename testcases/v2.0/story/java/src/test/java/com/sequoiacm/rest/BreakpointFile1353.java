...
public class BreakpointFile1353 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		BreakpointUtil.createBreakpointFile(ws, filePath, fileName+"_1", 1352*512, ScmChecksumType.CRC32);
		BreakpointUtil.createBreakpointFile(ws, filePath, fileName+"_2", 1353*1024, ScmChecksumType.CRC32);
		BreakpointUtil.createBreakpointFile(ws, filePath, fileName+"_3", 1353*2048, ScmChecksumType.CRC32);
		
		//查询upload_size等于于1353*1024的断点文件
		this.checkBreakpointFileByFilter(ws.getName(), "{'upload_size': 1385472}");
	...
	private void checkBreakpointFileByFilter(String wsName,String filter) throws Exception{
		RestWrapper rest = new RestWrapper();
		rest.connect(site.getSiteServiceName(), TestScmBase.scmUserName,TestScmBase.scmPassword);
		String response = rest.setRequestMethod(HttpMethod.GET)
			.setApi("/breakpointfiles?workspace_name="+wsName+"&&filter={uri}")
			.setUriVariables(new Object[]{filter})
			.setResponseType(String.class)
			.exec().getBody().toString();
		ArrayList<BSONObject> objList = (ArrayList<BSONObject>) JSON.parse(response);
		rest.disconnect();
