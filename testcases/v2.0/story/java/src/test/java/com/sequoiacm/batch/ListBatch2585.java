...
public class ListBatch2585 extends TestScmBase {
	...
	public Object[][] generateRangData() throws Exception {
		BSONObject orderby = ScmQueryBuilder.start(ScmAttributeName.Batch.TAGS+"-"+"inexistences").is(1).get();
		return new Object[][]{
				...
				//skip > fileList.size
				{filter,null, 1, 10,new ArrayList<ScmFileBasicInfo>()},
				//limit > fileList.size
	...
	private void test(BSONObject filter,BSONObject orderby, long skip, long limit, List<ScmBatchInfo> list) throws Exception {
		int actPageSize = 0;
		long tmpSkip = skip;
		int totalNum = 0;
			...
			while (tmpSkip < list.size()) {
				...
				while (cursor.hasNext()) {
					...
					try {
						...
						Assert.assertEquals(act.getName().contains(batchNamePrefix),true);
						count++;
