...
public class ElemMatch538 extends TestScmBase {
	...
	private void testQuery1() throws Exception {
		try {
			...
			BSONObject value = ScmQueryBuilder.start(siteIdKey).in(site.getSiteId()).and(lastTimeKey)
					.greaterThanEquals(0).and(lastTimeKey).lessThan(Long.MAX_VALUE).get();

			ScmFile file = ScmFactory.File.getInstance(ws, fileIdList.get(1));
			BSONObject cond = ScmQueryBuilder.start(key).elemMatch(value).and(fileNameKey).is(file.getAuthor()).get();
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""),
					("{ \"site_list\" : { \"$elemMatch\" : { " + "\"site_id\" : { \"$in\" : [ " + site.getSiteId()
							+ "]} , " + "\"last_access_time\" : { \"$gte\" : 0 , \"$lt\" : 9223372036854775807}}} , "
	...
	private void testQuery2() throws Exception {
		try {
			...
			BSONObject siteIdIsOk = ScmQueryBuilder.start(siteIdKey).notIn(notExistSiteId).get();
			BSONObject lastTimeIsOk = ScmQueryBuilder.start(lastTimeKey).greaterThan(0).and(lastTimeKey)
					.lessThanEquals(Long.MAX_VALUE).get();
			BSONObject value = ScmQueryBuilder.start().or(siteIdIsOk, lastTimeIsOk).get();

			ScmFile file = ScmFactory.File.getInstance(ws, fileIdList.get(1));
			BSONObject cond = ScmQueryBuilder.start(key).elemMatch(value).and(fileNameKey).is(file.getAuthor()).get();

			Assert.assertEquals(cond.toString().replaceAll("\\s*",""),
					("{ \"site_list\" : { \"$elemMatch\" : { \"$or\" : [ { " + "\"site_id\" : { \"$nin\" : [ "
							+ notExistSiteId + "]}} , { "
