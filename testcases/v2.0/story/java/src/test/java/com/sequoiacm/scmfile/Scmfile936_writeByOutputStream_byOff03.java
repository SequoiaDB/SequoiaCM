...
public class Scmfile936_writeByOutputStream_byOff03 extends TestScmBase {
	 ...
	 */
	@Test(groups = { "oneSite", "twoSite", "fourSite" })
	private void testOffLtFileSize02() throws IOException {
		int off = fileSize;
		int len = 1;
		try {
			this.writeScmFile(off, len);
			Assert.fail("expect fail, actual success, when off = fileSize and len > 0");
	 ...
	 */
	@Test(groups = { "oneSite", "twoSite", "fourSite" })
	private void testOffLtFileSize04() throws IOException {
		int off = fileSize + 1;
		int len = 1;
		try {
			this.writeScmFile(off, len);
			Assert.fail("expect fail, actual success, when off > fileSize and len = 1");
		} catch (ScmException e) {
			if (e.getError() != ScmError.INVALID_ARGUMENT) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
	...
	private ScmId writeScmFile(int off, int len) throws IOException, ScmException {
		ScmId fileId = null;
	...
	private void readAndCheckResults(ScmId fileId, int size, int len, int expScmfileSize) throws Exception {
		// read scmfile
		...
		Assert.assertEquals(new File(downloadPath).length(), expScmfileSize);

		// read content
		String downloadPath2 = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
				Thread.currentThread().getId());
		TestTools.LocalFile.readFile(filePath, size, len, downloadPath2);

		Assert.assertEquals(TestTools.getMD5(downloadPath), TestTools.getMD5(downloadPath2));
		Assert.assertNotEquals(TestTools.getMD5(downloadPath), TestTools.getMD5(filePath));
	}
