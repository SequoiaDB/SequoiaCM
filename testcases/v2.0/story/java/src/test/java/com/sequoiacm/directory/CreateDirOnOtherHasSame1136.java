...
public class CreateDirOnOtherHasSame1136 extends TestScmBase{
	...
	private void setUp() {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(subDirBase).get();
			ScmFileUtils.cleanFile(wsp, cond);
			deleteDir(ws,dirBasePath);
			
			ScmFactory.Directory.createInstance(ws, dirBasePath);
			subDir1 = ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + subDirBase + "_1");
			subDir2 = ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + subDirBase + "_2");
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void createEle(ScmDirectory subDir){
		try {
			...
			ScmFactory.Directory.createInstance(ws, subDir.getPath() + subDirBase + "_1_dir" );
