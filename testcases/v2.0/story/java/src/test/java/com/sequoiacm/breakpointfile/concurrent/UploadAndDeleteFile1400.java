...
/**
* test content:upload and delete the same breakpoint file concurrently
* testlink case:seqDB-1400
* @author wuyan
    * @Date    2018.05.22
* @version 1.00
*/
	...
	private void test() throws Exception {		
		...
		if( uploadBreakpointFile.isSuccess()){
			if( !deleteBreakpointFile.isSuccess()){
				...
				Assert.assertEquals(ScmError.INVALID_ARGUMENT,e.getError(),"delete must fail:"+deleteBreakpointFile.getErrorMsg());
				checkFileData();
			}else{
			...
			Assert.assertTrue( deleteBreakpointFile.isSuccess(), deleteBreakpointFile.getErrorMsg());
			ScmException e =(ScmException) uploadBreakpointFile.getExceptions().get(0);
			
			if( ScmError.INVALID_ARGUMENT != e.getError() &&  ScmError.FILE_NOT_FOUND != e.getError()
					&&  ScmError.HTTP_NOT_FOUND != e.getError()){
				Assert.assertFalse(true, "upload should fail e:"+e.getErrorCode()+":"+uploadBreakpointFile.getErrorMsg());
			}			
			checkDeleteResult();
		}		
	}
