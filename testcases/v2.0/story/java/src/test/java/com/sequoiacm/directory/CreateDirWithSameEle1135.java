...
public class CreateDirWithSameEle1135 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME).is(eleName).get();
	...
	private void createDirWithSameDir() throws ScmException {
		...
		try {
			dir = ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName);
			ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName);
			Assert.fail("dir alreay existed,it should not create same dir successfully");
	...
	private void createDirWithSameFile() throws ScmException {
		...
		try {
			...
			fileId = file.save();
			ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName);
			Assert.fail("parent dir alreay have a file with same name,it should not create dir successfully");
		} catch (ScmException e) {
	...
	private void createDirWithDiff() throws ScmException {
		...
		try {
			...
			file.setDirectory(scmDir);
			fileId = file.save();
			dir = ScmFactory.Directory.createInstance(ws, dirBasePath + "/" + eleName1);
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		} finally {
