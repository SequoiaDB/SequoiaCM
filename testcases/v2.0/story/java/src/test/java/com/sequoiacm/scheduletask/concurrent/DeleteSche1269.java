...
/**
 * @Testcase: SCM-1269:并发删除同一调度任务
 * @author huangxiaoni init
 * @date 2018.4.24
 */

public class DeleteSche1269 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(DeleteSche1269.class);
	...
	private final static String name = "schetask1269";
	...
	private void setUp() throws Exception {
		...
		//ScmScheduleUtils.sleepStrategy(ssA, wsA, scheduleId, fileIds.get(fileNum - 1), fileNum);
	}
	...
	private void test() throws Exception {		
		...
		delSche.start(20);
		if (!(delSche.isSuccess())) {
			Assert.fail(delSche.getErrorMsg());
		}
	...
	private void tearDown() throws Exception {
		try {
			if (!runSuccess) {
				...
				ScmScheduleUtils.outputScmfileInfo(wsA, fileIds.subList(0, fileNum));
			}
			if (null != scheduleId) {
				try {
