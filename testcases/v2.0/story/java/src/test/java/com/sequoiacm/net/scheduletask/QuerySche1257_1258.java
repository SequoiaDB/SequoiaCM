...
public class QuerySche1257_1258 extends TestScmBase {
	...
	private void setUp() throws ScmException {
		...
		System.out.println("--------------------bansite = " + branSite.toString());
	...
	private void test_coverAllTaskAttr() throws Exception {
		ScmCursor<ScmScheduleBasicInfo> cursor = null;
		try {
			BSONObject queryCond2 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).in(name).get();
			ScmScheduleCopyFileContent content2 = new ScmScheduleCopyFileContent(
					branSite.getSiteName(), rootSite.getSiteName(), "1d", queryCond2);
			
			BSONObject sourceSite = ScmQueryBuilder.start("source_site").is(branSite.getSiteName()).get();
			BSONObject targetSite = ScmQueryBuilder.start("target_site").is(rootSite.getSiteName()).get();
			BSONObject mstObj = ScmQueryBuilder.start("max_stay_time").is("1d").get();
			BSONObject type2 = ScmQueryBuilder.start("type").is("clean_file").get();
			BSONObject ctObj = ScmQueryBuilder.start("content").is(content2.toBSONObject()).get();
...
//			System.out.println(cond);
	...
	private void test_returnEmpty() throws Exception {
		...
		ScmCursor<ScmScheduleBasicInfo> cursor = ScmSystem.Schedule.list(ssA, cond);
	...
	private void createSchedules() throws ScmException {
		...
		BSONObject queryCond2 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).in(name).get();
		ScmScheduleCopyFileContent content2 = new ScmScheduleCopyFileContent(
				branSite.getSiteName(), rootSite.getSiteName(), maxStayTime2, queryCond2);
        ScmSchedule sche = ScmSystem.Schedule.create(ssA, wsp.getName(), 
