...
public class DeleteConf2320 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        try {
            ...
            List<ScmServiceInstance> scheList = ScmSystem.ServiceCenter.getServiceInstanceList(session, "schedule-server");
            List<ScmServiceInstance> authList = ScmSystem.ServiceCenter.getServiceInstanceList(session,"auth-server");

            Assert.assertTrue(actResult.getFailures().size() == 0, actResult.getFailures().toString());
            Assert.assertTrue(actResult.getSuccesses().size() == (scheList.size()+authList.size()), actResult.getSuccesses().toString());
            ...
            for(ScmServiceInstance instance : scheList){
                ConfUtil.checkDeletedConf(instance.getIp()+":" + instance.getPort(),expKeys);
            }
