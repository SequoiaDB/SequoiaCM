...
/**
 * @Description:  SCM-2583:正序分页列取批次列表
 * @author fanyu
 * @Date:2019年8月28日
 * @version:1.0
 */
public class ListBatch2583 extends TestScmBase {
	...
	private String batchNamePrefix = "batch2583";
	private String tag = "tag2583";
	...
	private void setUp() throws Exception {
		...
		for(int i = 0; i < batchNum; i++) {
			...
			scmBatch.addTag(tag);
			scmBatch.setName(batchNamePrefix + "-" + i);
			batchIdList.add(scmBatch.save());
		}
		filter  = ScmQueryBuilder.start(ScmAttributeName.Batch.TAGS).is(tag).get();
	...
	public Object[][] generateRangData() throws Exception {
		BSONObject positive = ScmQueryBuilder.start(ScmAttributeName.Batch.NAME).is(1).get();
		BSONObject dPositive = ScmQueryBuilder.start(ScmAttributeName.Batch.CREATE_TIME).is(1)
		...
		return new Object[][]{
				...
				//skip>10 limt=50
				{filter,positive, 10, 100,batchList,sortNameAtr1,typeAtr1},
				//skip == fileList.size
				{filter,positive, batchList.size(),10, batchList,sortNameAtr1,typeAtr1},
				//skip > fileList.size
				{filter,positive, 1, 10,new ArrayList<ScmFileBasicInfo>(),sortNameAtr1,typeAtr1},
				//limit > fileList.size
				{filter,positive, 0,batchList.size() + 1,batchList,sortNameAtr1,typeAtr1},
				//limit = -1
				{filter,positive, 10, -1,batchList,sortNameAtr1,typeAtr1},
				//orderby:多个字段 正序
				{filter,dPositive, 0, 10, batchList,sortNameAtr2,typeAtr2},
				{filter,dPositive, 2, 20, batchList,sortNameAtr2,typeAtr2}
		};
	}

	@Test(dataProvider = "dataProvider")
	private void test(BSONObject filter,BSONObject orderby, long skip, long limit, List<ScmBatchInfo> list
		...
		try {
			...
			while (tmpSkip < tmpList.size()) {
				ScmCursor<ScmBatchInfo> cursor = ScmFactory.Batch.listInstance(ws,filter, orderby,tmpSkip,limit);
				int count = 0;
				while (cursor.hasNext()) {
					ScmBatchInfo act = cursor.getNext();
					ScmBatchInfo exp = tmpList.get((int) (tmpSkip + count));
					try {
						Assert.assertEquals(act.getCreateTime(), exp.getCreateTime());
						Assert.assertEquals(act.getId(), exp.getId());
						Assert.assertEquals(act.getFilesCount(), exp.getFilesCount());
