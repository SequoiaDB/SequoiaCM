...
public class Sche_UpdateScopeByCleanTask1805a extends TestScmBase {	
	...
	private void test() throws Exception {			
		...
		VersionUtils.checkScheTaskFileSites(wsA, fileIdList,currentVersion,expCurSiteList1);
		SiteWrapper[] expHisSiteList1 = {  rootSite, branSite };
		VersionUtils.checkScheTaskFileSites(wsM, fileIdList,historyVersion,expHisSiteList1);
		
		//update task to clean history version file		
		updateScheTaskToHisVersionFile();
		//check siteinfo
		SiteWrapper[] expHisSiteList2 = {  rootSite };
		VersionUtils.checkScheTaskFileSites(wsM, fileIdList,historyVersion,expHisSiteList2);
		//write current version file again at the branSite
		readFile(wsA, currentVersion);
		//check siteinfo
		SiteWrapper[] expCurSiteList2 = { rootSite, branSite };
		VersionUtils.checkScheTaskFileSites(wsA, fileIdList,currentVersion,expCurSiteList2);
		runSuccess = true;
	}
	...
	private void createScheduleTask(ScmSession session) throws ScmException {
		...
		content = new ScmScheduleCleanFileContent(branSite.getSiteName(), maxStayTime, condition, ScopeType.SCOPE_CURRENT);
		cron = "* * * * * ?";
