...
/**
 * @FileName SCM-348: lessThan多个相同字段
 * @Author linsuqiang
 * @Date 2017-05-25
 * @Version 1.00
 */
...
/*
 * 1、countInstance带查询条件查询文件，对多个相同字段做lessThan匹配查询； 2、检查ScmQueryBuilder结果正确性；
 * 3、检查查询结果正确性；
 */

public class LessThan348 extends TestScmBase {
	...
	private static String fileName = "LessThan348";
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).lessThan(111).put(key).lessThan(value)
					.put(ScmAttributeName.File.AUTHOR).is(file.getAuthor()).get();

			String expCond = "{ \"" + key + "\" : { \"$lt\" : " + value + "} , \"author\" : \"" + author + "\"}";
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).lessThan(value).put("k2").lessThan(" ").get();

			String expCond = "{ \"" + key + "\" : { \"$lt\" : " + value + "} , " + "\"k2\" : { \"$lt\" : \" \"}}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));
