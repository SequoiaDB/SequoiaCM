...
public class LessThan348 extends TestScmBase {
    ...
    private void testQueryByExistCond() throws Exception {
        try {
            ...
            BSONObject cond = ScmQueryBuilder.start( key ).lessThan( 111 )
                    .put( key ).lessThan( value )
                    .put( ScmAttributeName.File.AUTHOR ).is( file.getAuthor() )
                    .get();

            String expCond = "{ \"" + key + "\" : { \"$lt\" : " + value
                    + "} , \"author\" : \"" + author + "\"}";
    ...
    private void testQueryByNotExistCond() throws Exception {
        try {
            ...
            BSONObject cond = ScmQueryBuilder.start( key ).lessThan( value )
                    .put( "k2" ).lessThan( " " ).get();
