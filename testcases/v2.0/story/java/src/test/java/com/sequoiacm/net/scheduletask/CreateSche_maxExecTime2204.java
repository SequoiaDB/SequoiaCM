...
public class CreateSche_maxExecTime2204 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			...
			//checkTask info
			BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Schedule.ID).is(scheduleId.get()).and("max_exec_time").is(maxExecTime).get();
			...
			ScmScheduleUtils.checkScmFile(wsA, fileIds, expSites);
			//checkTask info
			BSONObject cond1 = ScmQueryBuilder.start(ScmAttributeName.Schedule.ID).is(scheduleId.get()).and("max_exec_time").is(maxExecTime).get();
			...
			ScmScheduleUtils.checkScmFile(wsA, fileIds, expSites);
			//checkTask info
			BSONObject cond2 = ScmQueryBuilder.start(ScmAttributeName.Schedule.ID).is(scheduleId.get()).and("max_exec_time").is(maxExecTime).get();
	...
	private void createScheduleTask(long maxExecTime) {
		...
		try {
			...
			content = new ScmScheduleCopyFileContent(branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond,ScopeType.SCOPE_CURRENT, maxExecTime);
            cron = "* * * * * ?";
			//cron = "0 0/3 * * * ?";
	...
	private void updateScheMaxExecTime(long maxExecTime) {
		...
		try {
			...
			ScmScheduleCopyFileContent content = new ScmScheduleCopyFileContent(
					branSite.getSiteName(), rootSite.getSiteName(), maxStayTime, queryCond,ScopeType.SCOPE_CURRENT,maxExecTime);
	...
	private void checkTaskInfo(BSONObject cond,long maxExecTime) throws ScmException {
