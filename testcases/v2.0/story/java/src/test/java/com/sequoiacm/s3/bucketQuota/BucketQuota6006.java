...
/**
 * @description SCM-6006:复制对象上传后检查桶容量限额
 * @author ZhangYanan
...
public class BucketQuota6006 extends TestScmBase {
    private boolean runSuccess = false;
    private String keyName = "object6006";
    private String bucketName1 = "bucket6006a";
    private String bucketName2 = "bucket6006b";
    ...
    private void setUp() throws Exception {
        ...
        ScmEnableBucketQuotaConfig quotaConfig = ScmEnableBucketQuotaConfig
                .newBuilder( bucketName1 ).setMaxObjects( maxObjectNum )
    ...
    public void test1() throws ScmException {
        preHighWaterLevel();
        s3Client.putObject( bucketName2, keyName, new File( filePath3 ) );
        try {
        ...
        ScmBucketQuotaInfo bucketQuotaInfo = ScmFactory.Quota
                .getBucketQuota( session, bucketName1 );
        BucketQuotaUtils.checkQuotaInfo( bucketQuotaInfo, bucketName1,
                maxObjectNum, maxObjectSize * fileSize1, objectNum - 1,
                ( objectNum - 1 ) * fileSize1 );
        S3Utils.deleteAllObjects( s3Client, bucketName2 );

        s3Client.putObject( bucketName2, keyName, new File( filePath2 ) );
        s3Client.copyObject( bucketName2, keyName, bucketName1, keyName );
        ...
        S3Utils.deleteAllObjects( s3Client, bucketName1 );
    }
    ...
    public void preHighWaterLevel() {
        for ( int i = 0; i < objectNum - 1; i++ ) {
            s3Client.putObject( bucketName1, keyName + i,
                    new File( filePath1 ) );
        }
    }
