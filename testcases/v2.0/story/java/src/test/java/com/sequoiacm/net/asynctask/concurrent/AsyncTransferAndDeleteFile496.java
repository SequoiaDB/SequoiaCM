...
public class AsyncTransferAndDeleteFile496 extends TestScmBase {
    ...
    private void setUp() {
        ...
        try {
            ...
            ws_T = ScmInfo.getWs();
            branceSite= ScmNetUtils.getNonLastSite(ws_T);
    ...
    private void test() throws Exception {
        ...
        for (int i = 0; i < fileNum; i++) {
            AsyncTransferFromSubCenterA asyncTransferAT = new AsyncTransferFromSubCenterA(i);
        ...
        for (int i = 0; i < fileNum; i++) {
            ...
            if (!(asyncTransferAT.isSuccess() && deleteFileAT.isSuccess())) {
                Assert.fail(asyncTransferAT.getErrorMsg() + deleteFileAT.getErrorMsg());
            }
        }
    ...
    private void writeFileFromSubCenterA(ScmWorkspace ws, String filePath) throws ScmException {
        for (int i = 0; i < fileNum; i++) {
