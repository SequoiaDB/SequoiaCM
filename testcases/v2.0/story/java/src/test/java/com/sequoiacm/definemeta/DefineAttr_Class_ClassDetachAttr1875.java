...
public class DefineAttr_Class_ClassDetachAttr1875 extends TestScmBase {
	...
	try {
	    ScmClass class2 = ScmFactory.Class.getInstance(wsu, class1.getId());
	...
	Assert.assertEquals(class3.listAttrs().size(), 1, class3.listAttrs().toString());
	Assert.assertEquals(class3.listAttrs().get(0).getId(), attr.getId(), class3.listAttrs().get(0).toString());
	runSuccess = true;
    }
	...
	user = ScmFactory.User.createUser(session, name, ScmUserPasswordType.LOCAL, passwd);
