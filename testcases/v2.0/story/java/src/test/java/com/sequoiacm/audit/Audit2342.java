...
public class Audit2342 extends TestScmBase {
    ...
    private void test() throws ScmException {
        ...
        confMap.put(ConfigCommonDefind.scm_audit_userType+ScmUserPasswordType.LOCAL.name(), "META_ATTR_DQL");
    ...
    private void checkAudit(String username,String password) throws ScmException {
        ...
        try {
            ...
            if(username.equals(TestScmBase.scmUserName)) {
                Assert.assertEquals(ConfUtil.checkAudit(session,bson1, attrName), false, "Has the configuration been updated? attrId = " + attrId.get());
                Assert.assertEquals(ConfUtil.checkAudit(session,bson2, attrId.get()), true, "Has the configuration been updated? attrId = " + attrId.get());
            }else{
                Assert.assertEquals(ConfUtil.checkAudit(session, bson1, attrName), true, "Has the configuration been updated? ,attrId = " + attrId.get());
                Assert.assertEquals(ConfUtil.checkAudit(session,bson2 , attrId.get()), false, "Has the configuration been updated? ,attrId = " + attrId.get());
            }
        } finally {
            if(attrId != null){
    ...
    private ScmId craeteAndQueryAttr(String username,String password) throws ScmException {
