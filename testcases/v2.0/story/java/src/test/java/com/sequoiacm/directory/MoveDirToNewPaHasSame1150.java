 ...
 * @version:1.0
 */
public class MoveDirToNewPaHasSame1150 extends TestScmBase{
	private boolean runSuccess;
	private ScmSession session;
	private ScmWorkspace ws;
	private SiteWrapper site;
	private WsWrapper wsp;
	private String dirBasePath = "/MoveDirToNewPaHasSame1150";
	private String fullPath1 = dirBasePath + "/a/b/c";
	private String fullPath2 = dirBasePath + "/e/b/f";
	private ScmDirectory dir1;
	private ScmDirectory dir2;
	private String author = "MoveDirToNewPaHasSame1150";
	...
	private void tearDown() throws Exception {
		try {
			if (runSuccess || TestScmBase.forceClear) {
				...
				deleteDir(ws, dir1.getPath());
				deleteDir(ws, dir2.getPath());
			}
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
	...
	private void check(ScmId fileId, ScmDirectory dir, ScmWorkspace ws) {
		...
		try {
			...
			Assert.assertEquals(file.getDirectory().getParentDirectory().getPath(), dir.getParentDirectory().getPath());
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
