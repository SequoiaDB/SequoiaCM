...
public class DeleteWSWriteFile2175 extends TestScmBase {	
	...
	private void test() throws Exception {
		...
		try {
			...
			if(e.getError()!=ScmError.WORKSPACE_NOT_EXIST && e.getError()!=ScmError.OPERATION_UNAUTHORIZED){
				throw e;
			}
		}finally {
			is.close();
	 ...
	 * @throws ScmException 
	 */
	private void checkDeleteWs(String wsName, ScmSession session) throws InterruptedException, ScmException {
		for (int i = 0; i < 300; i++) {
			Thread.sleep(1000);
			try {
