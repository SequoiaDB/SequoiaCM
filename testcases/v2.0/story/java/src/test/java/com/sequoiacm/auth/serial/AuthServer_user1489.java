...
public class AuthServer_user1489 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(AuthServer_user1489.class);	
	...
	private void setUp() throws ScmException {
		...
		ScmUser scmUser = ScmFactory.User.createUser(session, NAME, ScmUserPasswordType.LOCAL, "aa");
	...
	private void test_createUser() throws ScmException, InterruptedException {
		// create user and role
		ScmUser scmUser = ScmFactory.User.createUser(session, TestScmBase.ldapUserName, ScmUserPasswordType.LDAP, "");
		ScmUserModifier modifier = new ScmUserModifier();
		modifier.addRole(NAME);
		ScmFactory.User.alterUser(session, scmUser, modifier);
	...
	private void test_listUser() throws ScmException {
		BSONObject filter = ScmQueryBuilder.start()
				...
				.get();
		ScmCursor<ScmUser> cursor = ScmFactory.User.listUsers(session, filter);
		int userNum = 0;
		while (cursor.hasNext()) {
			userNum++;
			ScmUser user = cursor.getNext();
			System.out.println("user1 = " + user.toString());
			Assert.assertEquals(user.getPasswordType(), ScmUserPasswordType.LDAP);
		}
	...
	private void test_deleteUser() throws ScmException {
		ScmUser user = ScmFactory.User.getUser(session, TestScmBase.ldapUserName);
