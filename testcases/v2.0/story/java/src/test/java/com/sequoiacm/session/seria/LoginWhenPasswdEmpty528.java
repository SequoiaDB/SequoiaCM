...
/**
 * @FileName SCM-528: User表用户存在、密码为空串
 * @Author linsuqiang
 * @Date 2017-06-25
 * @Version 1.00
 */
...
/*
 * 1、SCMSYSTEM.USER表用户存在、密码为空串； 2、登入SCM，检查登入结果；
 */

public class LoginWhenPasswdEmpty528 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			saveAndChangePasswd(userCL, TestScmBase.scmUserName, "");
	...
	private void test() {
		try {
			...
			Assert.fail("login shouldn't succeed when password is empty!");
