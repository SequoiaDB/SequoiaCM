...
public class AuthWs_UserWithNoAll1740 extends TestScmBase {
    ...
    private String[] usernameArr = { "AuthWs_UserWithNoAll1740_0", "AuthWs_UserWithNoAll1740_1",
	    "AuthWs_UserWithNoAll1740_2" };
    private String[] rolenameArr = { "1740_0", "1740_1", "1740_2" };
    private String[] privileges = { ScmPrivilegeDefine.READ, ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ
	    + "|" + ScmPrivilegeDefine.DELETE + "|" + ScmPrivilegeDefine.UPDATE, null };
	...
	try {
	    ...
	    grantPriAndAttachRole(session, rs, user, role, ScmPrivilegeDefine.ALL);
	...
	try {
	    ...
	    role = ScmFactory.Role.createRole(session, rolename, null);
	    ScmResource rs = ScmResourceFactory.createWorkspaceResource(wsp.getName());
	    grantPriAndAttachRole(session, rs, user, role, ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ);
	    Assert.fail("exp fail but act success,user = " + user.toString());
	} catch (ScmException e) {
	    if (e.getError() != ScmError.HTTP_FORBIDDEN) {
		e.printStackTrace();
	...
	ScmRole role = null;
	try {
	    session = TestScmTools.createSession(site, usernameArr[2], passwd);
	    user = ScmFactory.User.createUser(session, username, ScmUserPasswordType.LOCAL, passwd);
	    role = ScmFactory.Role.createRole(session, rolename, null);
	    ScmResource rs = ScmResourceFactory.createWorkspaceResource(wsp.getName());
	    grantPriAndAttachRole(session, rs, user, role, ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ);
	...
	for (int i = 0; i < usernameArr.length; i++) {
		...
		if (i < usernameArr.length - 1) {
			...
			ScmAuthUtils.checkPriority(site, user.getUsername(), passwd, role, wsp);
		} else {
		    ScmUserModifier modifier = new ScmUserModifier();
		    modifier.addRole(role);
		    ScmFactory.User.alterUser(sessionA, user, modifier);
