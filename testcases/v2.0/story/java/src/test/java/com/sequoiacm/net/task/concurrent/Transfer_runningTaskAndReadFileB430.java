...
public class Transfer_runningTaskAndReadFileB430 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(Transfer_runningTaskAndReadFileB430.class);
...
//		Assert.assertEquals(taskInfo.getActualCount(), fileNum);    //unfixed value 
		Assert.assertEquals(taskInfo.getFailCount(), taskInfo.getActualCount() - taskInfo.getSuccessCount());
		Assert.assertEquals(taskInfo.getSuccessCount(), taskInfo.getActualCount() - taskInfo.getFailCount());
	...
	private class StartTaskFromSubCenterA extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT, rootSite.getSiteName());
				// check result
				// task runningFlag
				while (true) {
					ScmTask taskInfo = ScmSystem.Task.getTask(session, taskId);
					if (taskInfo.getStopTime() != null) {
						if (taskInfo.getRunningFlag() == CommonDefine.TaskRunningFlag.SCM_TASK_FINISH) {
							break;
						} else if (taskInfo.getRunningFlag() == CommonDefine.TaskRunningFlag.SCM_TASK_ABORT) {
							logger.error("taskInfo: \n" + taskInfo.toString());
	...
	private void checkMetaAndLobs() {
		try {
			for (int i = 0; i < fileNum; i++) {
				...
				SiteWrapper[] expSiteList = { rootSite, siteList.get(0), siteList.get(1) };
