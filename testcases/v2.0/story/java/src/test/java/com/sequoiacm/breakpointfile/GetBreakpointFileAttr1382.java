...
public class GetBreakpointFileAttr1382 extends TestScmBase {	
	...
	private void getFileAttrInfo() throws Exception {
		...
		Assert.assertEquals(breakpointFile.getChecksumType(), ScmChecksumType.NONE);
		Assert.assertEquals(breakpointFile.getChecksum(), 0);
		Assert.assertEquals(breakpointFile.getCreateUser(), TestScmBase.scmUserName);
		Assert.assertEquals(breakpointFile.getUploadUser(), TestScmBase.scmUserName);
		...
		if (Math.abs(breakpointFile.getCreateTime().getTime() - localTime) > acceptableOffSet) {
			Assert.fail("time is different: createTime=" + breakpointFile.getCreateTime()
					+ ", localTime=" + localTime);
		}
		
		//update time later than create time
		if (breakpointFile.getUploadTime().getTime() < breakpointFile.getCreateTime().getTime()) {
