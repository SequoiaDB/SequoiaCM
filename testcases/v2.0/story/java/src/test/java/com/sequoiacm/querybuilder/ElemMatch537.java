...
public class ElemMatch537 extends TestScmBase {
	...
	private void testInvalidKey() throws Exception {
		try {
			...
			BSONObject value = ScmQueryBuilder.start(ScmAttributeName.File.SITE_ID).is(site.getSiteId()).get();
			BSONObject cond = ScmQueryBuilder.start(key).elemMatch(value).and(ScmAttributeName.File.AUTHOR)
					.is(file.getAuthor()).get();

			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), ("{ \"title\" : { \"$elemMatch\" : { \"site_id\" : " + site.getSiteId()
	...
	private void testInvalidElemKey() throws Exception {
		try {
			...
			BSONObject value = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(site.getSiteId()).get();
			BSONObject cond = ScmQueryBuilder.start(key).elemMatch(value).and(ScmAttributeName.File.AUTHOR)
					.is(file.getAuthor()).get();

			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), ("{ \"site_list\" : { \"$elemMatch\" : { \"author\" : "
					+ site.getSiteId() + "}} , \"author\" : \"" + authorName + "\"}").replaceAll("\\s*",""));
