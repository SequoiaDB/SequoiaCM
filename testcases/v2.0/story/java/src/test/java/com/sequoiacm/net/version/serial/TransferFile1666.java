...
/**
 * test content:Transfer file, specify the fields in transfer condition are not in the history table.
...
public class TransferFile1666 extends TestScmBase {	
	...
	private void startTransferTaskError(ScmWorkspace ws, ScmSession session) throws Exception {
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(authorName).get();	
		
		try {
			ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_HISTORY, targetSite.getSiteName()); 	
		...
		try {
			ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_ALL); 	
	    	Assert.fail("transfer file must bu fail!");
	    } catch (ScmException e) {
			if ( ScmError.INVALID_ARGUMENT != e.getError()) {
				Assert.fail("expErrorCode:-101  actError:"+e.getError()+e.getMessage());
