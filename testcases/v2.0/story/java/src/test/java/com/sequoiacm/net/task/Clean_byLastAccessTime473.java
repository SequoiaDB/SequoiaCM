...
public class Clean_byLastAccessTime473 extends TestScmBase {
	...
	private void startCleanTaskFromA(ScmSession ss) throws Exception {
		...
		BSONObject obj = ScmQueryBuilder.start(ScmAttributeName.File.SITE_ID).is(branceSite.getSiteId())
				.and(ScmAttributeName.File.LAST_ACCESS_TIME).greaterThanEquals(lastAccessTimeList.get(startNum)).get();
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(authorName)
	...
	private void readFileFromM(ScmSession ss) throws Exception {
		...
		for (int i = 0; i < fileNum; i++) {
			...
			for (int j = 0; j < file.getLocationList().size(); j++) {
				if (file.getLocationList().get(j).getSiteId() == branceSite.getSiteId()) {
					long lastAccessTime = file.getLocationList().get(j).getDate().getTime();
	...
	private void checkMetaAndLobs() {
		try {
			...
			ScmFileUtils.checkMetaAndData(ws_T,fileIdList.subList(startNum, fileNum), expSiteList1, localPath,
					filePath);

			SiteWrapper[] expSiteList2 = { rootSite, branceSite };
			ScmFileUtils.checkMetaAndData(ws_T,fileIdList.subList(0, startNum), expSiteList2, localPath, filePath);
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		}
	}
