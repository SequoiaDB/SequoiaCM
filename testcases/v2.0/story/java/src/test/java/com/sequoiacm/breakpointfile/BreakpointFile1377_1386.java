...
public class BreakpointFile1377_1386 extends TestScmBase {
	...
	private void test() throws JSONException, ScmException, IOException {
		...
		BreakpointUtil.createBreakpointFile(ws1, filePath, fileName, 1024*512, ScmChecksumType.CRC32);
		//跨站点上传断点文件
	...
	private void uploadBreakpointFile() throws ScmException, IOException {
		...
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws2, fileName);
		try {
			breakpointFile.upload(inputStream);
			Assert.fail("different site upload breakpointFile should fail");
		} catch (ScmException e) {
			Assert.assertEquals(e.getErrorCode(), ScmError.INVALID_ARGUMENT.getErrorCode(),"different site upload breakpointFile");
		}
		inputStream.close();
	}
	...
	private void checkBreakpointFile() throws ScmException {
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws2, fileName);
		Assert.assertEquals(breakpointFile.isCompleted(), false, "check breakpointFile isCompleted");
		Assert.assertEquals(breakpointFile.getSiteName(), siteList.get(0).getSiteName(), "check breakpointFile site");
		Assert.assertEquals(breakpointFile.getUploadSize(), 1024*512, "check breakpointFile uploadSize");
	}
