...
public class BreakpointFile1384 extends TestScmBase {
	...
	private void setUp() throws IOException, ScmException {
		...
		for(int i=0; i<10; i++){
			BreakpointUtil.createFile(filePath + "_" + i + ".txt", fileSize * i);
		}
	...
	private void test() throws ScmException, IOException {
		...
		BSONObject[] option1 = {new BasicBSONObject("file_name","scmfile1384_1"), new BasicBSONObject("file_name","scmfile1384_5")};
		...
		BSONObject[] option2 = {new BasicBSONObject("upload_size",new BasicBSONObject("$gt",3145728)), new BasicBSONObject("upload_size",new BasicBSONObject("$lt",8388608))};
		filter2.put("$and", option2 );
		String[] fileNameArr2 = {"scmfile1384_4", "scmfile1384_5", "scmfile1384_6", "scmfile1384_7"}; 
		...
		String[] fileNameArr4 = {"scmfile1384_7", "scmfile1384_8", "scmfile1384_9"}; 
		...
		String[] fileNameArr5 = {"scmfile1384_0", "scmfile1384_1", "scmfile1384_2" }; 
		...
		String[] fileNameArr6 = {"scmfile1384_0", "scmfile1384_1", "scmfile1384_2", "scmfile1384_3"}; 
		...
		String[] fileNameArr7 = {"scmfile1384_0","scmfile1384_2","scmfile1384_4","scmfile1384_6","scmfile1384_8"}; 
		listInstanceByOption( filter7, 5 , fileNameArr7);
		
		//带$not匹配符查询
		BSONObject filter8 = new BasicBSONObject();
		BSONObject[] option8 = {new BasicBSONObject("checksum_type","CRC32")};
		filter8.put("$not", option8 );
		String[] fileNameArr8 = {"scmfile1384_1","scmfile1384_3","scmfile1384_5","scmfile1384_7","scmfile1384_9"}; 
	...
	private void tearDown() {
		try {
			for (int i = 0; i < 10; i++) {
				ScmFactory.BreakpointFile.deleteInstance(ws, fileName + "_" + i);
			}
			TestTools.LocalFile.removeFile(localPath);
		} catch (Exception e) {
	...
	private void createBreakpointFile() throws ScmException, IOException{
		for(int i=0; i<10; i++){
			...
			ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.createInstance(ws, fileName + "_" +i, checkType);
			InputStream inputStream = new FileInputStream(filePath + "_" + i + ".txt" );
			breakpointFile.upload( inputStream );
			inputStream.close();
		}
	}
	
	private void listInstanceByOption(BSONObject filter, int fileNum, String[] fileNameArr) throws ScmException {
		System.out.println("filter: "+filter.toString());
		ScmCursor<ScmBreakpointFile> bFileCursor = ScmFactory.BreakpointFile.listInstance(ws, filter);
		...
		Assert.assertEquals(sbFileList.size(), fileNum, "select file number error");
		Assert.assertEquals(actfileNameArr, fileNameArr, "breakpointFile name error");
	}
