 ...
 *               the querey condtion contains fields other than the history version table
...
public class ListInstanceByScope1683 extends TestScmBase {
	...
	private void listInstanceByHistoryVersion() throws ScmException{
		...
		try {
			cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_HISTORY, condition);	
	...
	private void listInstanceByAllVersion() throws ScmException{
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME)
				.is(fileName).get();									
		ScmCursor<ScmFileBasicInfo> cursor = null;
		try {
			cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_ALL, condition);	
