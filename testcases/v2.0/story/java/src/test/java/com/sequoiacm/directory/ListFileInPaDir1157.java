...
public class ListFileInPaDir1157 extends TestScmBase{
	...
	private void setUp() {
		try {
			...
			for (int i = 'a'; i < 'f'; i++) {
				ScmFactory.Directory.createInstance(ws, dirBasePath + "/1157_" + (char)i);
			}
			createFile(ws, ScmFactory.Directory.getInstance(ws, dirBasePath + "/1157_e"));
			createFile(ws, ScmFactory.Directory.getInstance(ws, dirBasePath + "/1157_a"));
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void testCond() {
		...
		try {
			...
			BSONObject cond =  ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR)
					.is(author)
					.and(ScmAttributeName.File.MAJOR_VERSION)
					.is(1)
					.and(ScmAttributeName.File.TITLE)
					.is(author)
					.and(ScmAttributeName.File.CREATE_TIME)
					.greaterThan(0)
					.and(ScmAttributeName.File.UPDATE_TIME)
					.greaterThan(0)
					.get();
			ScmDirectory pdir =  ScmFactory.Directory.getInstance(ws, fullPath1 + "/" + dirName);
			ScmCursor<ScmFileBasicInfo> fileCursor = pdir.listFiles(cond);
			while(fileCursor.hasNext()){
				fileInfo = fileCursor.getNext();
				Assert.assertNotNull(fileInfo.getFileName());
				Assert.assertEquals(fileInfo.getFileId().get(), fileIdList.get(0).get());
				i++;
			}
			Assert.assertTrue(i==expDirNum);
		} catch (ScmException e) {
