    ...
    private void testNull() {
        try {
            ScmConfigProperties.builder()
                    ...
                    .build();
            Assert.fail(" ScmConfigProperties.builder().instance(null) must be failed when the instances is null");
    ...
    private void testHybrid() {
        try {
            ...
            ScmConfigProperties.builder()
                    ...
                    .updateProperty(ConfigCommonDefind.scm_audit_mask, "ALL")
                    .build();
            Assert.fail(" ScmConfigProperties.builder().instance(null)  must be failed when the instances  is 0");
        } catch (ScmException e) {
            if (e.getError() != ScmError.INVALID_ARGUMENT) {
                Assert.fail(e.getMessage());
            }
