...
public class DeleteScmFile445 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			...
			for (int i = 0; i < fileNum; i++) {
				BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME).is(fileName+"_"+i).get();
	...
	private void prepareScmFiles() throws ScmException {
		for (int i = 0; i < fileNum; i++) {
			ScmId fileId = ScmFileUtils.create(wsA, fileName+"_"+i, filePath);
		...
		for (ScmId fileId : fileIdList) {
			...
			for (int j = 0; j < wsArr.length; j++) {
				try {
					...
					ScmFileUtils.checkData(wsArr[j], fileId, localPath, filePath);
