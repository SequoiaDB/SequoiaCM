...
public class Clean_cleanCond474 extends TestScmBase {
	...
	private void setUp() throws Exception{
		...
		try {
			...
			ScmCursor<ScmFileBasicInfo> cur = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_CURRENT, ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(authorName).get());
			while (cur.hasNext()) {
				ScmFactory.File.deleteInstance(ws, cur.getNext().getFileId(), true);
			}
	...
	private void prepareFiles(ScmSession session) throws Exception {
		...
		for (int i = 0; i < fileNum; ++i) {
			...
			scmfile.setCreateTime(new Date());
