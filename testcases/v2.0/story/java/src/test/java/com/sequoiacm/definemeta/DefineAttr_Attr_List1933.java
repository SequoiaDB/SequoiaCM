...
public class DefineAttr_Attr_List1933 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			...
			ScmCursor<ScmAttribute> cursor = ScmFactory.Attribute.listInstance(ws, cond);
			int i = 0;
		...
		matcher.put(ScmAttributeName.Attribute.NAME + "1", submatcher.toString());
		try {
			ScmCursor<ScmAttribute> cursor = ScmFactory.Attribute.listInstance(ws, matcher);
			while (cursor.hasNext()) {
	...
	private BSONObject createCond() throws ScmException {
		BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Attribute.DESCRIPTION).exists(1)
				.and(ScmAttributeName.Attribute.CREATE_USER).greaterThanEquals(TestScmBase.scmUserName)
				.and(ScmAttributeName.Attribute.UPDATE_USER).lessThanEquals(TestScmBase.scmUserName)
				.and(ScmAttributeName.Attribute.CREATE_TIME).greaterThan(0).and(ScmAttributeName.Attribute.CREATE_TIME)
				.lessThan(Long.MAX_VALUE).and(ScmAttributeName.Attribute.NAME).notEquals(Integer.MAX_VALUE)
				.not(ScmQueryBuilder.start(ScmAttributeName.Attribute.DESCRIPTION).is(desc + "_1").get())
				.notIn(ScmQueryBuilder.start(ScmAttributeName.Attribute.DESCRIPTION).is(desc + "_1").get())
				.or(ScmQueryBuilder.start(ScmAttributeName.Attribute.DESCRIPTION).in(desc).get()).get();
		return cond;
	}
}
