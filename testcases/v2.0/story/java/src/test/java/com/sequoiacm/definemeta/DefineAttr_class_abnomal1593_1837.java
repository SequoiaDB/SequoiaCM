...
public class DefineAttr_class_abnomal1593_1837 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(DefineAttr_class_abnomal1593_1837.class);
	...
	private void test_classNotExist() throws Exception {	
		...
		ScmClassProperties properties = new ScmClassProperties("notExistClassId");
		try {
			file.setClassProperties(properties);
			Assert.fail("expect failed but actual succ.");
		} catch (ScmException e) {
			logger.info("classId not exist, errorMsg = [" + e.getError() + "]");
		} 
		runSuccess = true;
	}
