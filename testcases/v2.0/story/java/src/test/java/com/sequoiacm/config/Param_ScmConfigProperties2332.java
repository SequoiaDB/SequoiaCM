...
public class Param_ScmConfigProperties2332 extends TestScmBase {
    ...
    private void test() throws ScmException {
        ...
        Assert.assertEquals( confProp.getTargets().get( 0 ),
                site.getSiteServiceName() );
        Assert.assertEquals( confProp.isAcceptUnknownProps(), true );
        Assert.assertEquals( confProp.getDeleteProps().get( 0 ),
                ConfigCommonDefind.scm_audit_mask );
        Assert.assertEquals( confProp.getDeleteProps().get( 1 ),
                ConfigCommonDefind.scm_audit_userMask );
