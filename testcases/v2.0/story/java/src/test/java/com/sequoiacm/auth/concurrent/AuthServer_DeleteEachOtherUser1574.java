...
public class AuthServer_DeleteEachOtherUser1574 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			...
			for (int i = 0; i < userNum; i++) {
				ScmUser user = ScmFactory.User.createUser(session, username + "_" + i, ScmUserPasswordType.LOCAL,
						passwd);
				ScmUser user1 = ScmFactory.User.alterUser(session, user, modifier);
				userList.add(user1);
			}
		} catch (ScmException e) {
			e.printStackTrace();
	...
	private void test() {
		DeleteEachOther dThread1 = new DeleteEachOther(userList.get(0), userList.get(1));
		DeleteEachOther dThread2 = new DeleteEachOther(userList.get(1), userList.get(2));
		DeleteEachOther dThread3 = new DeleteEachOther(userList.get(2), userList.get(0));
		dThread1.start();
		dThread2.start();
		dThread3.start();
	...
	private class DeleteEachOther extends TestThreadBase {
		...
		public void exec() {
			...
			try {
				session = TestScmTools.createSession(site, this.srcUser.getUsername(), passwd);
				ScmFactory.User.deleteUser(session, dstUser.getUsername());
				check(dstUser);
			} catch (ScmException e) {
				if (e.getError() != ScmError.HTTP_BAD_REQUEST && e.getError() != ScmError.HTTP_INTERNAL_SERVER_ERROR
						&& e.getError() != ScmError.HTTP_UNAUTHORIZED && e.getError() != ScmError.HTTP_NOT_FOUND) {
					e.printStackTrace();
					Assert.fail(e.getMessage());
				}
			} finally {
