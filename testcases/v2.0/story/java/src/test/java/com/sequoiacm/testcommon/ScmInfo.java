...
public class ScmInfo {
	...
	public static List<SiteWrapper> getBranchSites(int num) {
		...
		for (int i = 1; i < num; i++) {
			...
			if (addNum < maxBranchSiteNum) {
				...
				branchSites.add(allBranchSites.get(addNum - maxBranchSiteNum));
			}
		}
	 ...
	 */
	public static List<ScmSiteInfo> getSiteList(ScmSession session) throws ScmException {
	 ...
	 *         branchSiteInfo2, ......]
	 */
	private static List<ScmSiteInfo> sortSiteList(List<ScmSiteInfo> siteInfoList) {
	...
	public static List<NodeWrapper> getNodeList() {
		...
		try {
			...
			cursor = clDB.query(null, "{\"_id\" : {\"$include\" : 0} }", "{\"site_id\" : 1, \"port\" : 1}", null);
			while (cursor.hasNext()) {
	 ...
	 */
	public static List<WsWrapper> getWsList(ScmSession session) throws ScmException {
	...
	/**
	 * get all serviceInstance by servicename if there is a new ws to use this function, otherwise,
	 * recommended to use getScheduleServers()
	 */
	public static List<ScmServiceInstance> getServiceInstances(ScmSession session,String serviceName) throws ScmException {
		return ScmSystem.ServiceCenter.getServiceInstanceList(session,serviceName);
	}
}
