...
public class Is378 extends TestScmBase {
	...
	private void testQueryByExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).is(111).put(key).is(value).get();
	...
	private void testQueryByNotExistCond() throws Exception {
		try {
			...
			BSONObject cond = ScmQueryBuilder.start(key).is(value).put("k2").is(" ").get();

			String expCond = "{ \"" + key + "\" : " + value + " , " + "\"k2\" : \" \"}";
			Assert.assertEquals(cond.toString().replaceAll("\\s*",""), expCond.replaceAll("\\s*",""));
	...
	private void readyScmFile() {
		try {
			for (int i = 0; i < fileNum; i++) {
				...
				scmfile.setFileName(fileName + "_" + TestTools.getRandomString(10) + i);
