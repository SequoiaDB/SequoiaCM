...
public class AuthWsDir_Read1730 extends TestScmBase {
    ...
    private void testGetDirByPath() {
        String dirpath = path;
        try {
            ScmDirectory dir = ScmFactory.Directory.getInstance(wsCRUD, dirpath);
    ...
    private void testGetFileMetaUnderDir() throws ScmException {
        ...
        try {
            ...
            // get file
            ScmDirectory actdir = ScmFactory.Directory.getInstance(wsCRUD, dirpath);
    ...
    private void testReadFileByPath() throws Exception {
        ...
        try {
            ...
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsCRUD, dirpath + "/" + fileName);
            String downloadPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
                    Thread.currentThread().getId());
            actfile.getContent(downloadPath);
            Assert.assertEquals(actfile.getFileName(), fileName);
    ...
    private void testReadFileByIdVersion() throws Exception {
        ...
        try {
            ...
            ScmFile actfile = ScmFactory.File.getInstance(wsCRUD, fileId, 1, 0);
            String downloadPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
                    Thread.currentThread().getId());
            actfile.getContent(downloadPath);
            Assert.assertEquals(actfile.getFileName(), fileName);
    ...
    private void testReadFileByPathVersion() throws Exception {
        ...
        try {
            ...
            ScmFile actfile = ScmFactory.File.getInstanceByPath(wsCRUD, dirpath + "/" + fileName, 1, 0);
            String downloadPath = TestTools.LocalFile.initDownloadPath(localPath, TestTools.getMethodName(),
                    Thread.currentThread().getId());
            actfile.getContent(downloadPath);
            Assert.assertEquals(actfile.getFileName(), fileName);
    ...
    private void tearDown() {
        try {
            ...
            ScmFactory.Role.revokePrivilege(sessionA, role, rs2, ScmPrivilegeType.CREATE);
            ScmFactory.Role.revokePrivilege(sessionA, role, rs2, ScmPrivilegeType.DELETE);
            ScmFactory.Role.revokePrivilege(sessionA, role, rs2, ScmPrivilegeType.UPDATE);
    ...
    private void prepare() throws Exception {
        try {
            ...
            rs2 = ScmResourceFactory.createDirectoryResource(wsp.getName(), path);
            grantPriAndAttachRole(sessionA, rs1, user, role, ScmPrivilegeType.READ);
            grantPriAndAttachRole(sessionA, rs2, user, role, ScmPrivilegeType.CREATE);
            grantPriAndAttachRole(sessionA, rs2, user, role, ScmPrivilegeType.DELETE);
            grantPriAndAttachRole(sessionA, rs2, user, role, ScmPrivilegeType.UPDATE);
