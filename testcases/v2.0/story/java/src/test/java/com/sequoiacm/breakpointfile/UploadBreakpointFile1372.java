...
public class UploadBreakpointFile1372 extends TestScmBase {
	...
	private ScmId checkUploadEmptyFileData(String fileName, byte[] expData) throws Exception{
		//save to file, than down file check the file data
		ScmFile file = ScmFactory.File.createInstance(ws);
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName);	
		file.setContent(breakpointFile);
        ...
        Assert.assertEquals(TestTools.getMD5(fileData), TestTools.getMD5(expData));		
		return fileId;
	}
	
	private void createEmptyBreakpointFile(ScmChecksumType checksumType, String fileName) throws ScmException, IOException {
