		...
		try {
			...
			if (0 != cnt) {
				...
				while (cursor.hasNext()) {
					...
					String ssCreateTime = ssInfo.get("creationTime").toString();
					sessionList.add("[_id:" + ssId + ", createTime:" + ssCreateTime + "]");
				}
				logger.error("remain session \nremainNum = " + cnt + ", session = " + sessionList);
			}
		} finally {
			if (null != cursor) {
				cursor.close();
