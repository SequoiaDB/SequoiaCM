...
public class UpdateContentBySameFile1688 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		UpdateContentThread updateContentThread1 = new UpdateContentThread(updateData1);
		UpdateContentThread updateContentThread2 = new UpdateContentThread(updateData2);
		updateContentThread1.start();
		updateContentThread2.start();
		
		if( updateContentThread1.isSuccess()){
			if( !updateContentThread2.isSuccess()){
				Assert.assertTrue( !updateContentThread2.isSuccess(), updateContentThread2.getErrorMsg());
				ScmException e =(ScmException) updateContentThread2.getExceptions().get(0);					
				Assert.assertEquals(e.getError(),ScmError.FILE_VERSION_MISMATCHING,
						"updateContent2 fail:"+updateContentThread2.getErrorMsg());
			...
			Assert.assertTrue( updateContentThread2.isSuccess(), updateContentThread2.getErrorMsg());
			ScmException e =(ScmException) updateContentThread1.getExceptions().get(0);				
			Assert.assertEquals(e.getError(),ScmError.FILE_VERSION_MISMATCHING,
					"updateContent1 fail:"+updateContentThread1.getErrorMsg());		
	...
	private void checkAllUpdateContentResult(ScmWorkspace ws,byte[] updatedata1, byte[] updatedata2) throws Exception{
		...
		if ( fileSize == updatedata1.length){
			VersionUtils.CheckFileContentByStream(ws, fileName, currentVersion, updatedata1);			
			VersionUtils.CheckFileContentByStream(ws, fileName, historyVersion2, updatedata2);
		}else if (fileSize == updatedata2.length ){
			VersionUtils.CheckFileContentByStream(ws, fileName, currentVersion, updatedata2);			
			VersionUtils.CheckFileContentByStream(ws, fileName, historyVersion2, updatedata1);
		}else{
	...
	private void checkUpdateContentResult(ScmWorkspace ws,byte[] updatedata) throws Exception{
