...
public class GetWorkSpaceInfo1828 extends TestScmBase {
	...
	private void createAndGetWorkSpace() throws ScmException{
		...
		ScmMetaLocation scmMetaLocation = new ScmSdbMetaLocation(rootSite.getSiteName(), 
				ScmShardingType.YEAR, TestSdbTools.getDomainNames(rootSite.getMetaDsUrl()).get(0));
		...
		for (int i = 0; i < siteList.size(); i++) {
			...
			if ( dataType.equals(DatasourceType.SEQUOIADB)){
				...
				scmDataLocationList.add(new ScmSdbDataLocation(siteName, domainName,ScmShardingType.YEAR,ScmShardingType.MONTH));
				break;
			}			
		}
		...
		ScmSdbDataLocation scmSdbDataLocation = (ScmSdbDataLocation) list.get(0);
		Assert.assertEquals(scmSdbDataLocation.getDomainName(), domainNames.get(0));			
		Assert.assertEquals(scmSdbDataLocation.getClShardingType(), ScmShardingType.MONTH);
		Assert.assertEquals(scmSdbDataLocation.getCsShardingType(), ScmShardingType.YEAR);
		Assert.assertEquals(scmSdbDataLocation.getType(), DatasourceType.SEQUOIADB);
		Assert.assertEquals(scmSdbDataLocation.getSiteName(), siteNames.get(0));		
	}	
}
