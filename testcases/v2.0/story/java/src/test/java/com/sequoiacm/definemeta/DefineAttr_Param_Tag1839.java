...
public class DefineAttr_Param_Tag1839 extends TestScmBase {
	...
	private void test() throws ScmException {
		...
		tags.addTag(" 18$39$");
		//testValueIsAll
		tags.addTag("1234567890 qertyuiopasdfghjkwzxcvbnml !@#$%^&*(){}|_+:\"<>?");
		//testValueIsChinese
		tags.addTag("标签1");
		...
		Assert.assertEquals(actBatchTags, expTags, batch.getTags().toString());
		...
		Assert.assertEquals(file.getTags().toSet().size(), 0, file.getTags().toString());
		Assert.assertEquals(batch.getTags().toSet().size(), 0, batch.getTags().toString());
	}
