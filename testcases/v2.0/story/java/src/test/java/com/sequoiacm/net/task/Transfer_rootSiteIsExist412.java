...
public class Transfer_rootSiteIsExist412 extends TestScmBase {
		...
		try {
			if (runSuccess || TestScmBase.forceClear) {
				...
				for (int i = 0; i < fileNum; ++i) {
					ScmFactory.File.deleteInstance(wsM, fileIdList.get(i), true);;
				}
				TestTools.LocalFile.removeFile(localPath);
	...
	private void readPartFile(ScmWorkspace ws, int readBegin, int readEnd) throws Exception {
	...
	private ScmId transferPartFile(ScmWorkspace ws, int transferBegin, int transferEnd) throws ScmException {
		...
		BSONObject condition = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(authorName).and(title)
				.greaterThan(begin).and(title).lessThan(end).get();
		return ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_CURRENT, targetSite.getSiteName());
	}

	private void checkTransfered(int checkBegin, int checkEnd) throws IOException, ScmException {
		...
		try {
			...
			ScmFileUtils.checkMetaAndData(ws_T,fileIdList.subList(checkBegin, checkEnd), expSiteList, localPath,
					filePath);
		} catch (Exception e) {
			Assert.fail(e.getMessage());
		} finally {
