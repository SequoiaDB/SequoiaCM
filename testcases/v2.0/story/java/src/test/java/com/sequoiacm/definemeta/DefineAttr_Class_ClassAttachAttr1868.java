...
/**
 * @Description: SCM-1868 :: 模型关联属性
 * @author fanyu
 * @Date:2018年7月4日
 * @version:1.0
 */
public class DefineAttr_Class_ClassAttachAttr1868 extends TestScmBase {
    private boolean runSuccess = false;
    private String name = "ClassAttachAttr1868";
    private String desc = "ClassAttachAttr1868";
    ...
    private void test() {
        ...
        try {
            ...
            check( classList.get( 0 ), attrList.subList( 0, 2 ) );
        } catch ( ScmException e ) {
            e.printStackTrace();
            Assert.fail( e.getMessage() );
        }
    ...
    private void check( ScmClass class1, List< ScmAttribute > attrList ) {
        try {
            ...
            for ( int i = 0; i < list.size(); i++ ) {
                Assert.assertEquals( list.get( i ).getDescription(),
                        attrList.get( i ).getDescription() );
            }
        } catch ( ScmException e ) {
            e.printStackTrace();
            Assert.fail( e.getMessage() );
