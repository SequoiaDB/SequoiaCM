...
public class AuthServer_CrudDiffRole1553 extends TestScmBase {
	...
	private void setUp() {
		...
		try {
			for (int i = 0; i < roleNum; i++) {
				ScmRole role = ScmFactory.Role.createRole(session, roleName + "_" + i, null);
				roleList.add(role);
			}
		} catch (ScmException e) {
			e.printStackTrace();
	...
	private class CreateRole extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				ScmRole role = ScmFactory.Role.createRole(session, rolename1, null);
				roleList.add(role);
		...
		private void check(String roleName, ScmRole role) {
			...
			try {
				...
				//Assert.assertEquals(actRole.getDescription(), role.getDescription());
	...
	private class QueryRole extends TestThreadBase {
		...
		public void exec() {
			try {
				ScmRole actrole = ScmFactory.Role.getRole(session, roleList.get(1).getRoleName());
