...
public class AuthWs_ListResource2280 extends TestScmBase {
	...
	private void testListRsByWs() {
		...
		try {
			int i = 0;
			cursor = ScmFactory.Resource.listResourceByWorkspace(sessionA, wsName2);
			while (cursor.hasNext()) {
				...
				if (!(resource.toStringFormat().equals(wsName2)
						|| resource.toStringFormat().equals(wsName2 + ":/AuthWs_Param_GetListResource2280"))) {
	...
	private void testDelSamePrefixWs() throws Exception {
		...
		try {
			...
			cursor = ScmFactory.Resource.listResourceByWorkspace(sessionA, wsName1);
			while (cursor.hasNext()) {
				ScmResource resource = cursor.getNext();
				System.out.println("resource = " + resource.toStringFormat());
				if (!(resource.toStringFormat().equals(wsName1)
						|| resource.toStringFormat().equals(wsName1 + ":/AuthWs_Param_GetListResource2280"))) {
					Assert.fail("exp true but act false");
				}
				i++;
			}
			...
			ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(wsName1, sessionA);
	...
	private void prepare() {
		try {
			...
			dirrs = ScmResourceFactory.createDirectoryResource(wsName1, dirpath);
			dirrs1 = ScmResourceFactory.createDirectoryResource(wsName2, dirpath);
			...
			grantPriAndAttachRole(sessionA, dirrs1, user, role, ScmPrivilegeType.READ);
			grantPriAndAttachRole(sessionA, dirrs1, user, role, ScmPrivilegeType.CREATE);
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
