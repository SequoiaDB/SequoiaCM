...
public class Param_listWorkSpace924 extends TestScmBase {
	...
	private void testSSIsNull() {
		try {
			...
			if (e.getErrorCode() != ScmError.INVALID_ARGUMENT.getErrorCode() || !e.getMessage().contains("session is null")) {
	...
	private void testSSNoExist() {
		...
		try {
			...
			Assert.fail("expect result is fail but actual is success, when session is unexist.");
		} catch (ScmException e) {
