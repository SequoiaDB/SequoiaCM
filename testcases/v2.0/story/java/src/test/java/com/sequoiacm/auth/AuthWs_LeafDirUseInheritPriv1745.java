...
public class AuthWs_LeafDirUseInheritPriv1745 extends TestScmBase {
    ...
    private String[] privileges = {ScmPrivilegeDefine.CREATE, ScmPrivilegeDefine.READ, ScmPrivilegeDefine.UPDATE,
            ScmPrivilegeDefine.DELETE + "|" + ScmPrivilegeDefine.CREATE + "|" + ScmPrivilegeDefine.READ + "|"
        ...
        try {
            try {
                for (int i = 0; i < rsList.size(); i++) {
                    ScmFactory.Role.revokePrivilege(sessionA, role, rsList.get(i), privileges[i]);
                }
    ...
    private void prepare() throws Exception {
        ...
        for (int i = 0; i < pathList.size() - 2; i++) {
            ScmResource rs = ScmResourceFactory.createDirectoryResource(wsp.getName(), pathList.get(i));
