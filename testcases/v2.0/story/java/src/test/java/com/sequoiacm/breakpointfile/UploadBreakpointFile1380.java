...
public class UploadBreakpointFile1380 extends TestScmBase {		
	...
	private ScmBreakpointFile uploadBreakpointFile() throws ScmException, IOException {
		// create file				
		...
		for ( int i = 0; i< 3; i++){			
			InputStream inputStream = new BreakpointStream(filePath, dataSizes[i]);	
			if ( i != 0){
				inputStream.read(new byte[dataSizes[i]],0, dataSizes[i]);
			}			
