...
public class CleanCurVersionFile1667 extends TestScmBase {	
	...
	private void startCleanTaskByCurrentVerFile(ScmWorkspace ws, ScmSession session, ScopeType scopeType) throws Exception {
	...
	private void checkHisVersionFileInfo(ScmWorkspace ws,int version) throws Exception{
		...
		while (cursor.hasNext()) {
			...
			size ++;
			ScmWorkspace ws1 = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionM);
