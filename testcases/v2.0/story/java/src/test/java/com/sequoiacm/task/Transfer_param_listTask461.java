...
public class Transfer_param_listTask461 extends TestScmBase {
	...
	private void matchInexistentTaskAttr() throws Exception {
		try {
			System.out.println("  taskId = " + taskId);
			BSONObject cond = ScmQueryBuilder.start("hhhWhatsthis").is(taskId.get()).get();
			ScmCursor<ScmTaskBasicInfo> cursor = ScmSystem.Task.listTask(sessionA, cond);
			if (cursor.hasNext()) {
				Assert.fail("list shouldn't have result when matcher has inexistent attribute");
			}
			cursor.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void listWhenSessionNull() throws Exception {
		try {
			BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.Task.ID).is(taskId).get();
