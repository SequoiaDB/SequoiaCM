 ...
 * @date 2019年01月07日
 */
public class CreateWorkspace2357 extends TestScmBase {
	private String wsName = "ws2357";
	private String namespace = "test2357";
	private ScmSession session = null;
	private SiteWrapper site = null;
	private Calendar cal = Calendar.getInstance();

	private String fileName = "scmfile2357";
	...
	private void setUp() throws Exception {
		...
		for(SiteWrapper tmpsite:sites) {
			if (tmpsite.getDataType().equals(ScmType.DatasourceType.HBASE)) {
				site = tmpsite;
				break;
			}
		}
	...
	private void test() throws Exception {
		...
		ScmHbaseDataLocation scmHbaseDataLocation = new ScmHbaseDataLocation(site.getSiteName());
		...
		if(cal.get(Calendar.MONTH)+1 < 10){
			tableName = namespace+":"+wsName+"_SCMFILE_"+cal.get(Calendar.YEAR)+ "0"+(cal.get(Calendar.MONTH)+1);
		}else{
			tableName = namespace+":"+wsName+"_SCMFILE_"+cal.get(Calendar.YEAR)+(cal.get(Calendar.MONTH)+1);
		}
		Assert.assertTrue(HbaseUtils.isInNS(site,namespace,tableName),"expect tableName is in namespace,tableName = " + tableName);
	}
	...
	private ScmWorkspace createWS(ScmSession session, String wsName, int siteNum, ScmDataLocation scmDataLocation)
