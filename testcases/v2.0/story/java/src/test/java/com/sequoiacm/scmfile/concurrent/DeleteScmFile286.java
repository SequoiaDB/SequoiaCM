...
public class DeleteScmFile286 extends TestScmBase {
	...
	private void setUp() throws ScmException {
		...
		try {
			...
			for (int i = 0; i < fileNum; i++) {
				BSONObject cond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author+"_M"+i).get();
				BSONObject cond1 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author+"_A"+i).get();
				BSONObject cond2 = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author+"_B"+i).get();
	...
	private void test() {
		...
		try {
			...
			if (!(deleteFromM.isSuccess() && deleteFromA.isSuccess() && deleteFromB.isSuccess())) {
				Assert.fail(deleteFromM.getErrorMsg() + deleteFromA.getErrorMsg() + deleteFromB.getErrorMsg());
			}
	...
	private class DeleteFromM extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				for (int i = 0; i < fileNum; i++) {
					ScmFactory.File.getInstance(ws, mainFileIds.get(i)).delete(true);
				}
			} finally {
				if (session != null) {
					session.close();
	...
	private class DeleteFromA extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				for (int i = 0; i < fileNum; i++) {
					ScmFactory.File.getInstance(ws, aSlvFileIds.get(i)).delete(true);
				}
			} finally {
				if (session != null) {
					session.close();
	...
	private class DeleteFromB extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				for (int i = 0; i < fileNum; i++) {
					ScmFactory.File.getInstance(ws, bSlvFileIds.get(i)).delete(true);
				}
			} finally {
				if (session != null) {
					session.close();
	...
	private void prepareScmFiles() throws ScmException {
		for (int i = 0; i < fileNum; i++) {
			ScmId mFileId = ScmFileUtils.create(wsM, fileName+"_M"+i, filePath);
			mainFileIds.add(mFileId);

			ScmId aFileId = ScmFileUtils.create(wsA, fileName+"_A"+i, filePath);
			aSlvFileIds.add(aFileId);

			ScmId bFileId = ScmFileUtils.create(wsB, fileName+"_B"+i, filePath);
		...
		try {
			...
			for (int i = 0; i < lists.size(); i++) {
				for (ScmId fileId : lists.get(i)) {
					try {
						// check meta
						BSONObject cond = new BasicBSONObject(ScmAttributeName.File.FILE_ID, fileId.get());
