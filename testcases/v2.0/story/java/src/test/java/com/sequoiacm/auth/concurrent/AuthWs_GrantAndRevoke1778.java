...
public class AuthWs_GrantAndRevoke1778 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        ScmCursor<ScmPrivilege> cursor = ScmFactory.Privilege.listPrivilegesByResource(sessionA, rs);
        while (cursor.hasNext()) {
            System.out.println("cursor = " + cursor.getNext().toString());
        }

        ScmCursor<ScmResource> cursor2 = ScmFactory.Resource.listResourceByWorkspace(sessionA, wsp.getName());
        while (cursor.hasNext()) {
    ...
    private class GrantRs extends TestThreadBase {
        ...
        public void exec() throws Exception {
            try {
                ...
                grantPriAndAttachRole(sessionA, this.rs, user, this.role, ScmPrivilegeType.CREATE);
            } catch (ScmException e) {
                e.printStackTrace();
                Assert.fail(e.getMessage());
            }
    ...
    private class RevokeRs extends TestThreadBase {
        ...
        public void exec() throws InterruptedException {
            try {
                ScmFactory.Role.revokePrivilege(sessionA, this.role, rs, ScmPrivilegeType.DELETE);
                Thread.sleep(10000);
            } catch (ScmException e) {
                e.printStackTrace();
                Assert.fail(e.getMessage());
    ...
    private void check(String dirpath) throws ScmException {
        ...
        try {
            ...
            Assert.fail("the user does not have priority to delete subpath = " + subpath);
        } catch (ScmException e) {
    ...
    private void prepare() throws Exception {
        rs = ScmResourceFactory.createDirectoryResource(wsp.getName(), dirpath);
