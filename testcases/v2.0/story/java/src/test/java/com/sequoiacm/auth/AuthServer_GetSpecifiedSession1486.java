...
public class AuthServer_GetSpecifiedSession1486 extends TestScmBase {
	...
	private void test() {
		try {
			ScmSessionInfo ssInfo = ScmFactory.Session.getSessionInfo(session, session1.getSessionId());
			Assert.assertEquals(ssInfo.getSessionId(), session1.getSessionId(), session1.toString());
			Assert.assertEquals(ssInfo.getUsername(), session1.getUser(), session1.toString());
			Assert.assertNotNull(ssInfo.getCreationTime());
			Assert.assertNotNull(ssInfo.getLastAccessedTime());
			Assert.assertEquals(ssInfo.getCreationTime() <= ssInfo.getLastAccessedTime(), true);
