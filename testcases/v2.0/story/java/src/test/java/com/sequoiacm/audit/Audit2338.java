...
public class Audit2338 extends TestScmBase {
    ...
    private void test() throws ScmException {
        ...
        //test ldap
        confMap.put(ConfigCommonDefind.scm_audit_user+name3,"BATCH_DQL|BATCH_DML");
        ...
        checkAudit(TestScmBase.ldapUserName,TestScmBase.ldapPassword,true,true);
    }
    ...
    private void checkAudit(String username,String password,boolean isLogged1,boolean isLogged2) throws ScmException {
        ...
        try{
            ...
            BSONObject bson1 = new BasicBSONObject().append(ScmAttributeName.Audit.TYPE, "CREATE_BATCH")
                    .append(ScmAttributeName.Audit.USERNAME,username);
            BSONObject bson2 = new BasicBSONObject().append(ScmAttributeName.Audit.TYPE, "BATCH_DQL")
                    .append(ScmAttributeName.Audit.USERNAME,username);
            Assert.assertEquals(ConfUtil.checkAudit(session,bson1
                    , batchId.get()), isLogged1, "Has the configuration been updated? batchId = " + batchId.get());
            Assert.assertEquals(ConfUtil.checkAudit(session, bson2
                        , batchId.get()), isLogged2, "Has not the configuration been updated? batchId = " + batchId.get());
        }finally{
            if(batchId != null) {
    ...
    private ScmId createAndQueryBatch(String username,String password,String batchName) throws ScmException {
