...
public class AuthServer_ModifySameUserDiffAttr1546 extends TestScmBase {
	...
	private class ModifyAttr extends TestThreadBase {
		...
		public void exec() {
			try {
				ScmUserModifier modifier = modiferList.get(atom.getAndIncrement());
		...
		private void check(ScmUser user1, ScmUserModifier modifier) {
			Assert.assertEquals(user1.getRoles().isEmpty(), modifier.getAddRoles().isEmpty());
			Assert.assertEquals(user1.getUserId(), user.getUserId());
			if (modifier.getPasswordType() == null) {
				Assert.assertEquals(user1.getPasswordType(), ScmUserPasswordType.LOCAL);
			}else{
				Assert.assertEquals(user1.getPasswordType(), modifier.getPasswordType());
			}
		}
	}
}
