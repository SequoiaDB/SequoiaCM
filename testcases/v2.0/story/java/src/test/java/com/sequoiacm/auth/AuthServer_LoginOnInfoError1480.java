...
public class AuthServer_LoginOnInfoError1480 extends TestScmBase {
	...
	private void test() {
		...
		try {
			session = TestScmTools.createSession(site, username, passwd + "_testInfoError");
			Assert.fail("exp login fail but act success,session = " + session.toString());
		} catch (ScmException e) {
	...
	private void testInexistUser() {
		...
		try {
			...
			Assert.fail("exp login fail but act success,session1 = " + session1.toString());
		} catch (ScmException e) {
			if (e.getError() != ScmError.HTTP_UNAUTHORIZED) {
				e.printStackTrace();
				Assert.fail(e.getMessage());
