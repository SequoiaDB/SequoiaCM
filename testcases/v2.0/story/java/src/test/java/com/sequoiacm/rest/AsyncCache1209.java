...
public class AsyncCache1209 extends TestScmBase {
	...
	private void setUp() throws Exception {
		...
		branchRest.connect(branchSite.getSiteServiceName(), TestScmBase.scmUserName,TestScmBase.scmPassword);
        rootRest = new RestWrapper();
        rootRest.connect(rootSite.getSiteServiceName(), TestScmBase.scmUserName,TestScmBase.scmPassword);
	...
	private void test() throws Exception {
		String response = branchRest.setRequestMethod(HttpMethod.POST)
				.setApi("files/" + fileId + "/async-cache?workspace_name=" + ws.getName())
		...
		try {
			...
			response = branchRest.setRequestMethod(HttpMethod.POST)
					.setApi("files/" + inexistentId + "/async-cache?workspace_name=" + ws.getName())
					.setResponseType(String.class).exec().getBody().toString();
	...
	private void checkAsyncCache(RestWrapper rest, WsWrapper ws, String fileId) throws Exception {
		int times = 120;
		int intervalSec = 1;
