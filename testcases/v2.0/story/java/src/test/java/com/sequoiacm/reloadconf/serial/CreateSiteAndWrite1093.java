	...
	private class ReloadConf extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				ScmConfigOption scOpt = new ScmConfigOption(TestScmBase.gateWayList.get(0)+"/"+rootSite.getSiteServiceName(), TestScmBase.scmUserName,
						TestScmBase.scmPassword);
				session = ScmFactory.Session.createSession(SessionType.NOT_AUTH_SESSION, scOpt);
				if (rootSite.getDataType().equals(DatasourceType.SEQUOIADB)) {
					ScmSiteUtils.createSite(session, newSiteName, TestScmBase.gateWayList.get(0),
							1, rootSite.getDataDsUrl(), user, passwdPath);
				} else if (rootSite.getDataType().equals(DatasourceType.HBASE)) {
					ScmSiteUtils.createSite(session, newSiteName,TestScmBase.gateWayList.get(0),
							2, rootSite.getDataDsUrl(), user, passwdPath);
				} else if (rootSite.getDataType().equals(DatasourceType.CEPH_S3)) {
					ScmSiteUtils.createSite(session, newSiteName,TestScmBase.gateWayList.get(0),
							3, rootSite.getDataDsUrl(), user, passwdPath);
				} else if (rootSite.getDataType().equals(DatasourceType.CEPH_SWIFT)) {
					ScmSiteUtils.createSite(session, newSiteName, TestScmBase.gateWayList.get(0),
							4, rootSite.getDataDsUrl(), user, passwdPath);
				} else {
					throw new Exception("DatasourceType is not exist,please check" + rootSite.getDataType());
				}
				ScmSiteUtils.deleteSite(session, newSiteName);
			} catch (ScmException e) {
				e.printStackTrace();
