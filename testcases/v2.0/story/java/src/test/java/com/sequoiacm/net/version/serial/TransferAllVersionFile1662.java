...
/**
 * test content:Transfer the all version file
 * testlink-case:SCM-1662
 * 
 * @author wuyan
 * @Date 2018.06.07
 * @modify By wuyan
...
public class TransferAllVersionFile1662 extends TestScmBase {	
	...
	private void setUp() throws IOException, ScmException {
		...
		filePath4 = localPath + File.separator + "localFile_" + fileSize4 + ".txt";
	...
	private void test() throws Exception {			
		...
		startTransferTaskByAllVerFile( wsS, sessionS, scopeType);
		
		checkTransferedFileSiteAndDataInfo(wsS, currentVersion,  historyVersion);
		checkNoTransferFileInfo(wsT, currentVersion,  historyVersion);	
		runSuccess = true;
	}
		...
		try {	
			if( runSuccess ){
				...
				for (String fileId : fileIdList) {
					ScmFactory.File.deleteInstance(wsT, new ScmId(fileId), true);
				}
				TestTools.LocalFile.removeFile(localPath);	
			}					
		} catch (Exception e) {
	...
	private void writeAndUpdateFile(ScmWorkspace ws) throws ScmException{
		for (int i = 0; i < fileNum; i++) {
			...
			if( i % 2 == 0 ){
				...
				fileId = VersionUtils.createFileByFile(ws, subfileName, filePath3,authorName);
				VersionUtils.updateContentByFile(ws, subfileName, fileId, filePath4);					
			}
			fileIdList.add(fileId.get());
		}
	}
	
	private void startTransferTaskByAllVerFile(ScmWorkspace ws, ScmSession session, ScopeType scopeType) throws Exception {
		condition = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).greaterThanEquals(fileSize2)
				.put(ScmAttributeName.File.FILE_ID).in(fileIdList).get();		
		taskId = ScmSystem.Task.startTransferTask(ws, condition, scopeType, targetSite.getSiteName());
		
		//wait task finish
		ScmTaskUtils.waitTaskFinish(session, taskId);		
	}
	
	private void checkTransferedFileSiteAndDataInfo(ScmWorkspace ws,int currentVersion, int historyVersion) throws Exception{
		//check the transfered file,check the sitelist and data			
		ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_ALL, condition);
		int size = 0;	
		...
		while (cursor.hasNext()) {
			...
			if ( version == currentVersion ){
				...
				VersionUtils.CheckFileContentByFile(ws, fileId, currentVersion, filePath2, localPath);
			}else{
				//check sitelist and transfered fileContent of the  historyVersion file
				VersionUtils.checkSite(ws, fileId, historyVersion, expSiteList);
				VersionUtils.CheckFileContentByFile(ws, fileId, historyVersion, filePath1, localPath);
			}						
			size++;
		}
		cursor.close();
	...
	private void checkNoTransferFileInfo(ScmWorkspace ws,int currentVersion, int historyVersion) throws ScmException{
		//check the no transfer file by all version 
		BSONObject condition = ScmQueryBuilder.start().put(ScmAttributeName.File.SIZE).lessThan(fileSize2)
				.put(ScmAttributeName.File.FILE_ID).in(fileIdList).get();			
		ScmCursor<ScmFileBasicInfo> cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_ALL, condition);
		int size = 0;			
