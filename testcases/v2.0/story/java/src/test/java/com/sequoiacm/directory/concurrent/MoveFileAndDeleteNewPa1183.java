 ...
 * @version:1.0
 */
public class MoveFileAndDeleteNewPa1183 extends TestScmBase{
	private boolean runSuccess;
	private ScmSession session;
	private ScmWorkspace ws;
	private SiteWrapper site;
	private WsWrapper wsp;
	private String dirBasePath = "/MoveFileAndDeleteNewPa1183";
	private String fullPath1 = dirBasePath + "/dir_1183_a";
	private String fullPath2 = dirBasePath + "/dir_1183_b";
	private ScmId fileId;
	private String author = "MoveFileAndDeleteNewPa1183";
	...
	private class MoveFile extends TestThreadBase {
		public void exec() {
			try {
				...
				ScmDirectory dir = ScmFactory.Directory.getInstance(ws, fullPath2);
				file.setDirectory(dir);
				check(fileId, dir);
			} catch (ScmException e) {
				if (e.getError() != ScmError.DIR_NOT_FOUND && e.getError() != ScmError.HTTP_NOT_FOUND) {
					check(fileId, dir);
					e.printStackTrace();
					Assert.fail(e.getMessage());
				}
	...
	private void createFile(ScmWorkspace ws, ScmDirectory dir) throws ScmException {
		try {
		...
		for (String path : pathList) {
			try {
				...
				if (e.getError() != ScmError.DIR_EXIST) {
					e.printStackTrace();
					Assert.fail(e.getMessage()+",path = " + path);
				}
			}
		}
