...
/**
 * @Description: SCM-2580:在目录下正序列取文件列表
 * @author fanyu
 * @Date:2019年09月04日
 * @version:1.0
 */
public class ListFileInPaDir2580 extends TestScmBase{
	private AtomicInteger expSuccessTestCount = new AtomicInteger(0);
	private SiteWrapper site;
	private WsWrapper wsp;
	private ScmSession session;
	private ScmWorkspace ws;
	private String dirPath = "/dir2580";
	private ScmDirectory scmDirectory;
	private String fileNamePrefix = "file2580";
	...
	public Object[][] generateRangData() throws Exception {
		BSONObject positive = ScmQueryBuilder.start(ScmAttributeName.File.FILE_NAME).is(1).get();
		BSONObject dPositive = ScmQueryBuilder.start(ScmAttributeName.File.FILE_ID).is(1)
				.and(ScmAttributeName.File.CREATE_TIME).is(1).and(ScmAttributeName.File.FILE_NAME).is(1).get();
		String[] sortNameAtr1 = new String[]{"fileName"};
		String[] sortNameAtr2 = new String[]{"fileId", "createDate", "fileName"};
		boolean[] typeAtr1 = new boolean[]{true};
		boolean[] typeAtr2 = new boolean[]{true, true, true};

		return new Object[][]{
				//filter  skip   limit initScmFiles  sortnameAtr  typeAtr
				//orderby:单个字段 正序
				//skip=0  limit=1
				...
				//skip == fileList.size
				{filter3, positive, fileList3.size(), 10, fileList3, sortNameAtr1, typeAtr1},
				//skip > fileList.size
				...
				//orderby:多个字段正序
				{filter1, dPositive, 0, 10, fileList1, sortNameAtr2, typeAtr2}
		};
	}

	@Test(dataProvider = "dataProvider")
	private void test(BSONObject filter, BSONObject orderby, int skip, int limit, List<ScmFileBasicInfo> list
				...
				while (cursor.hasNext()) {
					...
					try {
						...
						throw new Exception("filter = " + filter + ",orderby = " + orderby
								+ ",skip = " + skip + ",limit = " + limit + "，act = " + act+ ",exp = "
								+ exp, e);
					}
				}
				if (limit == 0 || count == 0) {
