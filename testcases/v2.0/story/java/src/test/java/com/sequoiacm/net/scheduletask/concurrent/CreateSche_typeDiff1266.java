...
public class CreateSche_typeDiff1266 extends TestScmBase {
	...
	private void setUp() {
		try {
			...
			wsR = ScmFactory.Workspace.getWorkspace(wss.get(1).getName(), ssR);
	...
	private void test() {
		try {
			...
			ScmScheduleUtils.checkScmFile(wsR, fileIds, fileNum / 2, fileNum, expSites2);
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private class CreateCleanSche extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				String cron = "* * * * * ?";
	            ScmSchedule sche = ScmSystem.Schedule.create(ssA1, wsp.getName(), 
	...
	private void writeScmFile(ScmWorkspace ws, int startNum, int endNum) throws ScmException, ParseException {
	...
	private void readyScmFile(ScmWorkspace ws, int startNum, int endNum) throws Exception {
		ScmCursor<ScmFileBasicInfo> cursor = null;
		try {
			for (int i = startNum; i < endNum; i++) {
				cursor = ScmFactory.File.listInstance(ws, ScopeType.SCOPE_CURRENT, queryCond);
