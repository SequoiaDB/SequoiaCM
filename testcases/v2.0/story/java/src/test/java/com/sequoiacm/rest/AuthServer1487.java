...
public class AuthServer1487 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		Assert.assertEquals(userInfo.getString("password_type"), ScmUserPasswordType.LOCAL.name());
		...
		Assert.assertEquals(userInfo1.getString("password_type"), ScmUserPasswordType.LOCAL.name());
		...
		Assert.assertEquals(userInfo2.length()>=1, true,userInfo2.toString());
		...
		Assert.assertEquals(roleInfo1.getString("role_name"), "ROLE_"+username);
		...
		Assert.assertEquals(roleInfo2.getString("role_name"), "ROLE_"+username);
		...
		String sessionId = sessions.getJSONObject(0).getString("session_id");
		...
		try{
			...
			if(e.getStatusCode().value() != ScmError.HTTP_NOT_FOUND.getErrorCode()) {
				throw e;
			}
		}
		...
		rest.setServerType("auth-server")
		    ...
		    .setResponseType(String.class).exec();
		try {
			rest.setServerType("auth-server").setRequestMethod(HttpMethod.GET)
					.setApi("/roles/" + username)
					.setResponseType(String.class).exec();
