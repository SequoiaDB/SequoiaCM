...
public class AuthWs_Resource1777 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        grantPriAndAttachRole(sessionA, rsList.get(0), user, roleList.get(0), ScmPrivilegeType.READ);
        grantPriAndAttachRole(sessionA, rsList.get(0), user, roleList.get(1), ScmPrivilegeType.READ);
        grantPriAndAttachRole(sessionA, rsList.get(1), user, roleList.get(1), ScmPrivilegeType.READ);

        ScmAuthUtils.checkPriority(site, username, passwd, roleList.get(0), wsp);
        ScmAuthUtils.checkPriority(site, username, passwd, roleList.get(1), wsp);
    ...
    private class GrantRs extends TestThreadBase {
        ...
        public void exec() throws InterruptedException {
            ...
            try {
                sessionA = TestScmTools.createSession(site);
                grantPriAndAttachRole(sessionA, this.rs, user, this.role, ScmPrivilegeType.READ);
            } catch (ScmException e) {
                e.printStackTrace();
                Assert.fail(e.getMessage());
            } finally {
    ...
    private void prepare() throws ScmException {
        for (String path : dirpaths) {
            ...
            ScmResource rs = ScmResourceFactory.createDirectoryResource(wsp.getName(), path);
            rsList.add(rs);
        }

        user = ScmFactory.User.createUser(sessionA, username, ScmUserPasswordType.LOCAL, passwd);
        for (String rolename : rolenames) {
            ScmRole role = ScmFactory.Role.createRole(sessionA, rolename, null);
            roleList.add(role);
        }
    }
}
