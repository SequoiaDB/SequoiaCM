...
/**
 * @description SCM-3722:listTask接口指定orderby参数获取task信息
 * @author YiPan
 ...
 * @version v1.0
 */
public class ListTask3722 extends TestScmBase {
    private String fileName = "file3722";
    ...
    private void orderById( int sort ) throws ScmException {
        ...
        ScmCursor< ScmTaskBasicInfo > cursor = ScmSystem.Task.listTask( session,
                new BasicBSONObject( "success_count", 0 ),
                new BasicBSONObject( ScmAttributeName.Task.ID, sort ), 0, -1 );
        while ( cursor.hasNext() ) {
            actIds.add( cursor.getNext().getId().get() );
        }
        Assert.assertNotEquals( actIds.size(), 0 );
