...
public class ListByQueryBuilder312 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		try {
			...
			while (cursor.hasNext()) {
				...
				// check results
				Assert.assertEquals(fileInfo.getFileName(), fileList.get(0).getFileName());
				Assert.assertEquals(fileInfo.getFileId(), fileList.get(0).getFileId(), fileInfo.toString());
				Assert.assertEquals(fileInfo.getMinorVersion(), fileList.get(0).getMinorVersion());
				Assert.assertEquals(fileInfo.getMajorVersion(), fileList.get(0).getMajorVersion());
				size++;
			}
	...
	private ScmQueryBuilder ScmBuilder() throws ScmException {
		...
		// greaterThan
		BSONObject greaterThan = ScmQueryBuilder.start(k2).greaterThan(-1).get();

		// greaterThanEquals
		BSONObject greaterThanEquals = ScmQueryBuilder.start(k2).greaterThanEquals(1).get();
		...
		// is
		BSONObject is = ScmQueryBuilder.start(k1).is(fileList.get(0).getFileName()).get();

		// and
		BSONObject and = ScmQueryBuilder.start().and(lessThan, greaterThan).get();

		ScmQueryBuilder builder = ScmQueryBuilder.start().and(or, exist, greaterThan, greaterThanEquals, lessThan, is,
				not, and);

		// System.out.println(cond.toString());
		String bsStr = "{ \"$and\" : [ { \"$or\" : [ { \"name\" : { \"$in\" : [ \"" + fileName + "0\" , \"" + fileName
				+ "1\"]}} , { \"name\" : { \"$nin\" : [ \"" + fileName + "1\" , \"" + fileName
		...
		Assert.assertEquals(builder.get().toString().replaceAll("\\s*",""), bsStr.replaceAll("\\s*",""));
