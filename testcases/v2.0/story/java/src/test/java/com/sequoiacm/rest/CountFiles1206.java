...
public class CountFiles1206 extends TestScmBase {
	...
	private void test() throws Exception {
		String response = rest.setRequestMethod(HttpMethod.HEAD)
				.setApi("files?workspace_name=" + ws.getName() + "&filter={uri}")
				.setUriVariables(new Object[]{ "{\"author\":\"" + authorName + "\"}" })
		...
		try {
			response = rest.setRequestMethod(HttpMethod.HEAD)
					.setApi("files?workspace_name=inexistent_ws_name1206&filter={uri}")
					.setUriVariables(new Object[]{"{\"author\":\"" + authorName + "\"}"})
					.setResponseType(String.class).exec().getHeaders().get("X-SCM-Count").toString();
			Assert.fail("count files of inexistent ws should not succeed");
		} catch (HttpClientErrorException|HttpServerErrorException e) {
			Assert.assertEquals(e.getStatusCode().value(), ScmError.HTTP_UNAUTHORIZED.getErrorCode());
		}
	}
