...
public class TransferHisVersionFile1665 extends TestScmBase {	
	...
	private void test() throws Exception {			
		...
		startTransferTaskByHistoryVerFile(wsS, sessionS);		

		// check task info, the task successCount is 0
		ScmTask taskInfo = ScmSystem.Task.getTask(sessionS, taskId);
	...
	private void startTransferTaskByHistoryVerFile(ScmWorkspace ws, ScmSession session) throws Exception {
		...
		taskId = ScmSystem.Task.startTransferTask(ws, condition, ScopeType.SCOPE_HISTORY, targetSite.getSiteName());
