...
public class GetInstance_Para1201 extends TestScmBase{
	...
	private void testGetChineseDir() {
		...
		try {
			...
			ScmDirectory subdir1 =  ScmFactory.Directory.getInstance(ws, dirBasePath + "/"  + pathName);
	...
	private void testCreateWsIsNull() {
		try {
			ScmDirectory dir = ScmFactory.Directory.getInstance(null, dirBasePath);
			Assert.fail("expect fail but act success," + dir.toString());
		} catch (ScmException e) {
			if (e.getError() != ScmError.INVALID_ARGUMENT) {
				e.printStackTrace();
	...
	private void testPaPathInexist() {
		...
		try {
			...
			dir.delete();
			ScmDirectory subdir = ScmFactory.Directory.getInstance(ws,paPath);
