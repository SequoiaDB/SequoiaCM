...
/**
 * @description SCM-5992:指定不同限额配置开启限额
 * @author ZhangYanan
...
public class BucketQuota5992 extends TestScmBase {
    private boolean runSuccess = false;
    private String keyName = "object5992";
    private String bucketName = "bucket5992";
    ...
    public void test() throws Exception {
        ...
        checkPutObjectSuccessQuota( -1, maxObjectSize1 );
        checkPutObjectFailedQuota( -1, maxObjectSize1 );
        ScmFactory.Quota.disableBucketQuota( session, bucketName );

        quotaConfig = ScmEnableBucketQuotaConfig.newBuilder( bucketName )
                .setMaxObjects( maxObjectNum2 ).build();
        ScmFactory.Quota.enableBucketQuota( session, quotaConfig );
        checkPutObjectSuccessQuota( maxObjectNum2, -1 );
        checkPutObjectFailedQuota( maxObjectNum2, -1 );
        ScmFactory.Quota.disableBucketQuota( session, bucketName );

        try {
            ScmEnableBucketQuotaConfig.newBuilder( bucketName ).build();
