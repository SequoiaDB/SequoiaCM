...
public class TestScmTools extends TestScmBase {
	 ...
	 */
	public static ScmSession createNoAuthSession(SiteWrapper site) throws ScmException {
		List<String> urlList = new ArrayList<String>();
		for (String gateway : gateWayList) {
			urlList.add(gateway + "/" + site.getSiteServiceName());
		}
		ScmConfigOption scOpt = new ScmConfigOption(urlList);
		return ScmFactory.Session.createSession(SessionType.NOT_AUTH_SESSION, scOpt);
	}

	/**
	 * create sessionMgr, by specified site and define user
	 */
	public static  ScmSessionMgr createSessionMgr(SiteWrapper site,String username,String password) throws ScmException {
		return createSessionMgr(site.getSiteServiceName(),username, password,Long.MAX_VALUE);
	}

	/**
	 * create sessionMgr, by specified site and default scm user
	 */
	public static  ScmSessionMgr createSessionMgr(SiteWrapper site) throws ScmException {
		return createSessionMgr(site.getSiteServiceName(),TestScmTools.scmUserName, TestScmTools.scmPassword,Long.MAX_VALUE);
	}

	/**
	 * create sessionMgr, by specified serviceName and user and syncGateWayInterval
	 */
	public static ScmSessionMgr createSessionMgr(String serviceName,String username, String password, long syncGateWayInterval) throws ScmException {
		List<String> urlList = new ArrayList<String>();
		for (String gateway : gateWayList) {
			urlList.add(gateway + "/" + serviceName);
		}
		...
		ScmConfigOption scOpt = new ScmConfigOption(urlList, username, password);
		return ScmFactory.Session.createSessionMgr(scOpt, syncGateWayInterval);
	}
}
