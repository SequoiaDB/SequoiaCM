...
public class CreateSche_disable2368 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        BSONObject bson = ScmQueryBuilder.start(ScmAttributeName.Schedule.ID).is(scheduleId.get()).get();
        //make sure the schedule is disabled
        for (int i = 0; i < 60; i++) {
            ScmCursor<ScmTaskBasicInfo> cursor = ScmSystem.Task.listTask(ssA, bson);
    ...
    private void createScheduleTask() throws ScmException {
        ...
        ScmScheduleContent content = new ScmScheduleCleanFileContent(branSite.getSiteName(), maxStayTime, bson);
        String cron = "* * * * * ?";
        ScmSchedule sche = ScmSystem.Schedule.create(ssA, wsp.getName(),
                ScheduleType.CLEAN_FILE, scheduleName, "", content, cron, false);
