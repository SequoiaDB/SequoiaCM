...
public class CreateSche_enable2dis2370 extends TestScmBase {
    ...
    private void test() throws Exception {
        ...
        ScmScheduleUtils.checkScmFile(wsA, fileIds.subList(fileNum+10, fileNum + 20), expSites1);

        //enable
        schedule.enable();
        SiteWrapper[] expSites2 = {rootSite,branSite};
        this.checkScheduleTaskInfo(true);
        ScmScheduleUtils.checkScmFile(wsA, fileIds.subList(fileNum+10, fileNum + 20), expSites2);
        runSuccess = true;
    }
    ...
    private void waitSchedueDisable(BSONObject cond) throws Exception {
        ...
        for(int i = 0; i < 60;i++){
            ...
            if(j == count){
                ...
                if(i == 59){
                    throw  new Exception("wait time out,disabled schedule is still runing!!!" +
