...
public class UDQSche_taskNotExist1256 extends TestScmBase {
	private static final Logger logger = Logger.getLogger(UDQSche_taskNotExist1256.class);
	...
	private void setUp() {
		try {
			wsp = ScmInfo.getWs();
			branSite = ScmNetUtils.getNonLastSite(wsp);
	...
	private void test_deleteTaskNotExist() throws Exception {
		...
		try {
			...
			logger.info("delete not exist schedule, errorMsg = [" + e.getError() + "]");
		}
	}
	...
	private void test_updateTaskNotExist() throws Exception {
		try {
			...
			logger.info("update not exist schedule, errorMsg = [" + e.getError() + "]");
		}
	}
