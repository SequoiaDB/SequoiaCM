...
public class GetFile1161 extends TestScmBase{
	...
	private void setUp() {
		...
		try {
			...
			//create file
			createFile(ws,ScmFactory.Directory.getInstance(ws, "/"),author);
	...
	private void testSubFile() throws Exception {
		try {
			...
			ScmDirectory subDir = ScmFactory.Directory.getInstance(ws, fullPath1);
	...
	private void testFile1() throws Exception {
		try {
            ScmFile file1 = ScmFactory.File.getInstanceByPath(ws, "/" + author);
			BSONObject cond1 = new BasicBSONObject();
			cond1.put("name",author);
			cond1.put("author", author);
			cond1.put("filesize",fileSize);
	...
	private void check(ScmFile file, BSONObject cond) throws Exception {
		try {
			Assert.assertTrue(file.getFileName().contains(cond.get("name").toString()));
			Assert.assertEquals(file.getAuthor(), cond.get("author"));
			Assert.assertEquals((int)file.getSize(), cond.get("filesize"));
			...
			Assert.assertEquals(file.getDirectory().getPath(), cond.get("path"));
			Assert.assertEquals(file.getMinorVersion(), 0);
			Assert.assertEquals(file.getMajorVersion(), 1);
			SiteWrapper[] expSiteList = { site };
			ScmFileUtils.checkMetaAndData(wsp, file.getFileId(), expSiteList, localPath, filePath);
		} catch (ScmException e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
		}
	...
	private void createFile(ScmWorkspace ws, ScmDirectory dir,String fileName) {
		ScmFile file;
		try {
