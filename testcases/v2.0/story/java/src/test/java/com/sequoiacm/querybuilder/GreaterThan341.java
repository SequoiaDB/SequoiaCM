...
/**
 * @Testcase: SCM-341:greaterThan多个不同字段，覆盖所有文件属性
 * @author huangxiaoni init
 * @date 2017.5.25
 */

public class GreaterThan341 extends TestScmBase {
    ...
    private String authorName = "GreaterThan341";
    ...
    private void testQuery() throws Exception {
        try {
            ...
            for ( Object[] kv : kvs ) {
                ...
                if ( null == builder ) {
                    builder = ScmQueryBuilder.start( key ).greaterThan( value );
                    bsStr = bsStr + subStr;
                } else {
                    builder.put( key ).greaterThan( value );
                    bsStr = bsStr + " , \"" + subStr;
                }
            }
            cond = builder.put( ScmAttributeName.File.AUTHOR ).is( authorName )
                    .get();
