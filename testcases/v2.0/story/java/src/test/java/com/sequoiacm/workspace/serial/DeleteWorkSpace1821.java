...
public class DeleteWorkSpace1821 extends TestScmBase {	
	...
	private void deleteWorkspaceByOtherUser() throws ScmException, InterruptedException{
		for (int i = 0; i < 10; i++) {
			Thread.sleep(1000);
			try {
				ScmFactory.Workspace.getWorkspace(wsName, session);
		...
		ScmUser user = ScmFactory.User.createUser(session, newUsername, ScmUserPasswordType.LOCAL, newPassword);
		...
		ScmFactory.Role.grantPrivilege(session, role, rs, ScmPrivilegeType.CREATE );
		ScmFactory.Role.grantPrivilege(session, role, rs, ScmPrivilegeType.READ);
		modifier.addRole(role);
		ScmFactory.User.alterUser(session, user, modifier);
		
		ScmSession newSession = TestScmTools.createSession(site, newUsername, newPassword);		
		...
		ScmWorkspace ws = ScmFactory.Workspace.getWorkspace(wsName, newSession);
