...
public class AuthWs_CreateRead1723 extends TestScmBase {
    ...
    private String[] usernameArr = {"AuthWs_Create1723_C", "AuthWs_Create1723_CR"};
    ...
    @Test(groups = {"oneSite", "twoSite", "fourSite"})
    private void testCreateFileByOutputStream() throws ScmException, IOException {
        ScmId fileId = null;
        ScmFile expfile = null;
    ...
    private void testCreateBatch() throws ScmException {
        ...
        try {
            ...
            Assert.assertEquals(actBatch.getCreateUser(), expBatch.getCreateUser());
            Assert.assertEquals(actBatch.getWorkspaceName(), expBatch.getWorkspaceName());
    ...
    @Test(groups = {"oneSite", "twoSite", "fourSite"})
    private void testCreateBreakPointFileByName() throws ScmException, IOException {
        BreakpointUtil.checkDBDataSource();
        String fileName = author + UUID.randomUUID();
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsC, fileName);
            InputStream inputStream = new BreakpointInputStream(new FileInputStream(new File(filePath)));
            expBreakpointFile.incrementalUpload(inputStream, false);
            inputStream.close();

            // check
            ScmBreakpointFile actBreakpointFile = ScmFactory.BreakpointFile.getInstance(wsA, fileName);
            Assert.assertEquals(actBreakpointFile.getFileName(), expBreakpointFile.getFileName());
            Assert.assertEquals(actBreakpointFile.getSiteName(), expBreakpointFile.getSiteName());
            Assert.assertEquals(actBreakpointFile.getChecksumType(), expBreakpointFile.getChecksumType());
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
    ...
    @Test(groups = {"oneSite", "twoSite", "fourSite"})
    private void testCreateBreakPointFileByChecksumType() throws ScmException, IOException {
        BreakpointUtil.checkDBDataSource();
        String fileName = author + UUID.randomUUID();
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsC, fileName, ScmChecksumType.CRC32);
            InputStream inputStream = new BreakpointInputStream(new FileInputStream(new File(filePath)));
            expBreakpointFile.incrementalUpload(inputStream, false);
            inputStream.close();
            // check
    ...
    @Test(groups = {"oneSite", "twoSite", "fourSite"})
    private void testCreateBreakPointFileBySize() throws ScmException, IOException {
        BreakpointUtil.checkDBDataSource();
        String fileName = author + UUID.randomUUID();
        ScmBreakpointFile expBreakpointFile = null;
        try {
            expBreakpointFile = ScmFactory.BreakpointFile.createInstance(wsC, fileName, ScmChecksumType.ADLER32, 0);
            InputStream inputStream = new BreakpointInputStream(new FileInputStream(new File(filePath)), 1);
            ...
            Assert.assertEquals(actBreakpointFile.getSiteName(), expBreakpointFile.getSiteName());
            Assert.assertEquals(actBreakpointFile.getChecksumType(), expBreakpointFile.getChecksumType());
            Assert.assertEquals(actBreakpointFile.getUploadSize(), expBreakpointFile.getUploadSize());
        } catch (ScmException e) {
            e.printStackTrace();
            Assert.fail(e.getMessage());
        } finally {
    ...
    private void testCreateSche() throws ScmException {
        ...
        try {
            queryCond = ScmQueryBuilder.start(ScmAttributeName.File.AUTHOR).is(author).get();
            ...
            String crond = "* * * * * ? 2029";
            expSche = ScmSystem.Schedule.create(sessionC, wsp.getName(), ScheduleType.COPY_FILE, scheName, null,
                    content, crond);

            // check
            ScmSchedule actSche = ScmSystem.Schedule.get(sessionA, expSche.getId());
            Assert.assertEquals(expSche.getCreaateUser(), actSche.getCreaateUser());
    ...
    private void tearDown() {
        try {
            ScmFactory.Role.revokePrivilege(sessionA, roleList.get(0), rs, ScmPrivilegeType.CREATE);
            ...
            ScmFactory.Role.deleteRole(sessionA, roleList.get(0).getRoleName());
            ScmFactory.Role.deleteRole(sessionA, roleList.get(1).getRoleName());
    ...
    private void prepare() throws Exception {
        try {
            ScmUser user = ScmFactory.User.createUser(sessionA, usernameArr[0], ScmUserPasswordType.LOCAL, passwd);
            userList.add(user);
            ScmRole role = ScmFactory.Role.createRole(sessionA, rolenameArr[0], null);
            roleList.add(role);

            ScmUser user1 = ScmFactory.User.createUser(sessionA, usernameArr[1], ScmUserPasswordType.LOCAL, passwd);
            userList.add(user1);
            ScmRole role1 = ScmFactory.Role.createRole(sessionA, rolenameArr[1], null);
            roleList.add(role1);
            ...
            grantPriAndAttachRole(sessionA, rs, user1, role1, ScmPrivilegeType.CREATE);
            grantPriAndAttachRole(sessionA, rs, user1, role1, ScmPrivilegeType.READ);

            ScmAuthUtils.checkPriority(site, usernameArr[0], passwd, role, wsp.getName());
            ScmAuthUtils.checkPriority(site, usernameArr[1], passwd, role1, wsp.getName());

            sessionC = TestScmTools.createSession(site, usernameArr[0], passwd);
            wsC = ScmFactory.Workspace.getWorkspace(wsp.getName(), sessionC);

            sessionCR = TestScmTools.createSession(site, usernameArr[1], passwd);
