...
public class UpdateFileInfo1207 extends TestScmBase {
	...
	private void testChinese() throws Exception {
		String response = rest.setRequestMethod(HttpMethod.PUT)
				.setApi("files/" + fileId + "?workspace_name=" + ws.getName() + "&major_version=1&minor_version=0")
		...
		JSONObject obj =  new JSONObject(fileInfo.substring(1, fileInfo.length() - 1));
	...
	private void testInvaild() throws Exception {
		try {
			rest.setRequestMethod(HttpMethod.PUT)
					.setApi("files/" + fileId + "?ws_name=" + ws.getName() + "&major_version=1&minor_version=0")
					.setParameter("file_info", "{ author: '' }")
					.setResponseType(String.class)
					.exec()
					.getHeaders()
					.toString();
			Assert.fail("updating file without necessary argument should not succeed");
