...
public class UpdateBatchAttr1296 extends TestScmBase {
	...
	private void test() throws Exception {
		...
		ScmAttribute oldScmAttribute =  ScmFactory.Attribute.createInstance(ws, oldAttr);
		ScmClass oldScmClass = ScmFactory.Class.createInstance(ws, oldClassName, "i am a old class1296");
		oldScmClass.attachAttr(oldScmAttribute.getId());
		
		oldScmClassId = oldScmClass.getId();
		oldAttrId = oldScmAttribute.getId();
    	
    	ScmClassProperties oldProps = new ScmClassProperties(oldScmClassId.toString());
		...
		Assert.assertEquals(batch.getClassProperties().toString(), oldProps.toString());
		...
		ScmAttribute newScmAttribute =  ScmFactory.Attribute.createInstance(ws, newAttr);
		ScmClass newScmClass = ScmFactory.Class.createInstance(ws, newClassName, "i am a new class1296");
		newScmClass.attachAttr(newScmAttribute.getId());
		
		newScmClassId = newScmClass.getId();
		newAttrId = newScmAttribute.getId();
    	
    	ScmClassProperties newProps = new ScmClassProperties(newScmClassId.toString());
		...
		Assert.assertEquals(batch.getClassProperties().toString(), newProps.toString());
		runSuccess = true;
	}
