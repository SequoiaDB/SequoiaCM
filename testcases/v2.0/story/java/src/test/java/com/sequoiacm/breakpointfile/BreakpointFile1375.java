...
public class BreakpointFile1375 extends TestScmBase {
	...
	private void test() throws ScmException, IOException, JSONException {
		...
		//创建断点文件,大小为1024*512
		BreakpointUtil.createBreakpointFile(ws, filePath,fileName1,1024*512, ScmChecksumType.NONE);
		BreakpointUtil.createBreakpointFile(ws, filePath,fileName2,1024*512, ScmChecksumType.NONE);
		BreakpointUtil.createBreakpointFile(ws, filePath,fileName3,1024*512, ScmChecksumType.NONE);
	...
	private void uploadBreakpointFile1() throws IOException, ScmException {
		...
		System.arraycopy(fileByte, 512*512, newFileByte, 0, newFileByte.length);
		
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName1);
	...
	private void uploadBreakpointFile2() throws IOException, ScmException {
		...
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName2);
	...
	private void uploadBreakpointFile3() throws IOException, ScmException {
		...
		for (int i = 0; i < 1024; i++) {
			newFileByte[fileSize+i] = basicByte[new Random().nextInt(9)];
		}
		
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName3);
		InputStream inputStream2 = new ByteArrayInputStream(newFileByte);
		breakpointFile.upload(inputStream2);
		inputStream2.close();
	...
	private void checkBreakpointFile(String fileName, int fileSize) throws ScmException{
		ScmBreakpointFile breakpointFile = ScmFactory.BreakpointFile.getInstance(ws, fileName);
		Assert.assertEquals(breakpointFile.getUploadSize(), fileSize, "check upload size");
	}
}
