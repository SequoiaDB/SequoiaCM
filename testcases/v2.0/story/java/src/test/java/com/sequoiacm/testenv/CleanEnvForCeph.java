...
public class CleanEnvForCeph extends TestScmBase {
	private static final Logger logger = Logger.getLogger(CleanEnvForCeph.class);
	...
	private void setUp() {
		...
		for (SiteWrapper site : sites) {
			...
			if (dsType.equals(DatasourceType.CEPH_S3) || dsType.equals(DatasourceType.CEPH_SWIFT)) {
				isCephEnv = true;
				break;
			} 
		}
	...
	private void deleteAllBuckets(SiteWrapper site) throws Exception {
		...
		try {
			...
			buckets = conn.listBuckets();
			if (0 != buckets.size()) {
				throw new Exception("failed to delete all buckets, remain buckets = " + buckets);
			}
		} catch (Exception e) {
			logger.error("failed to delete all buckets, siteId = " + site.getSiteId() + ", bucketName = " + bucketName
					+ ", key = " + key);
			throw e;
		}
	}
	...
	private void deleteAllContainers(SiteWrapper site) throws Exception {
		...
		try {
			...
			if (0 != containers.size()) {
				throw new Exception("failed to delete all containers, remain buckets = " + containers);
			}
		} catch (Exception e) {
