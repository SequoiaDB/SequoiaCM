 ...
 * @version:1.0
 */
public class MoveDirAndReNamePaDir1176B extends TestScmBase {
	private boolean runSuccess;
	private ScmSession session;
	private ScmWorkspace ws;
	private SiteWrapper site;
	private WsWrapper wsp;
	private String dirBasePath = "/MoveDirAndReNamePaDir1176B";
	...
	private void setUp() {
		try {
			...
			deleteDir(ws, fullPath1 + "/Dir_1176_d");
			deleteDir(ws, dirBasePath + "/Dir_1176_e");
	...
	private class MoveDir extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				expBSON1.put("path", fullPath1 + "/Dir_1176_d/");
				expBSON1.put("wsName", wsp.getName());
				expBSON1.put("paName", "Dir_1176_a");
				checkDir(fullPath1 + "/Dir_1176_d", expBSON1);
	...
	private class ReNamePaDir extends TestThreadBase {
		...
		public void exec() {
			try {
				...
				// check
				BSONObject expBSON1 = new BasicBSONObject();
				expBSON1.put("name", "Dir_1176_e");
				expBSON1.put("path", dirBasePath + "/Dir_1176_e/");
				expBSON1.put("wsName", wsp.getName());
				expBSON1.put("paName", "MoveDirAndReNamePaDir1176B");
				checkDir(dirBasePath + "/Dir_1176_e/", expBSON1);
			} catch (ScmException e) {
				if (e.getError() != ScmError.DIR_NOT_FOUND) {
					e.printStackTrace();
					Assert.fail(e.getMessage());
