...
public class Ssh {
    ...
    public void exec(String command) throws Exception {
        ...
        try {
            ...
            if (exitStatus != 0) {
                throw new Exception("ssh failed to execute commond '" + command + "',stderr:"
                        + stderr + " ,stdout:" + stdout + ",errcode: " + exitStatus);
            }
        }
    ...
    public String getScmInstallDir() throws Exception {
        ...
        try {
            ...
            ssh.exec(
                    "ps -ef | grep sequoiacm-content-server  | grep -v grep |  head -n 1  | awk -F \" \" '{print $NF}'");
            String str = ssh.getStdout();
            if (str.length() <= 0 || !str.startsWith("--logging.config=")) {
                throw new Exception(
                        "failed to get scm install dir:ps -ef | grep sequoiacm-content-server  | grep -v grep |  head -n 1  | awk -F \\\" \\\" '{print $NF}', stdout="
                                + str);
            }
