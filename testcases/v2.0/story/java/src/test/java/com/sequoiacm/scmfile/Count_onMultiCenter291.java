...
public class Count_onMultiCenter291 extends TestScmBase {
	...
	private void test() {
		try {
			...
			if (!(qryThd1.isSuccess() && qryThd2.isSuccess() && qryThd3.isSuccess())) {
				Assert.fail(qryThd1.getErrorMsg() + qryThd2.getErrorMsg() + qryThd3.getErrorMsg());
			}
		} catch (Exception e) {
			e.printStackTrace();
			Assert.fail(e.getMessage());
	...
	private class QueryThread extends TestThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				String startVal = ScmFactory.File.getInstance(ws, fileIdList.get(0)).getTitle();
				String endVal = ScmFactory.File.getInstance(ws, fileIdList.get(countNum)).getTitle();

				BSONObject fileCond = ScmQueryBuilder.start().and(key).greaterThanEquals(startVal).and(key)
						.lessThan(endVal).and("notexistkey").exists(0).get();

				long actCount = ScmFactory.File.countInstance(ws, ScopeType.SCOPE_CURRENT, fileCond);
				Assert.assertEquals(actCount, countNum, "fileCond = " + fileCond.toString());
			} finally {
				if (session != null) {
					session.close();
				}
