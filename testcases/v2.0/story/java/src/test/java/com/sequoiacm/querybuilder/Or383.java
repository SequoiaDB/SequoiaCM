...
public class Or383 extends TestScmBase {
	...
	private void testQuery() throws Exception {
		try {
			...
			for (Object[] kv : kvs) {
				...
				BSONObject obj = ScmQueryBuilder.start((String) kv[0]).is(kv[1]).get();
				if (null == builder) {
	...
	private void readyScmFile() {
		try {
			for (int i = 0; i < fileNum; i++) {
				...
				scmfile.setFileName(authorName + "_" + TestTools.getRandomString(10) + i);
	...
	private Object[][] kvsArr() throws ScmException {
		...
		return new Object[][] { new Object[] { ScmAttributeName.File.FILE_ID, file.getFileId().get() },
				...
				new Object[] { ScmAttributeName.File.MAJOR_VERSION, file.getMajorVersion() },
				new Object[] { ScmAttributeName.File.MINOR_VERSION, file.getMinorVersion() },
				...
				new Object[] { ScmAttributeName.File.UPDATE_USER, file.getUpdateUser() },
