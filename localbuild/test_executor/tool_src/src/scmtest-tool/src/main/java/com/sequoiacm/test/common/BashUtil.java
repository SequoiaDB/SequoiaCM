...
public class BashUtil {
    ...
    public static ExecResult exec(String command) throws IOException {
        ...
        try {
            ...
            logger.debug("Executing command locally, command:{}", cmd2Str(cmd));
            ...
            throw new IOException("Failed to execute command locally, command=" + cmd2Str(cmd), e);
        }
        finally {
            if (process != null) {
                process.destroy();
    ...
    private static String cmd2Str(String[] cmdArray) {
        String tmp = "";
