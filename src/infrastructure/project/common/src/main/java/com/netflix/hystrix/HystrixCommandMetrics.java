...
/**
 *
 * 修改hystrix健康统计规则，被信号量、线程池拒绝的请求不会计入统计
 * 使熔断器不会因为短时间内大量的请求被拒绝而产生熔断，具体的修改请见：
 * @see HealthCounts#plus(long[])
 */
public class HystrixCommandMetrics extends HystrixMetrics {
    ...
    static {
        logger.info("sequoiacm HystrixCommandMetrics.class loaded");
    }
     ...
     * Number that failed (failure + success + timeout).
     * Error percentage;
     */
    public static class HealthCounts {
        ...
        public HealthCounts plus(long[] eventTypeCounts) {
            ...
            /**
             * 计算时忽略信号量拒绝：semaphoreRejectedCount与线程池拒绝：threadPoolRejectedCount的请求
             * 这里统计的数据会在熔断器中使用：
             * @see HystrixCircuitBreaker.HystrixCircuitBreakerImpl#subscribeToStream()
             */
