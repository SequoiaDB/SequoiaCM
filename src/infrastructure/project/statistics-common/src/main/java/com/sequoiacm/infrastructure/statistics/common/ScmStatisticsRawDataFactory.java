...
public class ScmStatisticsRawDataFactory {
    public static ScmStatisticsRawData createRawData(String type, String user, long timestamp,
            long responseTime, String extraInfo) {
        switch (type) {
            case ScmStatisticsType.FILE_DOWNLOAD:
            case ScmStatisticsType.FILE_UPLOAD:
                return new ScmStatisticsFileRawData(type, user, timestamp, responseTime, extraInfo);
            default:
