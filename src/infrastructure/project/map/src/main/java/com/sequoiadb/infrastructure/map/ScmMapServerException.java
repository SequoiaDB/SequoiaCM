...
public class ScmMapServerException extends Exception {

    private static final long serialVersionUID = 2254727849190358010L;

    protected ScmMapError error;

    public ScmMapServerException(ScmMapError error, String message, Throwable cause) {
        super(message, cause);
        if (error == null) {
    ...
    public ScmMapServerException(ScmMapError error, String message) {
        super(message);
        if (error == null) {
            throw new NullPointerException("error is null");
        }
    ...
    public ScmMapServerException(int errorCode, String message) {
        super(message);
