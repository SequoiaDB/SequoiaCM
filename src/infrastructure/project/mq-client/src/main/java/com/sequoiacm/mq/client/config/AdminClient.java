...
public class AdminClient {
    ...
    public List<ConsumerGroupDetail> listGroup() throws MqException {
        return client.getGroups(null);
    }

    public List<ConsumerGroupDetail> listGroup(String topicName) throws MqException {
        return client.getGroups(topicName);
    }

    public long peekLatestMsgId(String topic) throws MqException {
        try {
            MessageInternal latestMsg = client.peekLatestMsg(topic);
            return latestMsg.getId();
        }
        catch (MqException e) {
            if (e.getError() == MqError.TOPIC_IS_EMPTY) {
                return -1;
            }
            throw e;
        }
