...
public class SdbBatchAccessor extends SdbMetaAccessor implements MetaBatchAccessor {
    ...
    public void createSubTable(ScmShardingType shardingType, Date createDate)
        ...
        try {
            ...
            upperb.put(FieldName.Batch.FIELD_INNER_CREATE_MONTH, range.getUpBound());

            options = new BasicBSONObject();
            options.put("LowBound", lowb);
            options.put("UpBound", upperb);
            logger.info("attaching cl:cl={}.{}, option={}", getCsName(), subClName, options);
            SequoiadbHelper.attachCL(sdb, getCsName(), getClName(), getCsName() + "." + subClName,
                    options);
        }
        finally {
