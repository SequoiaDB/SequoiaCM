...
public class CephS3DataReaderImpl implements ScmDataReader {
    ...
    public CephS3DataReaderImpl(String bucketName, String key, ScmService service, CephS3DataLocation cephS3DataLocation)
            throws CephS3Exception {
        this.dataService = (CephS3DataService) service;
        conn = dataService.getConn(cephS3DataLocation.getPrimaryUserInfo(), cephS3DataLocation.getStandbyUserInfo());
        if (conn == null) {
        ...
        try {
            ...
            conn = dataService.releaseAndTryGetAnotherConn(conn, cephS3DataLocation.getPrimaryUserInfo(), cephS3DataLocation.getStandbyUserInfo());
            if (conn == null) {
                throw e;
            }
            logger.warn(
