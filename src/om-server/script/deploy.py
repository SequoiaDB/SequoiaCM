...
def scm_admin(cmd):
    command("%s%s%s %s" % (BIN_PATH, os.sep, OM_ADMIN, cmd))


def scm_ctl(cmd):
    command("%s%s%s %s" % (BIN_PATH, os.sep, OM_CTL, cmd))
...
def deploy_scm(config, bin_path="." + os.sep + "bin", dryrun=False):
    ...
    if bin_path is not None:
        BIN_PATH = bin_path
    if 'om-server' in config:
        if not create_nodes('om-server', config['om-server']):
