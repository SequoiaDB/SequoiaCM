...
rootDir = sys.path[0] + os.sep + ".." + os.sep
OM_CTL = "omctl.sh"
OM_ADMIN = "omadmin.sh"
...
def install():
    if os.path.exists(rootDir+"/deploy/bin/" + OM_ADMIN):
        exec_cmd(rootDir+"/deploy/bin/" + OM_CTL + " stop --type all")
    else:
        print("INFO: " + rootDir+"/deploy/bin/" + OM_CTL + " notexist, ignore to stop node" )
    ...
    exec_cmd("cp -r " + rootDir + "/project/common/sequoiacm-om-assembly/target/sequoiacm-om-*-release/sequoiacm-om/* " + rootDir + "/deploy/")
    ...
    exec_cmd(rootDir+"/deploy/bin/" + OM_CTL + " start --type all")
