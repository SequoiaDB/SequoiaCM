// see http://jira:8080/browse/SEQUOIACM-675
...
public class NioBlockingSelector {
    ...
    static {
        log.info("sequoiacm NioBlockingSelector.class loaded");
    }
    ...
    /**
     * Performs a blocking write using the bytebuffer for data to be written If the
     * <code>selector</code> parameter is null, then it will perform a busy write
     * that could take up a lot of CPU cycles.
     * 
     * @param buf
     *            ByteBuffer - the buffer containing the data, we will write as long
     *            as <code>(buf.hasRemaining()==true)</code>
     * @param socket
     *            SocketChannel - the socket to write data to
     * @param writeTimeout
     *            long - the timeout for this write operation in milliseconds, -1
     *            means no timeout
     ...
     * 
     * @param buf
     *            ByteBuffer - the buffer containing the data, we will read as until
     *            we have read at least one byte or we timed out
     * @param socket
     *            SocketChannel - the socket to write data to
     * @param readTimeout
    ...
    protected static class BlockPoller extends Thread {
        ...
        @Override
        public void run() {
            log.info("sequoiacm NioBlockingSelector is running");
            while (run) {
                try {
                    events();
                    int keyCount = 0;
