...
import com.sequoiacm.s3.tools.command.ScmCreateNodeToolImpl;
import com.sequoiacm.s3.tools.command.ScmHelpToolImpl;
import com.sequoiacm.s3.tools.command.ScmTool;
import com.sequoiacm.s3.tools.common.ScmCommon;
...
public class S3Admin {
    private static Logger logger = LoggerFactory.getLogger(S3Admin.class.getName());
    public final static String helpMsg = "usage: confadmin <subcommand> [options] [args]" + "\r\n"
            + "Type 's3admin help [subcommand]' for help on a specific subcommand" + "\r\n"
            + "Type 's3admin --version' to see the program version" + "\r\n"
    ...
    private static ScmTool getInstanceByToolName(String toolName) throws ScmToolsException {
        ...
        else if (toolName.equals("init-region")) {
            instance = new InitRegionToolImpl();
        }
        else if (toolName.equals("refresh-accesskey")) {
            instance = new RefreshAccesskeyToolImpl();
        }
        else if (toolName.equals("help")) {
            instance = new ScmHelpToolImpl(S3Admin.class, false);
        }
        else if (toolName.equals("helpfull")) {
            instance = new ScmHelpToolImpl(S3Admin.class, true);
        }
        else {
