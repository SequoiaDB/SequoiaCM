...
public class LockManager {
    ...
    private boolean initLockFactory() throws MqException {
        try {
            ...
            closeLockFactory();
            throw new MqException(MqError.LOCK_ERROR, "failed to init lock manager", e);
        }
        return true;
    }
    ...
    public ScmLock acquiresReadLock(LockPath lockPath) throws MqException {
        try {
            checkAndInit();
            ...
            throw new MqException(MqError.LOCK_ERROR,
                    "failed to acquires readlock:locakPath=" + Arrays.toString(lockPath.getPath()),
                    e);
        }
    }

    public ScmLock acquiresWriteLock(LockPath lockPath) throws MqException {
        try {
            checkAndInit();
    ...
    public ScmLock acquiresLock(LockPath lockPath) throws MqException {
        try {
            checkAndInit();
    ...
    public ScmLock tryAcquiresLock(LockPath lockPath) throws MqException {
        try {
            checkAndInit();
            ScmLock lock = innerFactory.createLock(lockPath.getPath());
            if (lock.tryLock()) {
    ...
    private void checkAndInit() throws MqException {
        if (innerFactory == null) {
            boolean initSuccess = initLockFactory();
            if (!initSuccess) {
                throw new MqException(MqError.LOCK_ERROR, "failed to init lock manager");
            }
        }
    }
}
