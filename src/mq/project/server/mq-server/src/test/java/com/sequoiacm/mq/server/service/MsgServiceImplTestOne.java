...
@SpringBootTest(webEnvironment =  SpringBootTest.WebEnvironment.NONE,classes = {
        ...
        TestMsgRepository.class,
        TestPartitionRepository.class,
        TestTopicRepository.class
})
public class MsgServiceImplTestOne {
    @Autowired
    ...
    @Test
    public void checkMsgConsumedSituationOne() throws MqException{
        /*
         * 测试场景：
         * 三个分区分别为0分区、1分区、2分区（该分区不存在，我们不构建出来，
         * 但是使一些消息落在这个分区里，模拟2分区被删除），
         * 分区消息分配如下：
         * 0分区：1 2 3 4
         * 1分区：5 6 7 8 10
         * 2分区：9 11 12 (该分区不存在)
         ...
         * p1->10
         *
         * 查询消息 9 11 12 13 是否被消费
         * 查询消息 9 11 12 13 及其之前的消息是否被消费
         *
         * 预期结果：
         * true true true false
         * true true true false
         */
        Assert.assertTrue(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",9,false));
        ...
        Assert.assertTrue(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",12,false));
        ...
        Assert.assertTrue(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",9,true));
        Assert.assertTrue(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",11,true));
        Assert.assertTrue(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",12,true));
        Assert.assertFalse(msgService.checkMsgConsumed("sequoiadb_ws_FULLTEXT_FILE_OP","fulltext-server-sequoiadb_ws_FULLTEXT_FILE_OP",13,false));
    }
}

@Repository
class TestTopicRepository implements TopicRepository{
    @Override
...
@Repository
class TestPartitionRepository implements PartitionRepository{
    private List<ConsumerPartitionInfo> consumerPartitionInfoList;

    public TestPartitionRepository() {
        // 构建分区
        ...
        for(int i = 0;i<2;i++){
            ...
            if(i==0){
                ...
                o.put("last_delevered_id",4L);
            }else{
                o.put("partition_num",1);
                o.put("last_delevered_id",10L);
            }
...
@Repository
class TestMsgRepository implements MsgRepository{
    // 构造测试用的message
    private List<MessageInternal> messageInternalList;

    public TestMsgRepository() {
        // 构建消息
