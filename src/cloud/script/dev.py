...
def install():
    ...
    if os.path.exists(ctl_cmd):
        tools_jar = root_dir + os.sep + ".." + os.sep + "project" \
                    ...
                    + os.sep + "tools" \
                    + os.sep + "target" \
                    + os.sep + "sequoiacm-cloud-tools-*.jar "
        exec_cmd("cp -r " + tools_jar + deploy_dir + os.sep + "jars")
        exec_cmd(ctl_cmd + " stop --type all -f")
    ...
    package_path = root_dir + os.sep + ".." + os.sep + "project" \
        ...
        + os.sep + "assembly" \
        + os.sep + "target" \
        + os.sep + "sequoiacm-cloud-*-release" \
        + os.sep + "sequoiacm-cloud" \
        + os.sep + "* "
    ...
    exec_cmd(ctl_cmd + " stop --type all -f")
    # start
    exec_cmd(ctl_cmd + " start --type all --timeout 240")
