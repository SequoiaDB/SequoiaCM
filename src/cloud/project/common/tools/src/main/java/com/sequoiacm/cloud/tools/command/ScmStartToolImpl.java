...
import com.sequoiacm.cloud.tools.element.ScmNodeInfo;
...
public class ScmStartToolImpl implements ScmTool {
    public  int TIME_WAIT_PROCESS_RUNNING = 10000; // 10s
    ...
    public void process(String[] args) throws ScmToolsException {
        ...
        if (commandLine.hasOption(OPT_LONG_TIMEOUT)) {
            TIME_WAIT_PROCESS_STATUS_NORMAL = ScmCommandUtil.getTimeout(commandLine, OPT_LONG_TIMEOUT);
        }
        ...
        for (Integer key : needStartMap.keySet()) {
            try {
                ...
                if (pid == -1) {
                    ...
                    if (status.equals("UP")) {
                        System.out.println("Success:" + needStartMap.get(key)
                        + " is already started (" + pid + ")");
                        logger.info("Success:" + needStartMap.get(key) + " is already started ("
                                + pid + ")");
                        success++;
                    }
                    else {
                ...
                try {
                    ...
                    if (pid != -1) {
                        ...
                        if (runningStatus.equals("UP")) {
                            System.out.println("Success:" + node + " is successfully started ("
                                    + pid + ")");
                            logger.info("Success:" + node + " is successfully started (" + pid
                                    + ")");
                            success++;
                            it.remove();
