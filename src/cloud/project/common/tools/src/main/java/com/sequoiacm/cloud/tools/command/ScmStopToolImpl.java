...
public class ScmStopToolImpl implements ScmTool {
    private int STOP_TIMEOUT = 30*1000;
        ...
        for (Integer key : needStopMap.keySet()) {
            try {
                if (executor.getNodePid(key) != -1) {
                    ...
                    logger.info("Success:" + needStopMap.get(key) + " is already stopped");
                    System.out.println("Success:" + needStopMap.get(key) + " is already stopped");
                    success++;
                }
            }
            catch (ScmToolsException e) {
        ...
        while (true) {
            ...
            while (it.hasNext()) {
                ...
                try {
                    ...
                    if (pid == -1) {
                        logger.info("Success:" + node + " is successfully stopped");
                        System.out.println("Success:" + node + " is successfully stopped");
                        it.remove();
                        success++;
                    }
                }
