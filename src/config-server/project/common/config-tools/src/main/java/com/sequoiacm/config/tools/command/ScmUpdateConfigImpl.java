        ...
        try {
            ...
            if (failures.size() > 0) {
                for (ScmUpdateConfResult result : failures) {
                    System.out.println("update config failed: serviceName="
                            + result.getServiceName() + ", instance=" + result.getInstance()
                            + ", errorMessage=" + result.getErrorMessage());
                    logger.error(
                            "update config failed: serviceName={},nodeName={},config={},errorMessage={}",
            ...
            if (successes.size() > 0) {
                for (ScmUpdateConfResult result : successes) {
                    System.out.println("update config success: serviceName="
                            + result.getServiceName() + ", instance=" + result.getInstance());
                    logger.info("update config success: serviceName={},nodeName={},config={}",
                            result.getServiceName(), result.getInstance(), config);
                }
            }
    ...
    private ScmUpdateConfResultSet updateConfigProp(ScmSession ss, Integer type, String name,
        ...
        if (index != -1) {
            ...
            throw new ScmToolsException(
                    "invalid argument, config item only can contain one equal sign, please refer exam: key=value."
                            + " your origin value: " + config,
