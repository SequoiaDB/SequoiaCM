...
public class DeleteWorkspaceDao {
    ...
    public ScmConfOperateResult delete(WorkspaceFilter filter) throws ScmConfigException {
        ...
        try {
            ...
            if (oldWsRec != null) {
                versionDao.deleteVersion(ScmBusinessTypeDefine.WORKSPACE, filter.getWsName(),
                        transaction);

                // delete metadata version
                versionDao.deleteVersion(ScmBusinessTypeDefine.META_DATA, filter.getWsName(),
                        transaction);
        ...
        try {
            workspaceMetaservice.deleteWorkspaceMetaTable(oldWsRec, false);
        }
        catch (Exception e) {
    ...
    private ScmConfEvent createMetaDataDeleteEvent(WorkspaceFilter filter) {
        MetaDataNotifyOption notifycation = new MetaDataNotifyOption(filter.getWsName(), null);
        return new ScmConfEvent(ScmBusinessTypeDefine.META_DATA,EventType.DELTE, notifycation);
    }
    ...
    private ScmConfEvent createWorkspaceDeleteEvent(WorkspaceFilter filter) {
        ...
        return new ScmConfEvent(ScmBusinessTypeDefine.WORKSPACE, EventType.DELTE, notifycation);
    }
}
