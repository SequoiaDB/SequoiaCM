package com.sequoiacm.config.framework.config.quota.dao;
...
import com.sequoiacm.config.framework.config.quota.metasource.QuotaMetaService;
...
public class QuotaDao {
    ...
    private void create(QuotaConfig config) throws ScmConfigException {
        try {
            try {
                ...
                BSONObject bsonObject = configEntityTranslator.toConfigBSON(config);
    ...
    private ScmConfEvent createQuotaEvent(QuotaConfig quotaConfig) {
        QuotaNotifyOption notifyCation = new QuotaNotifyOption(quotaConfig.getType(),
                quotaConfig.getName(), 1);
        return new ScmConfEvent(ScmBusinessTypeDefine.QUOTA, EventType.CREATE, notifyCation);
    }

    public ScmConfOperateResult deleteQuota(QuotaFilter filter) throws ScmConfigException {
        logger.info("start to delete quota:filter={}", filter);
        QuotaConfig quotaConfig = delete(filter);
        logger.info("delete quota success:filter={}", filter);
    ...
    private ScmConfEvent deleteQuotaEvent(QuotaConfig quotaConfig) {
        QuotaNotifyOption notifyCation = new QuotaNotifyOption(quotaConfig.getType(),
                quotaConfig.getName(), 1);
        return new ScmConfEvent(ScmBusinessTypeDefine.QUOTA, EventType.DELTE, notifyCation);
    }
    ...
    private QuotaConfig delete(QuotaFilter filter) throws ScmConfigException {
        ...
        try {
            ...
            if (filter.getName() != null) {
                cond.put(FieldName.Quota.NAME, filter.getName());
            }
            if (filter.getType() != null) {
                cond.put(FieldName.Quota.TYPE, filter.getType());
            }
            quotaCursor = quotaTable.query(cond, null, null);
            if (quotaCursor.hasNext()) {
                ...
                QuotaConfig quotaConfig = (QuotaConfig) configEntityTranslator
                        .fromConfigBSON(ScmBusinessTypeDefine.QUOTA, quotaBson);

                quotaTable.delete(cond);
                return quotaConfig;
            }
            throw new ScmConfigException(ScmConfError.QUOTA_NOT_EXIST,
                    "quota is not exist: filter=" + filter);
        }
        catch (ScmConfigException e) {
            throw e;
        }
            ...
            throw new ScmConfigException(ScmConfError.SYSTEM_ERROR,
                    "delete quota failed:filter =" + filter, e);
        }
        finally {
            if (quotaCursor != null) {
                quotaCursor.close();
    ...
    public ScmConfOperateResult updateQuota(QuotaUpdater updator) throws ScmConfigException {
        logger.info("start to update quota:bson={}", updator);
        ScmConfOperateResult scmConfOperateResult = update(updator);
        logger.info("update quota success:bson={}", updator);
        return scmConfOperateResult;
    }

    public ScmConfOperateResult update(QuotaUpdater updator) throws ScmConfigException {
        ...
        try {
            ...
            if (quotaBSON != null) {
                ...
                int newVersion = BsonUtils.getNumberChecked(newQuotaConfig, FieldName.Quota.VERSION)
                        .intValue();
                QuotaConfig newConfig = (QuotaConfig) configEntityTranslator
                        .fromConfigBSON(ScmBusinessTypeDefine.QUOTA, newQuotaConfig);
            ...
            throw new ScmConfigException(ScmConfError.SYSTEM_ERROR,
                    "failed to update quota:" + updator,
                    e);
        }
    }

    private BSONObject createUpdateBson(QuotaUpdater updator) {
    ...
    private ScmConfEvent updateQuotaEvent(QuotaConfig quotaConfig, Integer newVersion) {
        QuotaNotifyOption notifyOption = new QuotaNotifyOption(quotaConfig.getType(),
                quotaConfig.getName(), newVersion);
        return new ScmConfEvent(ScmBusinessTypeDefine.QUOTA, EventType.UPDATE, notifyOption);
    }
    ...
    public List<Config> getQuotas(QuotaFilter filter) throws MetasourceException {
        ...
        try {
            ...
            cursor = quotaTable.query(cond, null,
                    null);
            while (cursor.hasNext()) {
                BSONObject bson = cursor.getNext();
                QuotaConfig quotaConfig = (QuotaConfig) configEntityTranslator
                        .fromConfigBSON(ScmBusinessTypeDefine.QUOTA, bson);
                res.add(quotaConfig);
            }
        }
        finally {
    ...
    public List<Version> getVersions(VersionFilter versionFilter) throws ScmConfigException {
        List<Version> res = new ArrayList<>();
        SequoiadbTableDao quotaTable = metaService.getQuotaTable();
        MetaCursor cursor = null;
        try {
            BasicBSONObject condition = new BasicBSONObject();
            if (versionFilter.getBusinessNames() != null
                    && !versionFilter.getBusinessNames().isEmpty()) {
                BasicBSONList orArr = new BasicBSONList();
                for (String businessName : versionFilter.getBusinessNames()) {
                    BasicBSONObject orObj = new BasicBSONObject();
                    orObj.put(FieldName.Quota.TYPE,
                            QuotaConfig.getTypeFromBusinessName(businessName));
                    orObj.put(FieldName.Quota.NAME,
                            QuotaConfig.getNameFromBusinessName(businessName));
                    orArr.add(orObj);
                }
                condition.put("$or", orArr);
            }
            cursor = quotaTable.query(condition, null, null);
            while (cursor.hasNext()) {
                BSONObject bson = cursor.getNext();
                ...
                int versionNum = BsonUtils.getNumberChecked(bson, FieldName.Quota.VERSION)
                        .intValue();
                Version version = new Version(ScmBusinessTypeDefine.QUOTA,
